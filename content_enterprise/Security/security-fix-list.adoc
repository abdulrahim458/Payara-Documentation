...
|https://nvd.nist.gov/vuln/detail/CVE-2026-24457[CVE-2026-24457^] | 9.1 | FIXED | An unsafe parsing of OpenMQ's configuration, allows a remote attacker to read arbitrary files from a MQ Broker's server. A full exploitation could read unauthorized files of the OpenMQ’s host OS. In some scenarios RCE could be achieved. | 6.38.0 | Fixed by patching OpenMQ.
