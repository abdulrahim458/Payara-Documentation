...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 --read-only --rm -v /opt/payara -v /tmp -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{page-version}
----