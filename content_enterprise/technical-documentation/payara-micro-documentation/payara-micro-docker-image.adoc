...
:page-description: Azul Payara Micro Docker image overview — official Docker images, supported tags, environment variables, and configuration options for containerized Payara Micro deployments.
...
[source,text]
----
docker run -p 8080:8080 nexus.payara.fish:5000/payara/micro:{release-id}
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id}
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id} --deploy /opt/payara/deployments/myapplication.war
----
...
----
FROM nexus.payara.fish:5000/payara/micro:{release-id}
COPY myapplication.war $DEPLOY_DIR
----

[#custom-crac-image]
=== Create Docker Images Utilising CRaC ===

CAUTION: This feature is in Tech Preview, and not recommended for use in production environments.

Below is an example of creating a custom image to use Payara Micro with a CRaC JDK.
This dockerfile makes use of your own Payara Micro installation and will create the checkpoint as part of the image.
Containers created from the image will restore the application from the checkpoint.
...
ENV PAYARA_DIR=${HOME_DIR}/micro \
    DEPLOY_DIR=${HOME_DIR}/deployments \
    CHECKPOINT_DIR=${HOME_DIR}/checkpoint
...
COPY --chown=payara:payara payara-micro.jar .
COPY --chown=payara:payara myapplication.war $DEPLOY_DIR

RUN java \
    ${JVM_ARGS} \
    -Dfish.payara.crac.checkpointAfterDeployment=true \
...
SHELL ["/bin/bash", "-c"]
ENTRYPOINT java -XX:CRaCEngine=warp -XX:CRaCRestoreFrom=${CHECKPOINT_DIR}
----

To build and run this docker image, the following commands can be used.
[source]
----
docker build . -t payara-enterprise-micro-crac-example
docker run -d -p 8080:8080 -p 6900:6900 --name payara-enterprise-micro-crac-example-1 payara-enterprise-micro-crac-example
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 nexus.payara.fish:5000/payara/micro:{release-id} --deployFromGAV "fish.payara.examples:my-application:1.0-SNAPSHOT" --additionalRepository https://172.17.0.10/content/repositories/snapshots
----
...
[source,text]
----
docker run -p 8080:8080 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id} --deploymentDir /opt/payara/deployments --contextroot myRoot
----
...
docker run -p 8080:8080 \
 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id} --deploy /opt/payara/deployments/myapplication.war --contextroot ROOT
----
...
Payara Micro will start in "clustering" mode by booting up the xref:payara-micro-configuration-and-management/micro-management/clustering.adoc[Data Grid], allowing other instances reachable in the network to join the grid automatically. 
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id} --noHazelcast
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id} --noCluster
----
...
a|`MEM_MAX_RAM_PERCENTAGE`
a| Value for the JVM argument `-XX:MaxRAMPercentage` which indicates the percentage of memory assigned to the container that can be used by the Java process
a| `70`

a|`MEM_XSS`
a| Value for the JVM argument `-Xss` which controls the stack size
a| `512K`

a|`JVM_ARGS`
| Additional JVM arguments which will be used to configure the Payara Servers DAS JVM settings
a|[.breakable]`-Djdk.util.zip.disableZip64ExtraFieldValidation=true`
...
The Payara Micro Docker image requires write access to the `HOME_DIR`, and `/tmp` directories to function.
If you wish to use `--read-only` Docker containers (or the equivalent `--readOnlyRootFilesystem` property in Kubernetes), you must mount volumes at these directories which the `payara` user has write access to.
This can be done using anonymous or tmpfs volumes if you do not wish to persist the data outside the life of the container.
Named volumes or bind mounts may be used if you wish the data to persist outside the life of the container.

Here is an example `run` command for a read-only Docker container which will delete itself and its anonymous volumes when stopped, deploying all artefacts located in the `~/payara-micro/applications` directory:

[source, shell, subs=attributes+]
----
docker run -p 8080:8080 --read-only --rm -v /opt/payara -v /tmp -v ~/payara-micro/applications:/opt/payara/deployments nexus.payara.fish:5000/payara/micro:{release-id}
----