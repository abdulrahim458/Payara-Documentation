...
:page-description: Azul Payara Server Docker image overview — official Docker images, supported tags, environment variables, and volume configuration for containerized Payara Server deployments.
:ordinal: 10
...
[source,text]
----
docker run -p 8080:8080 nexus.payara.fish:5000/payara/server-full:{release-id}
----
...
[source, shell, subs=attributes+]
----
docker run -p 4848:4848 -p 8080:8080 nexus.payara.fish:5000/payara/server-full:{release-id}
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara/apps:/opt/payara/deployments nexus.payara.fish:5000/payara/server-full:{release-id}
----
...
----
FROM nexus.payara.fish:5000/payara/server-full:{release-id}
...
a| `$CONFIG_DIR/post-boot-commands.asadmin` and `$CONFIG_DIR/pre-boot-commands.asadmin`

a|`POSTBOOT_COMMANDS`
a|The name of the file containing commands to be executed after the domain is started. This is the file written to in the `generate_deploy_commands.sh` script. The file will not be modified by the Docker container
a| `$CONFIG_DIR/post-boot-commands.asadmin`
...
a|`DEPLOY_PROPS`
a| Specifies a list of properties to be passed with the deploy commands generated in the `generate_deploy_commands.sh` script
|

a|`PAYARA_ARGS`
a| Additional arguments passed to the `start-domain` command that starts the DAS. _Use with caution._
|
...
a|`HOME_DIR`
a| The home directory for the `payara` user
a| `/opt/payara`

a|`PAYARA_DIR`
| The root directory of the Payara installation
a| `/opt/payara/appserver`

a|`SCRIPT_DIR`
a| The directory where the `generate_deploy_commands.sh` and `startInForeground.sh` scripts can be found
a| `/opt/payara/scripts`
...
a|`DEPLOY_DIR`
a| The directory where applications are searched for in `generate_deploy_commands.sh` script
a| `/opt/payara/deployments`
...
a|`PASSWORD_FILE`
a| The location of the password file for asadmin. This can be passed to asadmin using the `--passwordfile` parameter
a| `/opt/payara/passwordFile`

a|`DOMAIN_NAME`
| The name of the domain running within the container
a| `production`

a|`AS_ADMIN_MASTERPASSWORD`
a| The master password to pass to Payara Server. This is overridden if one is specified in the `$PASSWORD_FILE`
a| `changeit`
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v /local/path/with/boot/file:/config -e POSTBOOT_COMMANDS=/config/post-boot-commands.asadmin nexus.payara.fish:5000/payara/server-full:{release-id}
----
...
By default, Payara Server logs are written to the Docker standard output instead of the usual server.log file. However, you can configure the server to also log to this server.log file using the following command with a pre-boot command file:
...
docker run -p 8080:8080 nexus.payara.fish:5000/payara/server-full:{release-id} > payara-logs.txt 2>&1
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -it nexus.payara.fish:5000/payara/server-full:{release-id} bash
----

[#read-only]
=== Read Only Support Considerations

The Payara Server Docker image requires write access to the `PAYARA_DIR`, `CONFIG_DIR`, and `/tmp` directories to function.
...
Here is an example `run` command for a read-only Docker container which will delete itself and its anonymous volumes when stopped:

[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -p 4848:4848 --read-only --rm -v /opt/payara -v /tmp nexus.payara.fish:5000/payara/server-full:{release-id}
----
