...
docker run -p 8080:8080 nexus.payara.fish:5000/payara/server-full:{page-version} > payara-logs.txt 2>&1
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -p 4848:4848 --read-only --rm -v /opt/payara -v /tmp nexus.payara.fish:5000/payara/server-full:{page-version}
----
