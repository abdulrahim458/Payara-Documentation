...
:page-description: Upgrade Azul Payara Server — guides for upgrading between Payara Server versions using the Upgrade Tool, backup and restore method, or directory migration.
...
NOTE: If you are migrating from Azul Payara Community to Azul Payara, you must use the backup and restore method.

. Backing up and restoring the existing configuration to a new installation.
xref:backup-and-restore-method.adoc[Read Backup and Restore Method]

. Maintaining completely separate domain and node directories and pointing the new version to the existing directories.
xref:domain-and-node-directories-method.adoc[Read Domain and Node Directories Method]

. Using the Payara Upgrade Tool.
xref:upgrade-tool.adoc[Read Payara Upgrade Tool Method]
