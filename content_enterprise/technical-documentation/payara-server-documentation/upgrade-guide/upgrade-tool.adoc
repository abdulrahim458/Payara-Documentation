...
:page-description: Payara Server Upgrade Tool — automated tool for migrating domain configuration, deployed applications, and data from an older Payara Server version to a newer one.
:ordinal: 1
...
|
a|Yes, unless `--usedownloaded` is specified
...
a|`--stage`
|Boolean
|Determines if the upgrade should be installed in-place, or staged into "_x.new_" directories. Scripts are provided that can be used to apply and rollback the staged install. This option is automatically enabled when running the command on Windows.
...
a| `${as-install}/domains`
| No

a|`--usedownloaded`
|String
a|The path to the local Payara Server zip archive to use for upgrading instead of downloading from the Payara Enterprise repository. When this parameter is specified, the `--username`, `--password`, and `--version` parameters are not required.
|
|No
