= Diagnostics Tool
:page-description: Payara Server Diagnostics Tool — collect server configuration, thread dumps, log files, and JVM metrics into a diagnostic bundle for troubleshooting Azul Payara issues.
...
Diagnostics can be collected directly from a running instance in case its corresponding DAS it's not available by specifying the admin port of the target instance (see xref:../command-reference/asadmin.adoc#options[Asadmin Options]), like this:
