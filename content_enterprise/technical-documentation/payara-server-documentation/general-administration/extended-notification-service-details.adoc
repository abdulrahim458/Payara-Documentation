...
:page-description: Extended Notification Service in Azul Payara Server — configure Datadog, Slack, Discord, email, Teams, and SNMP notifiers for JVM and application health alerts.
:ordinal: 11
...
You will need a `JavaMail` session to send the notifications; instructions on setting up a `JavaMail` session can be found xref:administering-the-jakarta-mail-service.adoc[in this section].
...
* `Dynamic`: Sets whether the notifier will be enabled without a restart, `true`
enables it immediately, while `false` requires a restart.
* `JavaMail JNDI Name`: Sets the name of the Java Naming and Directory Interface (JNDI) of the
configured `JavaMail` session. The parameter name is `--jndiName` in the asadmin command.
* `Target Address`: Sets the email address which will receive the email notifications. Currently
...
Payara Server includes a special notifier to send notifications from the xref:administering-the-notification-service.adoc[Notification service] to the New Relic **Insights service** using its remote API.
...
----
  ...
  common: &default_settings
    ...
    # ...
    app_name: Payara Server {release-id}
...
* Backed up start script to /opt/{release-id}/glassfish/domains/domain1/config/domain.xml.20170613_180108
* Added agent switch to start script /opt/{release-id}/glassfish/domains/domain1/config/domain.xml

* No need to create New Relic configuration file because:
 A config file already exists: /opt/{release-id}/glassfish/domains/domain1/newrelic/newrelic.yml
...
[source, log, subs=attributes+]
----
INFO: New Relic Agent: Loading configuration file "/opt/{release-id}/glassfish/domains/domain1/newrelic/newrelic.yml"
INFO: New Relic Agent: Writing to log file: /opt/{release-id}/glassfish/domains/domain1/newrelic/logs/newrelic_agent.log
----
...
Payara Server includes a special notifier to send notifications from the xref:administering-the-notification-service.adoc[Notification service] to a Datadog account by means of an API key.
