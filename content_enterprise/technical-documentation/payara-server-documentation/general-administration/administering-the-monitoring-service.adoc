...
:page-description: Administer the Monitoring Service in Azul Payara Server — configure JMX monitoring, MBean statistics, and monitoring levels for server components and deployed applications.
:ordinal: 8
...
You can also use the xref:../command-reference/set.adoc#set[`set`] subcommand to enable monitoring for a module. Using the `set` command is not a dynamic procedure, so you need to restart Payara Server for your changes to take effect.
...
----
*  Enable monitoring by using the xref:../command-reference/enable-monitoring.adoc[`enable-monitoring`] subcommand. Server restart is not required.
...
+
As the io
*  Disable monitoring for a service or module by using the xref:../command-reference/disable-monitoring.adoc[`disable-monitoring`] subcommand. +
...
. Request the monitoring data by using the xref:../command-reference/monitor.adoc[`monitor`] subcommand.
...
*  List the objects that are enabled for monitoring by using the xref:../command-reference/list.adoc[`list`] subcommand. +
...
server.thread-pools
----
*  Get data for a monitored component or service by using the xref:../command-reference/get.adoc[`get`] subcommand.
