...
Although an object is monitorable, it is not necessarily being actively monitored. For instructions on activating monitoring, see xref:administering-the-monitoring-service.adoc#configuring-monitoring[Configuring Monitoring].
...
For available statistics, see xref:administering-the-monitoring-service.adoc#ejb-statistics[EJB Statistics], xref:administering-the-monitoring-service.adoc#jersey-statistics[Jersey Statistics], and xref:administering-the-monitoring-service.adoc#web-statistics[Web Statistics].
...
An example dotted name might be `server.connector-service.resource-adapter-1.connection-pools.pool-1`. For available statistics, see xref:administering-the-monitoring-service.adoc#jmsconnector-service-statistics[JMS/Connector Service Statistics].
...
An example dotted name under the virutal-server node might be `server.http-service.virtual-server1.request.requestcount`. For available statistics, see xref:administering-the-monitoring-service.adoc#http-service-statistics[HTTP Service Statistics].
...
An example dotted name under the `memory` node might be `server.jvm.memory.maxheapsize`. For available statistics, see xref:administering-the-monitoring-service.adoc#jvm-statistics[JVM Statistics].
...
An example dotted name under the `network` node might be `server.network.admin-listener.keep-alive.maxrequests-count`. For available statistics, see xref:administering-the-monitoring-service.adoc#network-statistics[Network Statistics].
...
An example dotted name might be `server.orb.transport.connectioncache.inbound.connectionsidle-count`. For available statistics, see xref:administering-the-monitoring-service.adoc#orb-statistics-connection-manager[ORB Statistics (Connection Manager)].
...
An example dotted name might be `server.resources.jdbc-connection-pool1.numconnfree.count`. For available statistics, see xref:administering-the-monitoring-service.adoc#resource-statistics-connection-pool[Resource Statistics (Connection Pool)].
...
An example dotted name might be `server.security.realm.realmcount-starttime`. For available statistics, see xref:administering-the-monitoring-service.adoc#security-statistics[Security Statistics].
...
An example dotted name might be `server.thread-pool.orb.threadpool.thread-pool-1.averagetimeinqueue-current`. For available statistics, see xref:administering-the-monitoring-service.adoc#thread-pool-statistics[Thread Pool Statistics].
...
An example dotted name might be `server.tranaction-service.activeids`. For available statistics, see xref:administering-the-monitoring-service.adoc#transaction-service-statistics[Transaction Service Statistics].
...
An example dotted name for the `servlet` node might be `server.web.servlet.activeservletsloadedcount`. For available statistics, see xref:administering-the-monitoring-service.adoc#web-module-common-statistics[Web Module Common Statistics].
...
xref:administering-the-monitoring-service.adoc#configuring-monitoring[Configuring Monitoring].
* The `monitor type` subcommand is used to display basic data for a
particular type of monitorable object. For instructions, see xref:administering-the-monitoring-service.adoc#viewing-common-monitoring-data[Viewing Common Monitoring Data].
...
xref:administering-the-monitoring-service.adoc#guidelines-for-using-the-list-and-get-subcommands-for-monitoring[Guidelines for Using the `list` and `get` Subcommands for Monitoring].
* The `get` subcommand is used to display comprehensive data, such as the attributes and values for a dotted name. The `get` subcommand used with a wildcard parameter displays all available attributes for any monitorable object. For additional information, see xref:administering-the-monitoring-service.adoc#guidelines-for-using-the-list-and-get-subcommands-for-monitoring[Guidelines for Using the `list` and `get` Subcommands for Monitoring].
...
You can also use the xref:technical-documentation/payara-server-documentation/command-reference/set.adoc#set[`set`] subcommand to enable monitoring for a module. Using the `set` command is not a dynamic procedure, so you need to restart Payara Server for your changes to take effect.
...
----
*  Enable monitoring by using the xref:technical-documentation/payara-server-documentation/command-reference/enable-monitoring.adoc[`enable-monitoring`] subcommand. Server restart is not required.
...
You can also use the xref:../command-reference/set-monitoring-level.adoc#set-monitoring-level[`set-monitoring-level`] subcommand to disable monitoring for a module. To do this, set its `--level` option to `OFF`.
...
+
As the io
*  Disable monitoring for a service or module by using the xref:technical-documentation/payara-server-documentation/command-reference/disable-monitoring.adoc[`disable-monitoring`] subcommand. +
...
A monitorable object must be configured for monitoring before you can display data on the object. See xref:administering-the-monitoring-service.adoc#to-enable-monitoring[To Enable Monitoring].

. Determine which type of monitorable object you want to monitor. +
Your choices for 5.0 are `jvm`, `httplistener`, and `webmodule`.
. Request the monitoring data by using the xref:technical-documentation/payara-server-documentation/command-reference/monitor.adoc[`monitor`] subcommand.
...
a|`list --monitor`
a|`server.resources.``connection-pool1`
a|No attributes, but a message saying "Use `get` subcommand with the `--monitor` option to view this node's attributes and values."
...
For detailed information on dotted names, see the xref:../command-reference/dotted-names.adoc[`dotted-names`] help page.
...
A monitorable object must be configured for monitoring before you can display information about the object. See xref:administering-the-monitoring-service.adoc#to-enable-monitoring[To Enable Monitoring] if needed.

*  List the objects that are enabled for monitoring by using the xref:technical-documentation/payara-server-documentation/command-reference/list.adoc[`list`] subcommand. +
...
server.thread-pools
----
*  Get data for a monitored component or service by using the xref:technical-documentation/payara-server-documentation/command-reference/get.adoc[`get`] subcommand.
...
EJBs fit into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#applications-tree-hierarchy[Applications Tree Hierarchy]. Use the following dotted name pattern to get EJB statistics for an application:
...
a|`cachehits`
|RangeStatistic
a|The number of times a user request found an entry in the cache.
...
a|`numpassivations`
|CountStatistic
a|Number of passivated beans. Applies only to stateful session beans.

a|`numpassivationerrors`
|CountStatistic
|Number of errors during passivation. Applies only to stateful session beans.

a|[.breakable]`numexpiredsessionsremoved`
|CountStatistic
|Number of expired sessions removed by the cleanup thread. Applies only to stateful session beans.

a|`numpassivationsuccess`
|CountStatistic
a|Number of times passivation completed successfully. Applies only to stateful session beans.
...
a|`createcount`
|CountStatistic
a|Number of times an EJB's `create` method is called.
...
a|`methodreadycount`
|RangeStatistic
a|Number of stateful or stateless session beans that are in the `MethodReady` state.

a|`passivecount`
|RangeStatistic
a|Number of stateful session beans that are in `Passive` state.
...
a|`removecount`
|CountStatistic
a|Number of times an EJB's `remove` method is called.
...
The HTTP service fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#http-service-tree-hierarchy[HTTP Service Tree Hierarchy].
...
Jersey fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#applications-tree-hierarchy[Applications Tree Hierarchy].
...
The JMS/Connector Service fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#jmscontainer-service-tree-hierarchy[JMS/Container Service Tree Hierarchy].
...
The JVM fits into the tree of objects as show in xref:administering-the-monitoring-service.adoc#jvm-tree-hierarchy[JVM Tree Hierarchy].
...
a|[.breakable]`objectpendingfinalizationcount`
|CountStatistic
...
a|`inputarguments-current`
|StringStatistic
a|Input arguments passed to the JVM; not including arguments to the `main` method

a|[.breakable]`managementspecversion-current`
|StringStatistic
...
a|`specvendor-current`
|StringStatistic
|JVM specification vendor

a|`specversion-current`
|StringStatistic
...
Network fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#network-tree-hierarchy[Network Tree Hierarchy].
...
a|`counttimeouts`
|CountStatistic
a|Number of keep-alive connections that timed out.
...
a|`peakqueued`
|CountStatistic
a|Largest number of connections that were in the queue simultaneously

a|`tickstotalqueued`
|CountStatistic
a|(Unsupported) Total number of ticks that connections have spent in the queue
...
The ORB fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#orb-tree-hierarchy[ORB Tree Hierarchy].
...
The connection pool resources fit into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#resources-tree-hierarchy[Resources Tree Hierarchy].
...
a|`numconnfailedvalidation`
|CountStatistic
a|Number of connections in the connection pool that failed validation from the start time until the
last sampling time
...
a|[.breakable]`numconnnotsuccessfullymatched`
|CountStatistic
...
a|[.breakable]`numconnsuccessfullymatched`
|CountStatistic
...
Security fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#security-tree-hierarchy[Security Tree Hierarchy].
...
a|`policyconfigurationcount`
|CountStatistic
|Number of policy configuration

a|`securitymanagercount`
|CountStatistic
...
a|`websecuritymanagercount`
|CountStatistic
|Number of security managers

a|[.breakable]`webpolicyconfigurationcount`
|CountStatistic
...
The thread pool fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#thread-pool-tree-hierarchy[Thread Pool Tree Hierarchy].
...
a|`blockedcount`
|CountStatistic
a|Total number of times that the thread entered the `BLOCKED` state.

a|`blockedtime`
|CountStatistic
a|Time elapsed (in milliseconds) since the thread entered the `BLOCKED` state. Returns -1 if thread contention monitoring is disabled.

a|`lockname`
|StringStatistic
a|String representation of the monitor lock that the thread is blocked to enter or waiting to be notified through the `Object.wait` method.
...
a|`waitedtime`
|CountStatistic
a|Elapsed time (in milliseconds) that the thread has been in a `WAITING` state. Returns -1 if thread contention monitoring is disabled.

a|`waitedcount`
|CountStatistic
a|Total number of times the thread was in `WAITING` or `TIMED_WAITING` states.
...
The web module fits into the tree of objects as shown in xref:administering-the-monitoring-service.adoc#web-tree-hierarchy[Web Tree Hierarchy].
...
a|[.breakable]`activeservletsloadedcount`
|RangeStatistic
|Number of currently loaded servlets

a|`servletprocessingtimes`
|CountStatistic
|Cumulative servlet processing times , in milliseconds

a|`totalservletsloadedcount`
|CountStatistic
...
*  Start the domain. For instructions, see xref:administering-domains.adoc#to-start-a-domain[To Start a Domain].
...
To see a non-exhaustive inventory of existing MBeans that are currently available for Payara Server, see xref:mbeans-inventory.adoc[here].
...
image:jmx-monitoring-service/enable_amx.png[alt="Enable AMX",width=100%]
