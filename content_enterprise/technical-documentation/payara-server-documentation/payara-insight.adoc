= Payara Insight
:page-description: Payara Insight — enterprise observability and monitoring dashboard for Azul Payara Server providing real-time metrics, request tracing, and application performance data.
:ordinal: 11
...
image::monitoring-console/mc_menu_left.png[Page Navigation,width=600px]

image::monitoring-console/mc_menu_left-min.png[Page Navigation (minimised),width=600px]
...
image::monitoring-console/mc_settings_app_general-h.png[General Application Settings,width=600px]
...
image::monitoring-console/mc_dialog_add-new-page.png[Add Page Dialog,width=600px]
...
image::monitoring-console/mc_settings_page_general-query-h.png[Query Page Settings,width=600px]
...
image::monitoring-console/mc_menu_delete.png[Delete a Page,width=600px]

Confirm the deletion.

image::monitoring-console/mc_dialog_delete-page.png[Delete a Page Confirmation Dialogue,width=600px]
...
image::monitoring-console/mc_menu_reset.png[Reset a Page,width=600px]

Confirm the reset.

image::monitoring-console/mc_dialog_reset-page.png[Reset a Page,width=600px]
...
image::monitoring-console/mc_sample_local-storage-chrome.png[Clear Local Storage in Chrome Browser,width=600px]
...
image::monitoring-console/mc_settings_page_general-h.png[Page Settings,width=600px]
...
image::monitoring-console/mc_dialog_sync-pages.png[Page Synchronisation,width=600px]
...
image::monitoring-console/mc_settings_app_pages-h.png[Page Synchronisation Application Settings,width=600px]
...
image::monitoring-console/mc_dialog_push-pages.png[Push All Pages Confirmation Dialogue,width=600px]
...
image::monitoring-console/mc_settings_page_general-h.png[General Page Settings,width=600px]
...
image::monitoring-console/mc_dialog_pull-page.png[Confirm Page Pull,width=600px]
image::monitoring-console/mc_dialog_push-page.png[Confirm Page Push,width=600px]
...
image::monitoring-console/mc_dialog_export-pages.png[Page Export Dialogue,width=600px]
...
image::monitoring-console/mc_dialog_import-pages.png[Page Import Dialogue,width=600px]
...
image::monitoring-console/mc_dialog_role-selection.png[Role Selection,width=100%]

The selection can be changed at any time in the _General_ application settings for _Role_.

image::monitoring-console/mc_settings_app_general-h.png[Role Setting,width=600px]
...
image::monitoring-console/mc_menu_right.png[Settings Menu Expanded,width=600px]
image::monitoring-console/mc_menu_right-min.png[Settings Menu Collapsed,width=600px]

This opens the _Add New Widget_ dialogue.

image::monitoring-console/mc_dialog_add-new-widget.png[Add New Widget Dialogue,width=100%]
...
image::monitoring-console/mc_settings_widget_general-h.png[Widget General Settings,width=600px]
...
image::monitoring-console/mc_settings_widget_data-h.png[Widget Data Settings,width=600px]
...
| Mode
| _(Default)_
a| If available for the type of widget the display mode can be switched between _Table_ and _List_.
...
| Series
| -
| The metric key as given when adding the metric, use pencil icon to edit

| Unit
a| `Count`
a| The unit controls how the raw number of a metric is interpreted and represented e.g. when displaying in axis labels.
...
| Unit 1/sec
a| `false`
| Whether to show the metric as a change per second (delta between two points in the series normalised to average delta per second).

| Upscaling
a| `(undefined)`
a| When defined all values are multiplied by this factor. This can e.g. be used to move a decimal range between zero and one to a percentage range `0-100`.

| Upscaling _decimal value_
a| `false`
a| Check to mark the metric as one that is provided as a decimal value during collection (floats or doubles and alike).
...
| Line Style
a| `(none checked)`
| Options on how to display the current value line in the graph: Check _Points_ to show individual data points on the line, check _Curvy_ to use Bézier curves instead of straight line connections.

| Background
| _Fill_
| When _Fill_ is checked the area between line and axis is filled. The background transparency can be configured globally in the _Appearance_ application settings.

| X-Axis
| _Labels_
| Whether to show labels for the x-axis.

| Y-Axis
a| `(undefined)`
| Set a minimum or maximum number for the y-axis instead. When not set range is derived automatically from the data points.

| Legend
| _Alphabetical_
a| The order applied to the legend. Possible choices:
...
| Limit
| (undefined)
| When set the maximum number of legend items shown (especially useful when ordering legend items)

| Limit Hide Constant Zero
a| `false`
| When set series that have been zero for a while are not shown in legend and line graph

| Coloring
| _Instance Name_
a| Used to select the colours the data is shown with.
...
image::monitoring-console/mc_settings_widget_extras-h.png[Widget Extras Settings,width=600px]
...
| History
|_None_
a| What period of additional aggregated data to show in the graph:
...
| Annotations
| (checked)
|  Whether to show annotations for in an alert table.

| Annotations (Fields)
| _(blank)_
| A comma or space separated list of fields to show for an annotation list or table. The given field order is reflected in the view.

| Aggregates
| (none checked)
| Check to add the all time minimum, maximum or average line to the graph

| Waterline
| (undefined)
| Adds a simple reference line that could mark an upper or lower limit. Enter in unit of y-axis, e.g. 20ms, 5s, 2GB, 4%, 123

| Threshold Reference
a| `Off`
a| Controls what value the _Alarming Threshold_ and _Critical Threshold_ are compared to.

Usually this is the most recent value but in some cases the all time minimum, maximum or average are useful too.

| Alarming Threshold
| (undefined)
a| The limit for the _"Alarming"_ state.
...
| Critical Threshold
| (undefined)
a| The limit of the _"Critical"_ state.
...
image::monitoring-console/mc_settings_widget_status-h.png[Widget Status Settings,width=600px]
...
a| `No Data`
a| `(undefined)`
| The message to show when no data for a metric is available. For example to hint about configurations needed to make the metric available.

a| `Alarming`
a| `(undefined)`
| The message to show when the threshold got exceeded and the status evaluates to _Alarming_.

a| `Critical`
a| `(undefined)`
| The message to show when the threshold got exceeded and the status evaluates to _Critical_.
...
image::monitoring-console/mc_settings_widget_alerts-h.png[Widget Alerts Settings,width=600px]
...
| Filter
| (none checked)
| Do not show (when checked) _Amber_ and/or _Red_ alerts, _Ongoing_ and/or _Stopped_ alerts, _Acknowledged_ and/or _Unacknowledged_ alerts.
...
image::monitoring-console/mc_sample_widget-alert.png[Example for an alert in line graph,width=600px]
...
image::monitoring-console/mc_sample_widget-alert-table.png[Example of alert table showing a subset of alerts,width=600px]

image::monitoring-console/mc_sample_page-alerts.png[Example of a "global" alert table shown all alerts (as used by the alert page),width=100%]
...
image::monitoring-console/mc_settings_widget_general-h.png[Remove Widget Button in General Widget Settings,width=600px]

Confirm you want to remove the widget.

image::monitoring-console/mc_dialog_delete-widget.png[Remove Widget Confirmation Dialogue,width=600px]
...
image::monitoring-console/mc_settings_page_general-h.png[Common Page Settings,width=600px]
...
| Name
| -
| The name as given by the user when the page was created or as defined in the page preset, only user-created pages can be renamed
...
| Number Of Columns
| 1
| The number of columns available to place widgets in
...
| Server Sync
| (checked)
a| `Auto`: Changes made by an administrator to the page or the widgets of this page are automatically pushed to the shared configuration if and only if the page has been explicitly pushed to the shared configuration at some point in the past.
...
| Max Size
a| `32`
| Maximum number of widgets added to the page when populating it from query matches (even if there are more metrics matching the query)

| Query Series
| -
| A series pattern used to query for metrics, matches are used to populate the page

| Query Interval
a| `1m`
| The interval in which the query is run and the page is repopulated with matches

| Filter Type
| -
| When selecting a type only metrics inferred to prefer that type are shown on the page
...
image::monitoring-console/mc_settings_app_general-h.png[Application General Settings,width=600px]
...
| Data Refresh
a| `2s`
| Interval duration used to update the currently shown page with new data (poll duration). To pause data update toggle the _play/pause_ toggle switch.

| Page Rotation
a| `1m`
| Duration each page is shown during a page rotation. To run or stop the rotation toggle the _on/off_ toggle switch. See section xref:#page-rotation[Page Rotation] for details.

| Role
| _User_
| Select the used user role. See section xref:#changing-user-role[Changing User Role] for details.

| Watches
| -
| Click the _Go to Watch Settings_ button to open the xref:#watch-management[Watch Management]
...
image::monitoring-console/mc_settings_app_pages-h.png[Application Pages Settings,width=600px]
...
image::monitoring-console/mc_settings_app_alerts-h.png[Application Alerts Settings,width=600px]
...
| Popups
| _On_
| When _On_ any alert status change is shown in form of a pop-up dialogue regardless of the currently viewed page.
...
image::monitoring-console/mc_settings_app_appearance-h.png[Application Appearance Settings,width=600px]
...
| Data # (Palette)
| (colours of _Payara Branding_ scheme)
a| The list of colours to use when colouring data series in graphs.
...
| Defaults
| (default of _Payara_ scheme)
a| Sets the default colours for _Waterline_, _Alarming_ and _Critical_ as well as _Red_, _Amber_, _Green_ and _White_ state lines, backgrounds and texts.
...
image::monitoring-console/mc_dialog_switch-theme.png[Switch Theme Dialogue,width=600px]
...
image::monitoring-console/mc_dialog_alerts2.png[Alert Acknowledge Checkbox (left),width=100%]
...
image::monitoring-console/mc_dialog_alerts.png[Confirm Alerts Dialogue,width=100%]
...
image::monitoring-console/mc_footer-alerts.png[Alerts Indicator,width=100%]
...
image::monitoring-console/mc_settings_app_general-h.png[Access Watch Settings,width=600px]
...
image::monitoring-console/mc_dialog_manage-watches.png[Watch Management Dialogue,width=600px]
...
image::monitoring-console/mc_dialog_add-new-watch.png[Add a New Watch Dialogue,width=600px]
...
a| `is` _operator_
| Start
| Select the comparison operator to use when comparing the actual value(s) to the threshold. The comparison is of form _actual operator threshold_.

a| `is` _threshold_
| Start
a| The numeric threshold. The upper/lower limit. With the right unit selected the number can be given with a unit such as `5min` for time units, or `4KB` for bytes.

a| `is` _scope_
| Start
| Select a scope for the actual values that are compared to the threshold. The semantics of different scopes are described in a separate table below.
...
a| `for last`
a| The comparison must be true for each point in the scope for the condition to be met. Similar to an _all_ logic.

a| `for average of last`
a| The comparison must be true for the single average value of all points in the scope for the condition to be met.

The number of points included is specified afterward as number of duration from which to consider points.

a| `in last`
a| The comparison must be true for at least one of the points included in the scope for the condition to be met. Similar to an _any_ logic.

The number of points included is specified afterward as number of duration from which to consider points.

a| `in sample`
a| The comparison must be true for at least one of the points available for the condition to be met.
...
image::monitoring-console/mc_dialog_edit-watch.png[Edit a User Watch Dialogue,width=600px]
...
image::monitoring-console/mc_page_core.png[Core Page,width=100%]
...
image::monitoring-console/mc_page_rag.png[Traffic Light Status Page,width=100%]
...
image::monitoring-console/mc_page_http.png[HTTP Page,width=100%]
...
image::monitoring-console/mc_page_jvm.png[JVM Page,width=100%]
...
image::monitoring-console/mc_page_monitoring.png[Monitoring Page,width=100%]
...
image::monitoring-console/mc_page_health-checks.png[Health Checks Page,width=100%]
...
image::monitoring-console/mc_page_threads.png[Thread Health Page,width=100%]
...
image::monitoring-console/mc_page_request-tracing.png[Request Tracing Page,width=100%]
...
image::monitoring-console/mc_dialog_request-tracing.png[Request Tracing Details Dialogue,width=100%]
...
image::monitoring-console/mc_page_sql.png[Slow SQL Page,width=100%]
...
image::monitoring-console/mc_page_alerts.png[Alerts Page,width=100%]
...
image::monitoring-console/mc_dialog_add-new-widget.png[Select MicroProfile Metrics,width=100%]
...
image::monitoring-console/mc_page_app-metrics.png[Application Metrics Page,width=100%]
