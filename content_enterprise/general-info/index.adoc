...
:page-description: Azul Payara documentation overview — getting started with Payara Server and Payara Micro, release notes, supported platforms, and Jakarta EE certification results.

This page provides some general information about the Azul Payara Edition.

[#release-strategy]
== Release Strategy

The Azul Payara Edition project is released each month. Each release incorporates fixes and enhancements from Platform engineering and Enterprise-only features.

[#versioning-scheme]
== Versioning Scheme

The general format of an Azul Payara version number is:
...
**Major Version** numbers start from **6** (the current major version of Azul Payara) and only increment by 1 when a new major version is released.  Fundamental changes will only be released in major Versions. There is currently no set cadence for this version change.
