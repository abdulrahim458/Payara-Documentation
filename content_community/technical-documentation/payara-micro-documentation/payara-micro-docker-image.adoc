...
:page-description: Azul Payara Micro Community Docker image overview — official Docker images, supported tags, environment variables, and configuration for containerized Payara Micro deployments.
...
[source,text]
----
docker run -p 8080:8080 payara/micro:{release-id}
----
...
[source, shell]
----
docker run -p 8080:8080 --read-only --rm -v /opt/payara -v /tmp -v ~/payara-micro/applications:/opt/payara/deployments payara/micro
----