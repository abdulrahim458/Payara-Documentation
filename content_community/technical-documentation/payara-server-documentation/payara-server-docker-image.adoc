...
:page-description: Azul Payara Server Community Docker image overview — official Docker images, supported tags, environment variables, and volume configuration for containerized Payara Server.
:ordinal: 10
...
[source,text]
----
docker run -p 8080:8080 payara/server-full:{release-id}
----
...
[source, shell, subs=attributes+]
----
docker run -p 8080:8080 -v ~/payara/apps:/opt/payara/deployments payara/server-full:{release-id}
----
...
Scripts added to this folder should be https://www.gnu.org/software/bash/[Bash^] scripts and they should end with the `.sh` suffix.
...
[source, shell]
----
docker run -p 8080:8080 -p 4848:4848 --read-only --rm -v /opt/payara -v /tmp payara/server-full
----
