...
:page-description: Upgrade Azul Payara Community Server — guides for upgrading between community releases using the Upgrade Tool, backup and restore, or directory migration.
...
. Backing up and restoring the existing configuration to a new installation. xref:backup-and-restore-method.adoc[Read Backup and Restore Method]

. Maintaining completely separate domain and node directories and pointing the new version to the existing directories. xref:domain-and-node-directories-method.adoc[Read Domain and Node Directories Method]
