= Cloud Connectors
:page-description: Cloud Connectors for Azul Payara Community — JCA-based connectors for integrating Payara Server and Payara Micro with Amazon SQS, Azure Service Bus, Apache Kafka, and MQTT.
...
* xref:amazon-sqs/index.adoc[Amazon SQS]
* xref:mqtt.adoc[MQTT]
...
image:cloud-connectors/admin-console.png[alt="Admin Console",width=100%]

Then configure the `Connector Classloading Policy` to `Global` in `{config} > Connector Service > Connector Classloading Policy`.

image:cloud-connectors/classloading-configuration.png[alt="Classloading Config",width=600px]
