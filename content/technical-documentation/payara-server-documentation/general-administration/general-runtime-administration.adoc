...
+
The behaviour in versions 6.2022.2 to 6.2025.9 aligned with `false`. The behaviour in versions prior to 6.2022.2 aligned with `true`.
+
Defaults to `true`.
*endif*: []
...
*ifeval*: ["{page-site-edition}" == "enterprise"]
*`fish.payara.ready-after-applications`*:
This option determines when the server marks itself as "ready".
+
When enabled, this will be once all post-boot configuration is applied and previously deployed applications have been loaded.
+
When disabled, this will be once all `InstanceLifecycleListener` and services have processed the `server_startup` event, but before post-boot configuration is applied and previously deployed applications are loaded.
+
The behaviour in prior versions of Enterprise 6 aligned with `false`. The behaviour in versions prior to Enterprise 5.47.0 aligned with `true`.
+
Defaults to `false`.
*endif*: []
