...
* Use the tenant name to read the configuration value from the respective xref:/technical-documentation/microprofile/config/index.adoc[MicroProfile Config] property
...
| `false`
| The post logout redirect URI to which the RP is requesting that the End-User's User Agent be redirected after a logout has been performed.
...
The client secret can be input directly, or for added security it can be aliased using xref:technical-documentation/microprofile/config/index.adoc[Microprofile Config]
...
+
image:security-connector/oidc/azure/app_registrations.png[alt="Select application",width=600px]

. You may also add the custom roles via **Roles and administrators** under the **Manage** section:
+
image:security-connector/oidc/azure/custom_role.png[alt="Add Custom Roles",width=600px]

. Now to map group claims, select **Token configuration** under the **Manage** section:
+
image:security-connector/oidc/azure/token_configuration.png[alt="Token configuration",width=600px]

. Press **Add groups claim** button to select group types and customize the ID and/or Access Token properties:
+
image:security-connector/oidc/azure/add_groups_claim.png[alt="Add Groups Claim",width=600px]
...
+
image:security-connector/oidc/keycloak/add-realm.png[alt="Add Realm",width=600px]

. Copy the OpenId endpoint configuration URL from endpoint section:
+
image:security-connector/oidc/keycloak/realm-endpoint.png[alt="Realm Endpoint",width=600px]
...
. Create the **Groups**:
+
image:security-connector/oidc/keycloak/create-group.png[alt="Create Groups",width=600px]

. Add the **User**:
+
image:security-connector/oidc/keycloak/add-user.png[alt="Add User",width=600px]

. After the user is created, set a new **password** for the user:
+
image:security-connector/oidc/keycloak/set-user-password.png[alt="Set Password",width=600px]

. Now map the user to roles. Click on **Role Mappings** tab and assign the roles to the user from the available roles:
+
image:security-connector/oidc/keycloak/user-role-mapping.png[alt="User Role Mapping",width=600px]

. Assign the user to the groups. Click on **Groups** tab and join the groups from the available groups:
+
image:security-connector/oidc/keycloak/join-group.png[alt="Join Groups",width=600px]

. Create the OpenId Client by clicking the **Client** option from sidebar and press the **create** button:
+
image:security-connector/oidc/keycloak/create-client.png[alt="Create OpenID Client",width=600px]
...
+
image:security-connector/oidc/keycloak/client-access-type-confidentail.png[alt="Access Type",width=600px]

. Next copy the client **secret** from **Credentials** tab.
+
image:security-connector/oidc/keycloak/client-secret.png[alt="Client Secret",width=600px]
...
+
image:security-connector/oidc/keycloak/groups-claim.png[alt="Groups Claim",width=600px]
