...
|`--globalcontextroot <context-path>`
|Sets the global context root of any deployed application. This will prepend the context root. See
xref:/technical-documentation/payara-micro-documentation/payara-micro-configuration-and-management/micro-management/deploying-applications/deploy-applications.adoc#define-global-context-root[Define global context root] for more details.|
|`--group <group-name>`
...
|`--warmup`|Exit the server immediately after configuration is done, applications are deployed and it's ready to serve requests. |
*endif*: []
*ifeval*: ["{page-site-edition}" == "enterprise"]
|`--warmup`|Exit the server immediately after configuration is done, applications are deployed and it's ready to serve requests. If using a CRaC JDK, this option is instead used to activate creating a checkpoint during the bootstrap of Payara Micro. See xref:/technical-documentation/payara-micro-documentation/payara-micro-configuration-and-management/micro-management/crac/crac-support.adoc[Payara Micro CRaC Support] for details. |
*endif*: []
|=======================================================================
