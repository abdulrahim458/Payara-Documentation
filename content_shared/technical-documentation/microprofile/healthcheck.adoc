...
:page-description: MicroProfile Health in Azul Payara — expose liveness and readiness health check endpoints for Payara Server and Payara Micro applications in cloud-native environments.

Azul Payara {release-id} provides MicroProfile HealthCheck {mpHealthVersion}
...
a|`/health/live`
|Liveness
a|On invocation, all registered health checks annotated with `@Liveness` are performed and the results aggregated.

a|`/health/ready`
|Readiness
a|On invocation, all registered health checks annotated with `@Readiness` are performed and the results aggregated.
...
a|On invocation, all registered health checks annotated with `@Liveness`, `@Readiness`, or `@Startup` are performed and the results aggregated.
...
image:microprofile/health-check.png[alt="Set Health Check Configuration",width=100%]
...
* xref:../payara-server-documentation/command-reference/set-microprofile-healthcheck-configuration.adoc#set-microprofile-healthcheck-configuration[`set-microprofile-healthcheck-configuration`]
* xref:../payara-server-documentation/command-reference/get-microprofile-healthcheck-configuration.adoc#get-microprofile-healthcheck-configuration[`get-microprofile-healthcheck-configuration`]
