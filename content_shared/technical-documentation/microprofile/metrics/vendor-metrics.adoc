...
:page-description: Custom vendor metrics in Azul Payara — Payara-specific MicroProfile Metrics exposing JVM, thread pool, request tracing, and data grid statistics.

Azul Payara supports exporting the JMX Beans and HealthCheck stats to the Vendor Metrics.

[#JMX-MBeans]
== JMX Mbeans

JMX MBeans can be exposed as **Custom Vendor Metrics** by the user by supplying a custom https://github.com/payara/Payara/blob/master/appserver/payara-appserver-modules/microprofile/metrics/src/main/resources/metrics.xml[metrics.xml^] file in the `domain-dir/config/` directory with the following format:
...
xref:../../payara-server-documentation/general-administration/administering-the-healthcheck-service.adoc[HealthCheck Services] can be exposed as **Custom Vendor Metrics** by the user by supplying a custom https://github.com/payara/Payara/blob/master/appserver/payara-appserver-modules/microprofile/metrics/src/main/resources/metrics.xml[metrics.xml^] file in the `domain-dir/config/` directory as well.
...
| Connection Pool
a| `healthcheck-cpool`
a| by default `H2Pool`, `__TimerPool`
a| `usedConnection`, `freeConnection`, `totalConnection`
