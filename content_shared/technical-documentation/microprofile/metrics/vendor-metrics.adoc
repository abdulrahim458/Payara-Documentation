...
xref:../../payara-server-documentation/general-administration/administering-the-healthcheck-service.adoc[HealthCheck Services] can be exposed as **Custom Vendor Metrics** by the user by supplying a custom link:https://github.com/payara/Payara/blob/master/appserver/payara-appserver-modules/microprofile/metrics/src/main/resources/metrics.xml[metrics.xml^] file in the `domain-dir/config/` directory as well.
