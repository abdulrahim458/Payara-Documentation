...
You can define the alias globally as a xref:technical-documentation/microprofile/config/index.adoc[MicroProfile Config] property, either in an application-specific config source only for a single application, or with a global config source for all applications. In this case you don't need to add it during every single Rest Client call.
