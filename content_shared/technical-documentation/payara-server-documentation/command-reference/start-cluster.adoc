...
NOTE: If all instances reside on the same host as the DAS, SSH is not required. You might require to start a cluster in which instances reside
on hosts where SSH is not configured that are remote from the DAS. In this situation, run the xref:start-local-instance.adoc#start-local-instance[`start-local-instance`] subcommand for each instance from the host where the instance resides.
