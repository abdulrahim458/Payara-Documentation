...
NOTE: This subcommand is equivalent to the xref:technical-documentation/payara-server-documentation/command-reference/install-node.adoc#install-node[`install-node`] subcommand.
...
*`--installdir`*:
  ...
  for example, `/export/payara7/`. If the directory does not exist, the subcommand creates the directory. +
