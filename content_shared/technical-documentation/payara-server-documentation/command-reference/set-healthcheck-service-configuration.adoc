...
Defaults to `false`. When enabled, the checker results are displayed on MicroProfile Health xref:../../microprofile/healthcheck.adoc#rest-endpoints[REST endpoints] as a readiness check.
...
* xref:set-healthcheck-configuration.adoc[`set-healthcheck-configuration`],
* xref:get-healthcheck-configuration.adoc[`get-healthcheck-configuration`]
