...
*`--name`*:
  ...
  (`.`) characters. For more information about module and application versions, see xref:technical-documentation/payara-server-documentation/application-deployment/index.adoc#module-and-application-versions[Module and Application Versions] in
...
*`--createtables`*:
  ...
  file determines whether or not tables are created. No unique constraints are created for the tables. This parameter is deprecated and will be removed in Payara 7.
*`--dropandcreatetables`*:
  ...
  `glassfish-ejb-jar.xml` file is set to true. This parameter is deprecated and will be removed in Payara 7.
*`--uniquetablenames`*:
  ...
  with unmapped CMP beans. This parameter is deprecated and will be removed in Payara 7.
*`--deploymentplan`*:
