...
+
See xref:../high-availability/administering-payara-server-instances.adoc#the-extra-terse-option[`extraterse`] for more information.
+
...
+
Specifically for the `create-instance` and `create-local-instance` commands, the `autoname` option can also be used to automatically resolve any name conflicts. See xref:auto-naming.adoc#auto-naming[`auto-naming`] for more information.
+
Default to `false`.
*`--help`*:
...
a|`AS_ADMIN_EXTRATERSE`
a|`--extraterse`

a|`AS_ADMIN_ECHO`
a|`--echo`

a|`AS_ADMIN_INTERACTIVE`
a|`--interactive`
