...
[source,shell]
----
asadmin [asadmin-options] create-node-docker [--help]
[--nodehost node-host]
...
The setup of a Docker node is more involved that other types of nodes, so it is recommended reading about the requirements and conditions for this command to work properly in xref:../high-availability/administering-payara-server-nodes.adoc#creating-a-docker-node[corresponding section] in Payara Server's High Availability section.
...
+
Defaults to `payara/server-node:{release-id}`.
*`--dockerPort`*:
...
* xref:delete-node-docker.adoc#delete-node-docker[`delete-node-docker`],
* xref:list-nodes.adoc#list-nodes[`list-nodes`],
* xref:update-node-docker.adoc#update-node-docker[`update-node-docker`]
