...
*`--name`*:
  ...
  The name can include an optional version identifier, which follows the name and is separated from the name by a colon (`:`). The version identifier must begin with a letter or number. It can contain alphanumeric characters plus underscore (`_`), dash (`-`), and period (`.`) characters. For more information about module and application versions, see "xref:technical-documentation/payara-server-documentation/application-deployment/index.adoc#module-and-application-versions[Module and Application Versions]" in Payara Server Application Deployment section.
*`--upload`*:
...
//TODO - Consider removing JavaDB/Derby support
  Specifies the name of the database vendor for which tables are created. Supported values include `db2`, `mssql`, `h2`, `mysql`, `oracle`, `derby`, `javadb`, `postgresql`, and `sybase`. These values are case-insensitive. If not specified, the value of the `database-vendor-name` attribute in `glassfish-ejb-jar.xml` is used. If the connection cannot be established, or if the value is not valid, SQL-92 compliance is presumed.
*`--deploymentplan`*:
...
*`--properties` or `--property`*:
  ...
  `keepSessions={false|true}`;;
    ...
    If the `--force` option is set to `true`, this property specifies whether active sessions of the application being redeployed are preserved and restored after redeployment. Applies to HTTP sessions in a web container. Default is `false`. +
    *`false`*:
...
*ifeval*: ["{page-site-edition}" == "community"]
  `warlibs={false|true}`;;
      ...
      Libraries stored in this directory allow developers to create and deploy skinny WARs, as they are subject to additional annotation and extension scanning as well as processing for CDI, EJB, Servlet, and more.
...
*Example 6 Deploying an Application Using an Alternate Jakarta EE 8 Deployment Descriptor File*
...
*Example 7 Deploying an Application Using an Alternate Payara Server Deployment Descriptor File*
