...
This subcommand requires the secure shell (SSH) to be configured on the host where the domain administration server (DAS) is running and on all hosts where instances in the cluster reside.

NOTE: If all instances reside on the same host as the DAS, SSH is not required. You might require to restart a cluster in which instances reside on hosts where SSH is not configured that are remote from the DAS. +
In this situation, run the xref:restart-local-instance.adoc#restart-local-instance[`restart-local-instance`] subcommand for each instance from the host where the instance resides.
