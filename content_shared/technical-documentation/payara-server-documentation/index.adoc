...
:page-description: Azul Payara Server technical documentation — administration, deployment, security, high availability, MicroProfile, and application development guides for Payara Server.
...
a|`ls [-l]`
a|The `-l` option is not required.

a|`{ \| }`
|Contains a set of choices for a required command option.
a|`-d {y\|n}`
a|The `-d` option requires that you use either the `y` argument or the `n` argument.

a|`${ }`
|Indicates a variable reference or a placeholder reference for a default path.
a|`${com.sun.javaRoot}`
a|References the value of the `com.sun.javaRoot` variable.

|-
|Joins simultaneous multiple keystrokes.
a|`Control-A`
a|Press the `Control key while you press the A key.
...
`+{USER_HOME}+/payara7/glassfish`

Installations on a Windows operating system:

`+{SystemDrive}+:\payara7\glassfish`
...
a|[.breakable]`domain-root-dir/<domain-name>`

a|`node-dir`
|Represents the directory for a configured node.
a|[.breakable]`as-install/glassfish/nodes/<node-name>`

a|`instance-dir`
|Represents the directory for a server instance.
a|`node-dir/<instance-name>`
