= Product Concepts
:page-description: Azul Payara Server product concepts — domains, clusters, instances, deployment groups, nodes, and the administration model for Payara Server environments.
:ordinal: 1
...
A server instance is a Payara Server running in a single Java Virtual Machine (JVM) process. The supported platforms for Payara Server are described in this following article: xref:../../../general-info/supported-platforms.adoc[Supported Platforms].
...
Administrative commands are provided in order to back up and restore the domain configuration and applications. With the standard backup and restore procedures, you can quickly restore working configurations. If the DAS host fails, you must create a new DAS installation to restore the previous domain configuration. For instructions, see "xref:../general-administration/administering-domains.adoc#administering-domains[Administering Domains]" in the Payara Server General Administration section.
...
When enabled, checkpointing generally occurs after any transaction involving the SFSB is completed, even if the transaction rolls back. For more information on developing stateful session beans,see "xref:../../application-development/using-jakarta-enterprise-beans-technology.adoc#using-session-beans[Using Session Beans]" in the Payara Server Application Development section. For more information on enabling SFSB failover, see "xref:../../application-development/using-jakarta-enterprise-beans-technology.adoc#stateful-session-bean-failover[Stateful Session Bean Failover]" in the Payara Server High Availability section.
...
For more information on configuring session persistence, see "xref:../high-availability/configuring-ha-session-persistence-and-failover.adoc#configuring-high-availability-session-persistence-and-failover[Configuring High Availability Session Persistence and Failover]" in the Payara Server High Availability section.
...
For information on deploying JMS applications and Message Queue clustering for message failover, see xref:planning-your-deployment.adoc#planning-your-deployment[Planning Message Queue Broker Deployment].
...
* xref:planning-your-deployment.adoc#establishing-performance-goals[Establishing Performance Goals]
* xref:planning-your-deployment.adoc#planning-the-network-configuration[Planning the Network Configuration]
* xref:planning-your-deployment.adoc#planning-for-availability[Planning for Availability]

. Size your system. For more information, see xref:planning-your-deployment.adoc#design-decisions[Design Decisions].
...
+
* "xref:../general-administration/administering-domains.adoc#administering-domains[Administering Domains]" in the Payara Server General Administration section
* "xref:../high-availability/administering-payara-server-nodes.adoc#administering-payara-server-nodes[Administering Payara Server Nodes]" in the Payara Server High Availability section
* "xref:../high-availability/administering-payara-server-clusters.adoc#administering-payara-server-clusters[Administering Payara Server Clusters]" in the Payara Server High Availability section
* "xref:../high-availability/administering-payara-server-instances.adoc#administering-payara-server-instances[Administering Payara Server Instances]" in the Payara Server High Availability section
* "xref:../general-administration/administering-http-connectivity.adoc#administering-virtual-servers[Administering Virtual Servers]" in the Payara Server General Administration section

. Configure your load balancer, tentatively, using Apache Mod JK. For more information, see "xref:../general-administration/administering-web-applications.adoc#administering-mod_jk[Administering mod_jk]" in the Payara Server General Administration section.

. Configure the web container and EJB container for replicated session persistence. +
For more information, see "xref:../high-availability/configuring-ha-session-persistence-and-failover.adoc#configuring-high-availability-session-persistence-and-failover[Configuring High Availability Session Persistence and Failover]" in the Payara Server High Availability section.

. If you are using messaging extensively, configure Java Message Service (JMS) clusters for failover.
+
For more information, see the following documentation:
+
* xref:planning-your-deployment.adoc#planning-message-queue-broker-deployment[Planning Message Queue Broker Deployment]
* "xref:../high-availability/configuring-jms-high-availability.adoc#configuring-java-message-service-high-availability[Configuring Java Message Service High Availability]" in the Payara Server High Availability section
