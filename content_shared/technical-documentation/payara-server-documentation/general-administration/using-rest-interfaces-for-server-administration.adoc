...
:page-description: REST management interface for Azul Payara Server — administer server configuration, deployments, and resources via the Payara REST management API.
:ordinal: 1
...
image:administration/rest-management.png[alt=""REST resource for managing a domain."",width=600px]
...
The `GET` method determines the methods and method parameters that an object in the tree supports and provides additional information about the object. For details, see xref:general-runtime-administration.adoc#to-retrieve-data-for-an-object-in-the-tree[To Retrieve Data for an Object in the Tree].
...
You can specify the format in which this information is presented. For more information, see xref:general-runtime-administration.adoc#formats-for-resource-representation-of-configuration-objects[Formats for Resource Representation of Configuration Objects].
...
. Ensure that the server is running. Operations on REST resources for Payara Server data require a running server.
. Determine the acceptable message parameters for the `POST` method of the resource that represents the parent of the object. For information about how to perform this step, see xref:general-runtime-administration.adoc#to-determine-the-methods-and-method-parameters-that-an-object-in-the-tree-supports[To Determine the Methods and Method Parameters That an Object in the Tree Supports].
...
+
For information about how to perform this step, see xref:general-runtime-administration.adoc#to-retrieve-data-for-an-object-in-the-tree[To Retrieve Data for an Object in the Tree].
...
. Confirm that the object has been updated. For information about how to perform this step, see xref:general-runtime-administration.adoc#to-retrieve-data-for-an-object-in-the-tree[To Retrieve Data for an Object in the Tree].
...
. Ensure that the server is running. Operations on REST resources for Payara Server data require a running server.
. Confirm that the object can be deleted. For information about how to perform this step, see xref:general-runtime-administration.adoc#to-determine-the-methods-and-method-parameters-that-an-object-in-the-tree-supports[To Determine the Methods and Method Parameters That an Object in the Tree Supports].
...
* xref:../security-guide/administering-user-security.adoc#to-create-an-authentication-realm[To Create an Authentication Realm] in the Payara Server Security Guide
* xref:../security-guide/administering-user-security.adoc#to-create-a-file-user[To Create a File User] in the Payara Server Security Guide
* xref:administering-http-connectivity.adoc#configure-an-http-listener-for-ssl[To Configure an HTTP Listener for SSL]
...
*command-list*:
One or more metadata sets separated by a comma (`,`) that represent the `asadmin` subcommands for performing non—CRUD operations on the resource. For the format of each metadata set, see xref:using-rest-interfaces-for-server-administration.adoc#json-representation-of-a-command-in-a-command-list[JSON Representation of a Command in a Command List].
*method-list*:
One or more metadata sets separated by a comma (`,`) that represent the methods that the resource supports. For the format of each metadata set, see xref:using-rest-interfaces-for-server-administration.adoc#json-representation-of-a-method-in-a-method-list[JSON Representation of a Method in a Method List].
*attributes*:
...
*query-parameter-list*:
Zero or more metadata sets separated by a comma (`,`) that represent the query parameters that are allowed for the method. For the format of each metadata set, see xref:general-runtime-administration.adoc#json-representation-of-a-message-parameter-or-a-query-parameter[JSON Representation of a Message Parameter or a Query Parameter].
...
*methods*:
One or more XML elements that represent the methods that the resource supports. For the format of each element, see xref:using-rest-interfaces-for-server-administration.adoc#xml-representation-of-a-resource-method[XML Representation of a Resource Method].
*attributes*:
...
*commands*:
One or more XML elements that represent the `asadmin` subcommands for performing non—CRUD operations on the resource. For the format of each element, see xref:using-rest-interfaces-for-server-administration.adoc#xml-representation-of-a-command[XML Representation of a Command].
*children*:
...
<map>
 <entry key="extraProperties">
  <map>
   ...
   <entry key="entity">
    <map>
     <entry key="installDir" value="/export/payara7"/>
...
The result of the attempt to get the resource.
*entity*:
Contains the statistics that the monitoring object provides. For the format of each metadata set, see xref:using-rest-interfaces-for-server-administration.adoc#json-representation-of-a-statistic-in-a-statistics-list[JSON Representation of a Statistic in a Statistics List].
*childResources*:
Zero or more child resources separated by a comma (`,`). Each child resource is specified as "resource-name":"url". +
resource-name;;
The name of the resource as displayed in client applications that access the parent of the resource.
...
*statistics*:
Zero or more XML elements that represent the statistics that the monitoring object provides. For the format of each element, see xref:general-runtime-administration.adoc#xml-representation-of-a-statistic[XML Representation of a Statistic].
*children*:
Zero or more XML elements that represent the children of the resource. Each element is specified as follows:
+
[source,xml]
...
image:administration/rest-monitoring.png[alt=""REST resource that provides class loader statistics."",width=600px]
...
*log-level*:
The severity level of the message in the log record. For more information, see xref:administering-the-logging-service.adoc#setting-log-levels[Setting Log Levels].
*product-name*:
...
+
For detailed information about the names of logger classes in the Payara Server, see xref:administering-the-logging-service.adoc#logger-namespaces[Logger Namespaces].
*thread-id*:
  ...
  "records": [
    {
      ...
      "loggedLevel":"INFO",
      "productName":"Payara Server {release-id}",
...
The replaceable item in this format is the records, which is one or more XML elements that represent the log records in the log file. For the format of each element, see xref:general-runtime-administration.adoc#xml-representation-of-a-log-record[XML Representation of a Log Record].
...
The fully qualified name of the Java class of the logger class that created the log message. Each component of Payara Server provides its own logger class.
+
For detailed information about the names of logger classes in Payara Server, see xref:administering-the-logging-service.adoc#logger-namespaces[Logger Namespaces].
*message-id*:
A unique identifier for the message. For messages from Payara Server, this identifier consists of a module code and a numerical value, for example, `CORE5004`. All `SEVERE` and `WARNING` messages and some `INFO` messages from Payara Server contain a message identifier.
*thread-id*:
The numerical identifier of the thread that created the message.
...
----
<records>
 <record loggedDateTimeInMS="1300743782815" loggedLevel="INFO" loggerName="org.glassfish.admingui" messageID="" nameValuePairs="_ThreadID=25;_ThreadName=Thread-1;" productName="Payara {release-id}" recordNumber="475"/>
 <record loggedDateTimeInMS="1300728893368" loggedLevel="INFO" loggerName="jakarta.enterprise.system.core.com.sun.enterprise.v3.admin.adapter" messageID="" nameValuePairs="_ThreadID=238;_ThreadName=Thread-1;" productName="Payara {release-id}" recordNumber="474"/>
 <record loggedDateTimeInMS="1300728893367" loggedLevel="INFO" loggerName="jakarta.enterprise.system.core.com.sun.enterprise.v3.server" messageid="core10010" nameValuePairs="_ThreadID=238;_ThreadName=Thread-1;" productName="Payara {release-id}" recordNumber="473"/>
</records>
----
