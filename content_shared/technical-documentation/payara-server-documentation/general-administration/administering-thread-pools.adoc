...
xref:technical-documentation/payara-server-documentation/command-reference/create-threadpool.adoc[`create-threadpool`] subcommand. +
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the existing thread pools by using the xref:technical-documentation/payara-server-documentation/command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
...
. List the existing thread pools by using the xref:technical-documentation/payara-server-documentation/command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
. Modify the values for a thread pool by using the xref:technical-documentation/payara-server-documentation/command-reference/set.adoc[`set`] subcommand. +
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the existing thread pools by using the xref:technical-documentation/payara-server-documentation/command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
. Delete the specified thread pool by using the xref:technical-documentation/payara-server-documentation/command-reference/delete-threadpool.adoc[`delete-threadpool`] subcommand.
. To apply your changes, restart Payara Server. See xref:administering-domains.adoc#to-restart-a-domain[To Restart a Domain]. +
...
a|Cannot be less than 0, larger than max int, and must be larger than `thread-max-pool-size`
...
image:ejb/ejb-thread-pool-admin-console-configuration.png[alt="Admin Console Configuration",width=100%]
