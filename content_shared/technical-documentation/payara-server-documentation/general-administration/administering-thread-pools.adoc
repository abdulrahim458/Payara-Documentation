...
:page-description: Administer thread pools in Azul Payara Server — create, configure, and tune request processing thread pools for HTTP and IIOP traffic handling.
:ordinal: 5
...
xref:../command-reference/create-threadpool.adoc[`create-threadpool`] subcommand. +
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the existing thread pools by using the xref:../command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
...
. List the existing thread pools by using the xref:../command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
. Modify the values for a thread pool by using the xref:../command-reference/set.adoc[`set`] subcommand. +
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the existing thread pools by using the xref:../command-reference/list-threadpools.adoc[`list-threadpools`] subcommand.
. Delete the specified thread pool by using the xref:../command-reference/delete-threadpool.adoc[`delete-threadpool`] subcommand.
. To apply your changes, restart Payara Server. See xref:administering-domains.adoc#to-restart-a-domain[To Restart a Domain]. +
