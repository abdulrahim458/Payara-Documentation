= Mbeans Inventory
:page-description: MBeans inventory for Azul Payara Server — reference listing of all JMX MBeans exposed by Payara Server for monitoring via JConsole or management tools.
:ordinal: 25
...
a| `totalcompilationtime-current`
...
See xref:administering-the-monitoring-service.adoc#enable-jmx[enable JMX Monitoring] for how to view all available MBeans.

IMPORTANT: Azul Payara MBeans are only available when AMX is booted. This is done automatically once a JMX client connects. For all other cases, AMX must be either configured to boot at server startup (through domain config) or booted via a JMX operation `bootAMX` on the `amx-support:type=boot-amx` MBean using a non-JMX client (e.g. over a local socket). +
...
a| `activeapplicationsdeployedcount`
| Number of applications deployed
|

a| `totalapplicationsdeployedcount`
...
a| `numconnsuccessfullymatched`
| Number of connections successfully matched
|

a| `numconnnotsuccessfullymatched`
...
a| This will be one of `GET`, `HEAD`, `POST`, `PUT`, `DELETE`, `TRACE`, `OPTIONS`, `CONNECT` or `PATCH`
...
a| `countbytestransmitted`
| The number of bytes transmitted
|

a| `countbytesreceived`
...
a| `methodreadycount`
a| Number of stateful session beans in `MethodReady` state
|
...
a| Returns `false` if service is working fine
