...
| `osgi`
| Delegates the command line to the Apache Felix Gogo remote shell for the execution of OSGi shell commands. For procedural information in this guide, see xref:technical-documentation/payara-server-documentation/general-administration/index.adoc#to-run-apache-felix-gogo-remote-shell-commands[To Run Apache Felix Gogo Remote Shell Commands].
