...
:page-description: Configuration variables reference for Azul Payara Server — system properties, environment variables, and micro-profile config sources available for runtime configuration.
:ordinal: 23
...
MicroProfile Configuration variables must be declared in one of the Azul Payara xref:../../microprofile/config/index.adoc#config-sources[supported configuration sources].
...
**   ejb-jar.xml / payara-ejb-jar.xml / glassfish-ejb-jar.xml (deprecated)
**   application.xml / payara-application.xml / glassfish-application.xml (deprecated)
