...
:page-description: Administer the Object Request Broker (ORB) in Azul Payara Server — configure IIOP listeners, thread pools, and RMI-IIOP settings for remote EJB communication.
:ordinal: 18
...
. Create an IIOP listener by using the xref:../command-reference/create-iiop-listener.adoc[`create-iiop-listener`] subcommand. Information about the properties for the subcommand is included in this help page.
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the IIOP listeners by using the xref:../command-reference/list-iiop-listeners.adoc[`list-iiop-listeners`] subcommand.
...
. List the IIOP listeners by using the xref:../command-reference/list-iiop-listeners.adoc[`list-iiop-listeners`] subcommand.
. Modify the values for the specified IIOP listener by using the  xref:../command-reference/set.adoc[`set`] subcommand. The listener is identified by its dotted name.
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the IIOP listeners by using the xref:../command-reference/list-iiop-listeners.adoc[`list-iiop-listeners`] subcommand.
. Delete an IIOP listener by using the xref:../command-reference/delete-iiop-listener.adoc[`delete-iiop-listener`] subcommand.
. To apply your changes, restart Payara Server. See xref:administering-domains.adoc#to-restart-a-domain[To Restart a Domain].
