...
:page-description: Administer the JVM in Azul Payara Server — configure JVM options, heap size, GC settings, JVM flags, and profiler settings for Payara Server processes.
:ordinal: 3
...
Use the `create-jvm-options` subcommand in remote mode to create JVM options in the Java configuration element of the `domain.xml` file.
...
Use the `delete-jvm-options` subcommand in remote mode to delete JVM options from the Java configuration element of the `domain.xml` file.
...
=== Azul Payara Proprietary JVM Options
...
image::jvm-options/configure-jdk-versions-admin-console.png[Configure JDK versions for JVM options in Admin console,width=100%]
