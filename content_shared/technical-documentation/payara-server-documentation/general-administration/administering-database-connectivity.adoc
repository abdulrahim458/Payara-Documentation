...
* Database. The repository where data is stored for an enterprise. Jakarta EE applications access relational databases through the JDBC API. For administration procedures, see xref:administering-database-connectivity.adoc#setting-up-the-database[Setting Up the Database].
* JDBC Connection Pool. A JDBC connection pool is a group of reusable connections for a particular database. For administration procedures, see xref:administering-database-connectivity.adoc#administering-jdbc-connection-pools[Administering JDBC Connection Pools].
...
+
For a list of drivers supported by Payara Server, see xref:administering-database-connectivity.adoc#configuration-specifics-for-jdbc-drivers[Configuration Specifics for JDBC Drivers].
. Make the JDBC driver JAR file accessible to the domain administration server (DAS).
+
See xref:administering-database-connectivity.adoc#integrating-the-jdbc-driver[Integrating the JDBC Driver].
...
See xref:administering-database-connectivity.adoc#to-create-a-jdbc-connection-pool[To Create a JDBC Connection Pool] and
xref:administering-database-connectivity.adoc#to-create-a-jdbc-resource[To Create a JDBC Resource].
The final step is to integrate the JDBC driver into an administrative domain as described in
xref:administering-database-connectivity.adoc#integrating-the-jdbc-driver[Integrating the JDBC Driver].
...
Start the database by using the xref:../command-reference/start-database.adoc[`start-database`] subcommand.
...
Log redirected to C:\payara-server\_WORKDIR\7.2025.1\payara7\glassfish\databases\h2.log.
...
. Stop the database by using the xref:../command-reference/stop-database.adoc[`stop-database`] subcommand.
...
Creating a JDBC connection pool is a dynamic event and does not require server restart. However, there are some parameters that do require server restart. See xref:../../../index.adoc#configuration-changes-that-require-restart[Configuration Changes That Require Restart].

*Before You Begin*

Before creating the connection pool, you must first install and integrate the database and its associated JDBC driver. For instructions, see xref:administering-database-connectivity.adoc#setting-up-the-database[Setting Up the Database].

. Ensure that the server is running. +
Remote subcommands require a running server.
. Create the JDBC connection pool by using the
xref:technical-documentation/payara-server-documentation/command-reference/create-jdbc-connection-pool.adoc#create-jdbc-connection-pool[`create-jdbc-connection-pool`] subcommand.
. If needed, restart the server. Some parameters require server restart. See
xref:../../../index.adoc#configuration-changes-that-require-restart[Configuration Changes That Require Restart].
...
image:connection-pools/connection_pools_1.png[alt="New JDBC Connection Pool",width=100%]
...
image:connection-pools/connection_pools_2.png[alt="Connection pool properties",width=100%]
...
. Ensure that the server is running. Remote subcommands require a running server.
. Ping a connection pool by using the xref:technical-documentation/payara-server-documentation/command-reference/ping-connection-pool.adoc[`ping-connection-pool`] subcommand.
...
You can also specify that a JDBC connection pool is automatically tested when created or reconfigured by setting its `--ping` option to `true`(the default is `false`). See xref:administering-database-connectivity.adoc#to-create-a-jdbc-connection-pool[To Create a JDBC ConnectionPool] or xref:administering-database-connectivity.adoc#to-update-a-jdbc-connection-pool[To Update a JDBC Connection Pool].
...
xref:technical-documentation/payara-server-documentation/command-reference/list-jdbc-connection-pools.adoc[`list-jdbc-connection-pools`] subcommand.
...
image:connection-pools/connection_pools_3.png[alt="Connection Pool sizes in the admin console",width=100%]
...
image:connection-pools/connection_pools_4.png[alt="Connection validation configuration in Admin console",width=100%]

Additional information regarding enabling connection validation through 'asadmin' commands, along with various options, is elaborated in the the xref:../../application-development/using-the-jdbc-api-for-database-access.adoc[`Handling Invalid Connections`] section of our Payara Server Application Development JDBC documentation.
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the JDBC connection pools by using the xref:../command-reference/list-jdbc-connection-pools.adoc[`list-jdbc-connection-pools`] subcommand.
. If necessary, notify users that the JDBC connection pool is being deleted.
. Delete the connection pool by using the xref:../command-reference/delete-jdbc-connection-pool.adoc[`delete-jdbc-connection-pool`] subcommand.
...
* Specify it using the `--property` option in the `create-jdbc-connection-pool` subcommand. For more information, see xref:../command-reference/create-jdbc-connection-pool.adoc[`create-jdbc-connection-pool`].
...
* Specify the `--initsql` option in the `asadmin create-jdbc-connection-pool` command. For more information, see xref:../command-reference/create-jdbc-connection-pool.adoc[`create-jdbc-connection-pool`].
...
xref:../command-reference/create-jdbc-connection-pool.adoc[`create-jdbc-connection-pool`].
...
image:connection-pools/connection_pools_5.png[alt="Leak Detection setting in Admin console",width=100%]
...
[source, log]
----
[#|2025-11-12T18:51:01.467+0000|FINE|Payara 7.2025.1|javax.enterprise.resource.sqltrace.com.sun.gjc.util|_ThreadID=35;_ThreadName=http-listener-1(5);_TimeMillis=1454611861467;_LevelValue=500;ClassName=com.sun.gjc.util.SQLTraceLogger;MethodName=sqlTrace;|
...
image:advanced-jdbc/logjdbccall.png[alt="Enabling SQL Tracing in the administration console",width=100%]
...
SQL
|`fish.payara.skip-client-info-validation` |`Boolean` |`false` |When set to `true`,
client info will not be validated.
...
Before creating a JDBC resource, you must first create a JDBC connection pool. For instructions, see xref:administering-database-connectivity.adoc#to-create-a-jdbc-connection-pool[To Create a JDBC Connection Pool].

. Ensure that the server is running. Remote subcommands require a running server.
. Create a JDBC resource by using the
xref:technical-documentation/payara-server-documentation/command-reference/create-jdbc-resource.adoc[`create-jdbc-resource`] subcommand.
+
...
. Ensure that the server is running. Remote subcommands require a running server.
. List JDBC resources by using the xref:technical-documentation/payara-server-documentation/command-reference/list-jdbc-resources.adoc[`list-jdbc-resources`] subcommand.
...
. List JDBC resources by using the xref:technical-documentation/payara-server-documentation/command-reference/list-jdbc-resources.adoc[`list-jdbc-resources`] subcommand.
. Modify the values for the specified JDBC resource by using the xref:technical-documentation/payara-server-documentation/command-reference/set.adoc[`set`] subcommand.
...
. Delete a JDBC resource by using the xref:technical-documentation/payara-server-documentation/command-reference/delete-jdbc-resource.adoc[`delete-jdbc-resource`] subcommand.
...
Instructions for creating JDBC resources in general are provided in xref:administering-database-connectivity.adoc#to-create-a-jdbc-resource[To Create a JDBC Resource]. Use the following procedure to enable the preconfigured `jdbc/__default` resource for a clustered Payara Server environment.
...
For a list of the JDBC drivers currently supported by the Payara Server, see the Payara Release Notes. For configurations of supported and other drivers, see xref:administering-database-connectivity.adoc#configuration-specifics-for-jdbc-drivers[Configuration Specifics for JDBC Drivers].
...
To integrate the JDBC driver into a Payara Server domain, copy the JAR files into the `domain-dir/lib` directory, then restart the server. This makes classes accessible to all applications or modules deployed on servers that share the same configuration. For more information about Payara Server class loaders, see "xref:../../application-development/class-loaders.adoc[Class Loaders]" in the Payara Server Application Development section.
...
image:advanced-jdbc/activate-sql-statements.png[alt="Activate JDBC Connection pool monitoring with administration console",width=100%]
...
see xref:administering-the-monitoring-service.adoc#jmx-monitoring-service[jmx monitoring configuration] and xref:administering-the-monitoring-service.adoc#rest-monitoring[rest monitoring configuration] on how to do this.
...
image:advanced-jdbc/slowsqllogging.png[alt="Slow SQL Logging settings in the administration console",width=100%]
...
More information on how to implement a SQL Trace listener can be found in the xref:../../application-development/using-the-jdbc-api-for-database-access.adoc#sql-trace-listeners[Application Development section].
...
image:advanced-jdbc/sqltracelistener.png[alt="Enabling SQL Tracing in the administration console",width=100%]
