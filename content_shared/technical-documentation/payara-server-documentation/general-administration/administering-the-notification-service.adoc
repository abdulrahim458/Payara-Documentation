...
:page-description: Administer the Notification Service in Azul Payara Server — configure notifiers (Slack, email, SNMP, JMX) to receive alerts for JVM, GC, and application health events.
:ordinal: 11

Payara Server comes with a general Notification service that can be used to log events which come from other services, such as the xref:administering-the-monitoring-service.adoc[Monitoring Service], the xref:administering-the-healthcheck-service.adoc[HealthCheck service] or the xref:administering-the-request-tracing-service.adoc[Request Tracing Service].
...
* xref:../command-reference/notification-configure.adoc[`notification-configure`]
* xref:../command-reference/list-notifiers.adoc[`list-notifiers`]
* xref:../command-reference/get-notification-configuration.adoc[`get-notification-configuration`]
* xref:../command-reference/set-notification-configuration.adoc[`set-notification-configuration`]
...
image:notification-service/log/log-admin-console-configuration.png[alt="Log Notifier Configuration",width=100%]
...
See xref:administering-the-logging-service.adoc#notification-logging[Notification Logging] for more information on how to configure specific details of the server logging settings.
...
+
image:notification-service/jms/prepare-jms-destination.png[alt="Create JMS Queue",width=100%]
+
...
+
image:notification-service/jms/jms-admin-console-configuration.png[alt="Configure JMS Notifier in Admin Console",width=100%]
+
...
image:notification-service/event-bus/event-bus-admin-console-configuration.png[alt="Event Bus Configuration",width=100%]
...
image:notification-service/cdi-event-bus/cdi-event-bus-notif-config.png[alt="CDI Event Bus Configuration",width=100%]
...
IMPORTANT: For an application to use the `Inbound` annotation and `EventbusMessage` class, it will have to define the xref:../../public-api/index.adoc[Payara Public API] as a project dependency.
...
To configure notification settings for JMX Monitoring, you will need to enable it. Have a look at xref:administering-the-monitoring-service.adoc#to-enable-monitoring[How To Enable JMX Monitoring]
...
image:jmx-monitoring-service/jmx-log-frequency.png[alt="JMX Log Frequency",width=100%]
...
image:jmx-monitoring-service/jmx-add-notifiers.png[alt="Add JMX Notifiers",width=100%]
...
* xref:administering-the-logging-service.adoc[Logging Configuration]
