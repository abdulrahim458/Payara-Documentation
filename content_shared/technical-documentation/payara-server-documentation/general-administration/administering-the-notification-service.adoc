...
Payara Server comes with a general Notification service that can be used to log events which come from other services, such as the xref:technical-documentation/payara-server-documentation/general-administration/administering-the-monitoring-service.adoc[Monitoring Service], the xref:administering-the-healthcheck-service.adoc[HealthCheck service] or the xref:administering-the-request-tracing-service.adoc[Request Tracing Service].
