...
:page-description: Administer web applications in Azul Payara Server — configure session management, cookie settings, virtual servers, and default web module behavior.
:ordinal: 22
...
balancing with Payara Server refer to xref:../high-availability/configuring-http-load-balancing.adoc#configuring-http-load-balancing[Configuring HTTP Load Balancing] in the Payara Server High Availability section.
...
* `apache2/conf/workers.properties` xref:administering-web-applications.adoc#example-6-4[Example 6-4] and
xref:administering-web-applications.adoc#example-6-5[Example 6-5] provide examples of configuring these two files.
...
. Create a jk-enabled network listener by using the xref:../command-reference/create-network-listener.adoc[`create-network-listener`] subcommand.
+
[source,shell]
----
...
This example shows a `workers.properties` that is set for `mod_jk`. This `workers.properties` file is referenced in the second line of xref:administering-web-applications.adoc#example-6-4[Example 6-4].
...
.  On each of the instances, perform the steps in  xref:administering-web-applications.adoc#to-enable-mod_jk[To Enable `mod_jk`]. +
If your instances run on the same machine, you must choose different JK ports. The ports must match `worker.worker*.port` in your `workers.properties` file. See the properties file in xref:administering-web-applications.adoc#example-6-5[Example 6-5].

. On each of the instances, create the `jvmRoute` system property of Payara Server by using the xref:../command-reference/create-jvm-options.adoc[`create-jvm-options`] subcommand. +
...
. Perform the steps in xref:administering-web-applications.adoc#to-enable-mod_jk[To Enable `mod_jk`].
...
. Follow instructions from xref:administering-http-connectivity.adoc#to-configure-an-http-listener-for-ssl[To Configure and HTTP Listener for SSL] on the `mod_jk` connector. +
Use the following format:
+
[source,shell]
----
