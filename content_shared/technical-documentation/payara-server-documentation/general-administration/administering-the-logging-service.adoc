...
:page-description: Administer the logging service in Azul Payara Server — configure log levels, handlers, log format, log rotation, and per-module logging for server and applications.
:ordinal: 7
...
|DAS
a|[.breakable]`domain-dir/logs/server.log`

|Server instance
a|[.breakable]`instance-dir/logs/server.log`
...
You can change the default name or location of a log file by modifying the logging properties file for the corresponding instance, described in xref:administering-the-logging-service.adoc#to-change-the-name-and-location-of-the-log-file[To Change the Name and Location of the Log File].

[#log-records]
==== Log Records

Payara Server writes log records in either the Oracle Diagnostics Logging (ODL) format or the Uniform Log Formatter (ULF) format. The default format is ODL. For information about changing the format, see xref:administering-the-logging-service.adoc#setting-the-log-file-format[Setting the Log File Format].
...
----
[2013-04-12T08:08:30.154-0700] [Payara Server {release-id}] [INFO] [AS-WEB-GLUE-00172] [jakarta.enterprise.web] [tid: _ThreadID=217 _ThreadName=admin-listener(21)]
...
* ProductName-Version represents the current version of Payara Server. For example: `Payara Server {release-id}`
...
[source, log, subs=attributes+]
----
[#|2013-04-18T09:27:44.315-0700|INFO|Payara Server {release-id}|jakarta.enterprise.web.core| _ThreadID=15;_ThreadName=RunLevelControllerThread-1366302462682;_TimeMillis=1366302464315;_LevelValue=800;_MessageID=AS-WEB-CORE-00306;| Setting JAAS app name glassfish-web|#]
----

The Administration Console presents log records in a more readable display. For information about viewing log records using the Administration Console, see xref:administering-the-logging-service.adoc#viewing-log-records[Viewing Log Records].
...
You can configure the logging service to change the default settings for log file rotation, as explained in xref:administering-the-logging-service.adoc#setting-log-file-rotation[Setting Log File Rotation].
...
For information about how to display logger namespaces and log levels, see xref:administering-the-logging-service.adoc#to-list-log-levels[To List Log Levels].
For information about how to display a list of loggers and logger details, see xref:administering-the-logging-service.adoc#to-list-loggers[To List Loggers].
...
a|xref:../command-reference/collect-log-files.adoc[`collect-log-files`]
|Collects all available log files into a ZIP archive.
a|`--target=target-name`

a|xref:../command-reference/list-log-attributes.adoc[`list-log-attributes`]
|Lists logging attributes in the logging properties file.
|target-name operand

a|xref:../command-reference/list-log-levels.adoc[`list-log-levels`]
|Lists the loggers in the logging properties file and their log levels
|target-name operand

a|xref:../command-reference/rotate-log.adoc[`rotate-log`]
|Rotates the log file by renaming it and creating a new log file to store new messages.
a|`--target=target-name`

a|xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`]
|Sets the specified logging attributes in the logging properties file.
a|`--target=target-name`

a|xref:../command-reference/set-log-file-format.adoc[`set-log-file-format`]
|Sets the log file formatter.
a|`--target=target-name`

a|xref:../command-reference/set-log-levels.adoc[`set-log-levels`]
...
|DAS
a|[.breakable]`domain-dir/config/logging.properties`

|A configuration
a|`domain-dir/config/<config-name>/logging.properties`, where `<config-name>` represents the name of a configuration that is shared by one or more instances or clusters.

|An instance
a|`domain-dir/config/<instance-name>-config/logging.properties`, where `<instance-name>` represents the name of the instance.

|A cluster
a|`domain-dir/config/<cluster-name>-config/logging.properties`, where `<cluster-name>` represents the name of the cluster.
...
. Use the xref:../command-reference/list-log-attributes.adoc[`list-log-attributes`] subcommand in remote mode to obtain the current log attribute settings.
+
...
. Use the xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand in remote  mode to define a custom name or location of the log file.
+
...
. List the existing module loggers and log levels by using the xref:../command-reference/list-log-levels.adoc[`list-log-levels`] subcommand.
...
. Set the global log level by using the xref:../command-reference/set-log-levels.adoc[`set-log-levels`] subcommand, specifying the log level of the `java.util.logging.ConsoleHandler` logger.
+
...
. List the existing module loggers and their log levels by using the xref:../command-reference/list-log-levels.adoc[`list-log-levels`] subcommand.
. Set the log level for a module by using the
xref:../command-reference/set-log-levels.adoc[`set-log-levels`] subcommand.
+
...
Log formats for all server instances in a cluster will be the same. For information about log formats, see xref:administering-the-logging-service.adoc#log-records[Log Records].
...
. Set the formatter by using the xref:../command-reference/set-log-file-format.adoc[`set-log-file-format`] subcommand.
. To apply your change, restart Payara Server.
...
image:logging/json_example.png[alt="Example log file with JSON format",width=100%]
...
image:logging/json_config.png[alt="JSON format configuration in Web Console",width=100%]
...
image:logging/json_underscore_prefix_example.png[alt="Example log file with underscore prefix in JSON fields",width=100%]
...
xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand, specifying the following attribute and its value (`true` or `false`): `com.sun.enterprise.server.logging.GFFileHandler.multiLineMode=value`
.  To apply your change, restart Payara Server.
...
As explained in xref:administering-the-logging-service.adoc#log-files[Log Files], Payara Server by default rotates log files when they reach `2 MB` in size.
...
. Ensure that the target server or cluster is running.
. Change the rotation file size limit by using the xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand, specifying the following attribute and the desired limit in bytes: `com.sun.enterprise.server.logging.GFFileHandler.rotationLimitInBytes=bytes`
. To apply your change, restart Payara Server.
...
. Ensure that the target server or cluster is running.
. Change the rotation time limit by using the xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand, specifying the following attribute and the desired limit in minutes: `com.sun.enterprise.server.logging.GFFileHandler.rotationTimelimitInMinutes=minutes`
. To apply your change, restart Payara Server.
...
. Ensure that the target server or cluster is running.
. Change the limit number of retained log files by using the xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand, specifying the following attribute and the desired file limit number: `com.sun.enterprise.server.logging.GFFileHandler.maxHistoryFiles=minutes`
...
image:logging/compress_on_rotation.png[alt="Compress on rotation enabled",width=100%]
...
. Ensure that the target server or cluster is running.
. Rotate log files by using the xref:../command-reference/rotate-log.adoc[`rotate-log`] subcommand.
...
By default, Payara Server log records are captured in a server log file using the format described in xref:administering-the-logging-service.adoc#viewing-log-records[Log Records].
...
. Ensure that the target server or cluster is running. Remote subcommands require a running server.
. Use the xref:../command-reference/set-log-attributes.adoc[`set-log-attributes`] subcommand to add the
...
The *Write to System Log* option allows you to enable and disable the action of logging to the system log. Disabling this option should help to minimize disk usage.

[#enable-write-to-system-log-via-admin-console]
==== Enable Write to System Log Using the Admin Console

To configure the `Write to System Log` option using the Admin Console:

image:logging/write_to_system_log.jpg[alt="Write to System Log enabled",width=100%]

[#enable-write-to-system-log-via-asadmin-cli]
==== Enable Write to System Log Using the Asadmin CLI

Use the following command to enable or disable the Write to System Log feature:
...
[#set-sytem-log-host-via-admin-console]
===== Set the System Log Host Using the Admin Console

To configure the `System Log Host` option using the Admin Console:

image:logging/system_log_host.jpg[alt="System Log Host",width=100%]

[#set-sytem-log-host-via-asadmin-cli]
===== Set the System Log Host Using the Asadmin CLI

Use the following command to set the system log host:

[source, shell]
----
asadmin set-log-attributes com.sun.enterprise.server.logging.SyslogHandler.host=localhost
----

[#set-system-log-facility-via-admin-console]
==== Set the System Log Facility Using the Admin Console

To configure the `System Log Facility` option using the Admin Console:

image:logging/system_log_facility.png[alt="System Log Facility",width=100%]

[#set-system-log-facility-via-asadmin-cli]
==== Set the System Log Facility Using the Asadmin CLI

Use the following command to set the system log facility:

[source, shell]
----
asadmin set-log-attributes com.sun.enterprise.server.logging.SyslogHandler.facility=daemon
----
...
image:logging/log_to_file.png[alt="Log to File enabled",width=100%]
...
image:logging/log_to_file.png[alt="Log to Console disabled",width=100%]
...
image:logging/fast_logging.png[alt="Fast Logging enabled",width=100%]
...
+
Remote subcommands require a running server.
. Use the xref:../command-reference/collect-log-files.adoc[`collect-log-files`] subcommand to create the ZIP archive.
+
...
+
Remote commands require a running server.
. List loggers by using the xref:../command-reference/list-loggers.adoc[`list-loggers`] subcommand.
...
image:logging/max_file_size.png[alt="Max File Size",width=100%]
...
For more information on how to configure HTTP Access logging, see xref:administering-http-connectivity.adoc#administering-http-configurations[Administering HTTP Configurations].

[#notification-logging]
== Notification Service Logging

The Notification Service captures information about events which come from other services, such as the JMX Monitoring Service, the HealthCheck service or the Request Tracing service and captures this information so that xref:administering-the-notification-service.adoc#notifiers[all configured notifiers] can redirect these events. The Log Notifier, turned on by default, will print these notifications events in the server's log.
...
image:logging/notification-logging/payara_notification_log_to_file.png[alt="Log to File enabled",width=100%]
...
image:logging/notification-logging/payara_notification_rotation_on_date_change.png[alt="Rotation on Date Change enabled",width=100%]
...
image:logging/notification-logging/payara_notification_rotation_on_file_size.png[alt="File Rotation Limit",width=100%]
...
image:logging/notification-logging/payara_notification_rotation_on_time_limit.png[alt="File Rotation Time Limit",width=100%]
...
The **Log File Logging Format** option can be used to change the notification log entries' format. There are 3 logging formats available: `ULF`, `ODL` and `JSON`, each one represented by a specific formatter class present in the Azul Payara API.
...
image:logging/notification-logging/payara_notification_change_logging_format.png[alt="Change Logging Format",width=100%]
...
image:logging/notification-logging/payara_notification_set_maximum_number_of_history_files.png[alt="Maximum History File",width=100%]
...
image:logging/notification-logging/payara_notification_change_name_and_location_of_log_file.png[alt="Log File",width=100%]
...
image:logging/notification-logging/payara_notification_set_compress_on_rotation.png[alt="Compress on Rotation Enabled",width=100%]
