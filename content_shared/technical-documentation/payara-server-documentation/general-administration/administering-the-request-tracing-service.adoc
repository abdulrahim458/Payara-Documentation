...
:page-description: Administer the Request Tracing Service in Azul Payara Server — configure tracing of slow requests, configure thresholds, and integrate with OpenTelemetry exporters.
:ordinal: 10
...
Alternatively you can view a short summary xref:../../application-development/using-request-tracing-in-applications.adoc#terminology[here]

The Request Tracing service has the concept of sample rates, affording you extra options to limit the amount of traces processed. Azul Payara offers two options:
...
As Azul Payara offers support for MicroProfile Telemetry, it is possible to manually instrument your application code for creating spans.

For the full details, see the xref:../../microprofile/opentelemetry-and-opentracing.adoc#opentelemetry-and-opentracing[OpenTelemetry support] in the Azul Payara Application Development section.

[#opentracing-support]
== Opentracing Support

In-line with Azul Payara's support for xref:../../microprofile/opentracing.adoc[MicroProfile OpenTracing], there's added support for http://opentracing.io/[OpenTracing.io^] code instrumentation.
...
image:request-tracing/configuration-admin-console.png[alt="Request Tracing Configuration in the Admin Console",width=100%]
...
IMPORTANT: Keep in mind that for request tracing events to be relayed to the selected notifiers, both the xref:administering-the-notification-service.adoc[Notification Service] and each respective notifier must be enabled first.
...
* xref:../command-reference/get-requesttracing-configuration.adoc[`get-requesttracing-configuration`]
* xref:../command-reference/set-requesttracing-configuration.adoc[`set-requesttracing-configuration`]
