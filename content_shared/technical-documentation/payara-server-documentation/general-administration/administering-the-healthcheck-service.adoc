...
:page-description: Administer the Healthcheck Service in Azul Payara Server — configure health checks for CPU, heap memory, thread count, and JDBC connection pool status with threshold alerts.
:ordinal: 9
...
image:healthcheck/admin-console-configuration.png[alt="HealthCheck Configuration in the Admin Console",width=100%]
...
IMPORTANT: Keep in mind that for HealthCheck events to be relayed to the active notifiers, both the xref:administering-the-notification-service.adoc[Notification Service] and each respective notifier must be enabled first.
...
image:healthcheck/cpu-usage-checker-configuration.png[alt="CPU Usage Checker Configuration in the Admin Console",width=100%]
...
image:healthcheck/hogging-threads-checker-configuration.png[alt="Hogging Threads Checker Configuration in the Admin Console",width=100%]
...
image:healthcheck/stuck-threads-checker-configuration.png[alt="Stuck Threads Checker Configuration in the Admin Console",width=100%]
...
For more information and examples for how to use these Asadmin CLI commands, see the xref:../command-reference/set-healthcheck-configuration.adoc[`set-healthcheck-configuration`] and xref:../command-reference/set-healthcheck-service-configuration.adoc[`set-healthcheck-service-configuration`] entries in the Command Reference section.
