...
:page-description: Administer Jakarta Mail in Azul Payara Server — configure mail session resources with SMTP, IMAP, and POP3 settings for use in Jakarta EE applications.
:ordinal: 19
...
| JNDI Name
a| The unique name for the mail session. Use the naming sub-context prefix mail/ for Jakarta Mail resources. For example: `mail/MySession`

| Mail Host
a| The host name of the default mail server. The connect methods of the store and transport objects use this value if a protocol-specific host property is not supplied.
+
...
a| The email address of the default user, in the form: `username@host.domain`.
...
. Create a Jakarta Mail resource by using the xref:../command-reference/create-javamail-resource.adoc[`create-javamail-resource`] subcommand.
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the Jakarta Mail resources by using the xref:../command-reference/list-javamail-resources.adoc[`list-javamail-resources`] subcommand.
...
. List the Jakarta Mail resources by using the xref:../command-reference/list-javamail-resources.adoc[`list-javamail-resources`] subcommand.
. Modify the values for the specified Jakarta Mail source by using the xref:../command-reference/set.adoc[`set`] subcommand. The resource is identified by its dotted name.
...
. Ensure that the server is running. Remote subcommands require a running server.
. List the Jakarta Mail resources by using the xref:../command-reference/list-javamail-resources.adoc[`list-javamail-resources`] subcommands.
. Delete a Jakarta Mail resource by using the xref:../command-reference/delete-javamail-resource.adoc[`delete-javamail-resource`] subcommands.
. To apply your changes, restart Payara Server. xref:administering-domains.adoc#to-restart-a-domain[To Restart a Domain].
...
image:mail/admin-console-javamail-location.png[alt="image",width=100%]
...
image:mail/admin-console-javamail-configuration.png[alt="image",width=100%]
