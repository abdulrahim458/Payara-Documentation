...
:page-description: Administer clusters in Azul Payara Server — create, configure, and manage Payara Server clusters for session replication, load balancing, and high availability.
:ordinal: 3
...
Below are sample xref:../command-reference/get.adoc[`get`] subcommands to get all the GMS configuration settings (attributes associated with the referenced `mycfg` configuration) and GMS cluster settings (attributes and properties associated with a cluster named `mycluster`).
...
. Create a configuration using the xref:../command-reference/copy-config.adoc[`copy-config`] subcommand. For example:
+
[source,shell]
----
...
. Use the xref:../command-reference/get.adoc[`get`] subcommand to determine the settings to change. For example:
+
[source,shell]
----
asadmin> get "configs.config.mycfg.group-management-service.*"
...
----
For a complete list of the dotted names for these settings, see xref:administering-payara-server-clusters.adoc#dotted-names-for-gms-settings[Dotted Names for GMS Settings].
. Use the xref:../command-reference/set.adoc[`set`] subcommand to change the settings. For example:
+
[source,shell]
----
...
. Ensure that the DAS and cluster are running. Remote subcommands require a running server.
. Check whether server instances in a cluster are running by using the xref:../command-reference/list-instances.adoc[`list-instances`] subcommand.
...
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for configuring the cluster, see the xref:../command-reference/create-cluster.adoc[`create-cluster`] help page.
...
* xref:../command-reference/create-cluster.adoc[`create-cluster`]
* xref:../command-reference/create-system-properties.adoc[`create-system-properties`]
...
. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:../command-reference/list-clusters.adoc[`list-clusters`] subcommand.
+
[source,shell]
----
...
xref:../command-reference/list-clusters.adoc[`list-clusters`]
...
*cluster-name*:
  The name of the cluster that you are deleting.
. Run the xref:../command-reference/delete-cluster.adoc[`delete-cluster`] subcommand.
+
[source,shell]
----
...
* xref:administering-payara-server-instances.adoc#to-delete-an-instance-centrally[To Delete an Instance Centrally]
* xref:administering-payara-server-instances.adoc#to-delete-an-instance-locally[To Delete an Instance Locally]
* xref:../command-reference/delete-cluster.adoc[`delete-cluster`]
* xref:../command-reference/list-clusters.adoc[`list-clusters`]
