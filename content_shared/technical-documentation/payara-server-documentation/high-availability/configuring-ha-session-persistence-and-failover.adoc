...
:page-description: Configure HA session persistence and failover in Azul Payara Server — replicate HTTP sessions, stateful EJB state, and SFSB data across cluster nodes.
:ordinal: 9
...
xref:administering-payara-server-clusters.adoc#group-management-service[Group Management Service].
+
...
* The web application must be deployed using the `deploy` subcommand with the `--availabilityenabled` option set to `true`. For more information on these subcommands, see xref:../command-reference/deploy.adoc#deploy[`deploy`].
...
+
For information about how to disable these features, see the xref:../application-deployment/index.adoc[Payara Server Application Deployment section].
* Payara Server does not support High Availability Database (HA-DB) configurations. Instead, use in-memory replication, as described in xref:../../../index.adoc#high-availability-session-persistence[High Availability Session Persistence].
...
+
For instructions, see xref:configuring-http-load-balancing.adoc#setting-up-http-load-balancing[Setting Up HTTP Load Balancing].
...
+
For more information, see xref:configuring-ha-session-persistence-and-failover.adoc#configuring-availability-for-an-individual-bean[Configuring Availability for an Individual Bean].

. Make each web module distributable if you want it to be highly
available.
+
For more information, see "xref:../application-deployment/deploying-applications.adoc#web-module-deployment-guidelines[Web Module Deployment Guidelines]" in the Payara Server Application Deployment section.
...
* xref:configuring-ha-session-persistence-and-failover.adoc#configuring-availability-for-individual-web-applications[Configuring Availability for Individual Web Applications]
* xref:configuring-ha-session-persistence-and-failover.adoc#using-single-sign-on-with-session-failover[Using Single Sign-on with Session Failover]
...
The state of an SFSB is not necessarily transactional and might be significantly modified as a result of non-transactional business methods. If this is the case for an SFSB, you can specify a list of checkpointed methods, as described in xref:configuring-ha-session-persistence-and-failover.adoc#specifying-methods-to-be-checkpointed[Specifying Methods to Be Checkpointed]
...
* If you are deploying using use the `asadmin deploy` commands, set the `--availabilityenabled` option to `true`.
+
For more information, see xref:../command-reference/deploy.adoc[`deploy`].
