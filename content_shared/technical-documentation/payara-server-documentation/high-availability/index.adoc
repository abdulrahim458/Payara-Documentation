...
:page-description: High availability in Azul Payara Server — clustering, session failover, deployment groups, load balancing, and the domain data grid for resilient Jakarta EE deployments.
...
See xref:configuring-http-load-balancing.adoc#configuring-payara-server-with-apache-http-server-and-mod_jk[Configuring Payara Server with Apache HTTP Server and `mod_jk`] and xref:configuring-http-load-balancing.adoc#configuring-payara-server-with-apache-http-server-and-mod_proxy_ajp[Configuring Payara Server with Apache HTTP Server and `mod_proxy_ajp`] for more information.
...
For more information about in-memory session replication in Payara Server, see xref:../../../index.adoc#how-payara-server-provides-high-availability[How Payara Server Provides High Availability].
For detailed instructions on configuring high availability session persistence,
see xref:configuring-ha-session-persistence-and-failover.adoc#configuring-high-availability-session-persistence-and-failover[Configuring High Availability Session Persistence and Failover].
...
+
For more information about Message Queue clustering, see xref:configuring-jms-high-availability.adoc#using-message-queue-broker-clusters-with-payara-server[Using Message Queue Broker Clusters With Payara Server].
...
+
For more information about JMS connection failover, see xref:configuring-jms-high-availability.adoc#connection-failover[Connection Failover].
...
For more information on RMI-IIOP load balancing and failover, see xref:rmi-iiop-load-balancing-and-failover.adoc#rmi-iiop-load-balancing-and-failover[RMI-IIOP Load Balancing and Failover].
...
* xref:../../../index.adoc#storage-for-session-state-data[Storage for Session State Data]
* xref:../../../index.adoc#highly-available-clusters[Highly Available Clusters]
...
For more information, see xref:enabling-centralized-administration-of-server-instances.adoc#enabling-centralized-administration-of-payara-server-instances[Enabling Centralized Administration of Payara Server Instances].
...
* Back up the domain periodically, so you have periodic snapshots. After a hardware failure, re-create the DAS on a new host, as described in "xref:../general-administration/administering-domains.adoc#re-creating-the-domain-administration-server-das[Re-Creating the Domain Administration Server (DAS)]" in Payara Server General Administration section.
...
Payara Server provide tools for backing up and restoring Payara Server instances. For more information, see xref:administering-payara-server-instances.adoc#to-resynchronize-an-instance-and-the-das-offline[To Resynchronize an Instance and the DAS Offline].
...
For information about planning a high-availability deployment, including assessing hardware requirements, planning network configuration, and selecting a topology, see the xref:../deployment-planning/index.adoc[Payara Server Deployment Planning section]. This manual also provides a high-level introduction to concepts such as:
