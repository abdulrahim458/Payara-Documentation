...
:page-description: Configure JMS high availability in Azul Payara Server — set up OpenMQ broker clusters for Jakarta Messaging failover and message persistence across cluster nodes.
:ordinal: 10
...
.  Configure the Payara Server cluster to use a Message Queue conventional broker cluster with master broker by using the xref:../command-reference/configure-jms-cluster.adoc[`configure-jms-cluster`] subcommand:
+
[source,shell]
----
...
+
For information about password file entries, see the xref:../command-reference/asadmin.adoc[`asadmin`] help page.
...
. Change the master broker by using the xref:../command-reference/change-master-broker.adoc[`change-master-broker`] subcommand:
+
[source,shell]
----
...
. Configure the Payara Server cluster to use a Message Queue enhanced broker cluster by using the xref:../command-reference/configure-jms-cluster.adoc[`configure-jms-cluster`] subcommand:
+
[source,shell]
----
...
. Delete the `default_JMS_host` JMS host by using the xref:../command-reference/delete-jms-host.adoc[`delete-jms-host`] subcommand:
+
[source,shell]
----
asadmin delete-jms-host --target payara-cluster-name default_JMS_host
----
. Create a JMS host for each broker in the broker cluster by using the xref:../command-reference/create-jms-host.adoc[`create-jms-host`] subcommand.
+
