...
CAUTION: Before using this procedure to reconfigure an existing cluster, you must follow the special procedures to migrate to another type of broker cluster, as described in xref:configuring-jms-high-availability.adoc#to-migrate-between-types-of-embedded-or-local-conventional-broker-clusters[To Migrate Between Types of Embedded or Local Conventional Broker Clusters]. +
...
.  Configure the Payara Server cluster to use a Message Queue conventional broker cluster with master broker by using the xref:technical-documentation/payara-server-documentation/command-reference/configure-jms-cluster.adoc[`configure-jms-cluster`] subcommand:
+
[source,shell]
----
...
+
For information about password file entries, see the xref:technical-documentation/payara-server-documentation/command-reference/asadmin.adoc[`asadmin`] help page.
...
. Change the master broker by using the xref:technical-documentation/payara-server-documentation/command-reference/change-master-broker.adoc[`change-master-broker`] subcommand:
+
[source,shell]
----
...
. Configure the Payara Server cluster to use a Message Queue enhanced broker cluster by using the xref:technical-documentation/payara-server-documentation/command-reference/configure-jms-cluster.adoc[`configure-jms-cluster`] subcommand:
+
[source,shell]
----
...
. Delete the `default_JMS_host` JMS host by using the xref:technical-documentation/payara-server-documentation/command-reference/delete-jms-host.adoc[`delete-jms-host`] subcommand:
+
[source,shell]
----
asadmin delete-jms-host --target payara-cluster-name default_JMS_host
----
. Create a JMS host for each broker in the broker cluster by using the xref:technical-documentation/payara-server-documentation/command-reference/create-jms-host.adoc[`create-jms-host`] subcommand.
+
...
. Create instances in the Payara Server cluster, as described in xref:administering-payara-server-instances.adoc#to-create-an-instance-centrally[To Create an Instance Centrally] and xref:administering-payara-server-instances.adoc#to-create-an-instance-locally[To Create an Instance Locally].
...
You can override these settings using JMS connection factory settings. For details, see "xref:../general-administration/administering-the-java-message-service.adoc#administering-jms-connection-factories-and-destinations[Administering JMS Connection Factories and Destinations]" in the Payara Server General Administration section.
