...
For information about setting up SSH, see xref:enabling-centralized-administration-of-server-instances.adoc#enabling-centralized-administration-of-payara-server-instances[Enabling Centralized Administration of Payara Server Instances].
...
** xref:administering-payara-server-nodes.adoc#to-create-an-ssh-node[To Create an `SSH` Node]
* The user of the DAS can use SSH to log in to the host for the node where the instance is to reside.

If any of these prerequisites is not met, create the instance locally as explained in xref:administering-payara-server-instances.adoc#to-create-an-instance-locally[To Create an Instance Locally].

If you are adding the instance to a cluster, ensure that the cluster to which you are adding the instance exists. For information about how to create a cluster, see xref:administering-payara-server-clusters.adoc#to-create-a-cluster[To Create a Cluster].

If the instance is to reference an existing named configuration, ensure that the configuration exists. For more information, see xref:administering-named-configurations.adoc#to-create-a-named-configuration[To Create a Named Configuration].
...
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for configuring the instance, see the xref:technical-documentation/payara-server-documentation/command-reference/create-instance.adoc[`create-instance`] help page.
...
* xref:administering-payara-server-nodes.adoc#to-create-an-ssh-node[To Create an `SSH` Node]
* xref:administering-payara-server-instances.adoc#to-create-an-instance-locally[To Create an Instance Locally]
...
. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/list-instances.adoc[`list-instances`] subcommand.
+
[source,shell]
----
...
If this instance must be deleted, use the xref:technical-documentation/payara-server-documentation/command-reference/change-master-broker.adoc[`change-master-broker`] subcommand first to assign the master broker to a different instance.
...
* xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-centrally[To Stop an Individual Instance Centrally]
* xref:administering-payara-server-instances.adoc#to-stop-an-individual-instance-locally[To Stop an Individual Instance Locally]
...
. Run the xref:technical-documentation/payara-server-documentation/command-reference/delete-instance.adoc[`delete-instance`] subcommand.
+
[source,shell]
----
...
* xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-centrally[To Stop an Individual Instance Centrally]
* xref:administering-payara-server-instances.adoc#to-stop-an-individual-instance-locally[To Stop an Individual Instance Locally]
* xref:../command-reference/change-master-broker.adoc[`change-master-broker`(1)]
* xref:../command-reference/delete-instance.adoc[`delete-instance`]
* xref:../command-reference/list-instances.adoc[`list-instances`]
...
If any of these prerequisites is not met, start the cluster by starting each instance locally as explained in xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-locally[To Start an Individual Instance Locally].

. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/start-cluster.adoc[`start-cluster`] subcommand.
+
[source,shell]
----
...
* xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-locally[To Start an Individual Instance Locally]
...
. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/stop-cluster.adoc[`stop-cluster`] subcommand.
+
[source,shell]
----
...
If any of these prerequisites is not met, start the instance locally as explained in xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-locally[To Start an Individual Instance Locally].
...
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for controlling the behavior of the instance, see the xref:technical-documentation/payara-server-documentation/command-reference/start-instance.adoc[`start-instance`] help page.
...
instance Location: /export/payara7/glassfish/nodes/sj02/pmd-i2
Log File: /export/payara7/glassfish/nodes/sj02/pmd-i2/logs/server.log
Admin Port: 24851
...
. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/stop-instance.adoc[`stop-instance`] subcommand.
...
When this subcommand restarts an instance, the DAS synchronizes the instance with changes since the last synchronization as described in xref:administering-payara-server-instances.adoc#default-synchronization-for-files-and-directories[Default Synchronization for Files and Directories].

If you require different synchronization behavior, stop and start the
instance as explained in xref:administering-payara-server-instances.adoc#to-resynchronize-an-instance-and-the-das-online[To Resynchronize an Instance and the DAS Online].

. Ensure that the DAS is running. Remote subcommands require a running server.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/restart-instance.adoc[`restart-instance`] subcommand.
+
[source,shell]
----
...
* xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-centrally[To Stop an Individual Instance Centrally]
* xref:administering-payara-server-instances.adoc#to-start-an-individual-instance-centrally[To Start an Individual Instance Centrally]
* xref:technical-documentation/payara-server-documentation/command-reference/restart-instance.adoc#restart-instance[`restart-instance`(1)]
...
TIP: Even SSH is set up, you can obtain information about instances in a domain without logging in to each host where remote instances reside. For instructions, see xref:administering-payara-server-instances.adoc#to-list-all-instances-in-a-domain[To List All Instances in a Domain].
...
If you change GMS-related system properties for an existing instance, the DAS and the instance must be restarted to apply the changes. For information about GMS, see xref:administering-payara-server-clusters.adoc#group-management-service[Group Management Service].
...
* xref:administering-payara-server-nodes.adoc#to-create-an-ssh-node[To Create an `SSH` Node]
* xref:administering-payara-server-nodes.adoc#to-create-a-config-node[To Create a `CONFIG` Node]
* xref:administering-payara-server-nodes.adoc#creating-a-docker-node[To Create a `DOCKER` Node]
...
+
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for configuring the instance, see the xref:../command-reference/create-local-instance.adoc[`create-local-instance`] help page.
...
NODE NAME          TYPE    NODE HOST         INSTALL DIRECTORY   REFERENCED BY
localhost-domain1  CONFIG  localhost         /export/payara7
xk01               CONFIG  xk01.example.com  /export/payara7  kui-i1
Command list-nodes executed successfully.
----
...
If this instance must be deleted, use the xref:technical-documentation/payara-server-documentation/command-reference/change-master-broker.adoc[`change-master-broker`] subcommand to assign the master broker to a different instance first.
...
*instance-name*:
The name of the instance that you are deleting.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/delete-local-instance.adoc[`delete-local-instance`] subcommand.
+
[source,shell]
----
...
For information about all the options for controlling the behavior of the instance, see the xref:technical-documentation/payara-server-documentation/command-reference/start-local-instance.adoc[`start-local-instance`] help page.
...
After starting an instance, you can deploy applications to the instance. For more information, see the xref:../application-deployment/index.adoc[Payara Server Application Deployment section].
...
. Log in to the host that is represented by the node where the instance resides.
. Run the xref:technical-documentation/payara-server-documentation/command-reference/stop-local-instance.adoc[`stop-local-instance`] subcommand.
+
[source,shell]
----
...
WARNING: When xref:../command-reference/auto-naming.adoc[auto-naming] is enabled, Payara Server will only attempt to resolve conflicts with instance names - it will not attempt to resolve conflicts in container name.
...
Payara Server supports creating instances by using Docker directly, making use of the xref:../command-reference/auto-naming.adoc[auto-naming] feature to resolve any conflicts.
...
----
docker container create --network host --mount type=bind,source="/app/passwordfile-docker.txt",target="/pathInDocker/passwordfile.txt",readonly -e PAYARA_DAS_HOST=payaraDas -e PAYARA_DAS_PORT=4848 -e PAYARA_PASSWORD_FILE=/pathInDocker/passwordfile.txt payara/server-node:{release-id}
...
./payara7/bin/asadmin -I false -T -a -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt _create-node-temp --nodehost 192.168.1.103
Running command create-local-instance:
./payara7/bin/asadmin -I false -T -a -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt create-local-instance --node Sarcastic-Catfish --dockernode true --ip 192.168.1.103
...
./payara7/bin/asadmin -I false -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt _set-docker-container-id --instance Cooperative-Spookfish --id 994e6d5db276304843481601932857fa224dfd9f9cda8578f3b09f8f11bf6004
...
a|Arrays must be surrounded with square braces & array values separated using the vertical bar symbol `\|`
a|[.breakable]`{ENV: [arg1=foo,arg2=bar]}`

a|[.breakable]`Docker.Env=[arg1:foo\|arg2:bar]`
|Each child object of a parent object is specified individually

a|[.breakable]`{HostConfig: {Memory: 2048, CpuShares: 3}`
a|
`Docker.HostConfig.Memory=2048`

`Docker.HostConfig.CpuShares=3`

a|The colon character is used to denote the value of an object within an array & objects within an array are
separated using a comma `,`
a|[.breakable]`{HostConfig: {BlkioDeviceReadBps: [{Path: /opt/foo, Rate: 24},{Path: /opt/bar, Rate: 48}]}`

a|[.breakable]`Docker.HostConfig.BlkioDeviceReadBps=[Path:/opt/foo,Rate:24\|Path:/opt/bar,Rate:48]`
...
The following Docker Environment properties are used by the default Docker image, `payara/server-node:{release-id}`, which you may wish to override to match your configuration (particularly if creating containers directly from Docker):
...
a| First result of `hostname -I` (all IP addresses, excluding loopback)

a| `PAYARA_DEPLOYMENT_GROUP`
a| The name of the xref:administering-deployment-groups.adoc[Deployment Group] that the instance should join.
...
./payara7/bin/asadmin -I false -t -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt list-instances --nostatus ManagedContainer2
...
./payara7/bin/asadmin -I false -t -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt _get-docker-container-id --instance ManagedContainer2
...
Running command create-local-instance:
./payara7/bin/asadmin -I false -T -a -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt create-local-instance --node DockerNode1 --dockernode true --ip dockerHost1 ManagedContainer2
...
./payara7/bin/asadmin -I false -H payaraDas -p 4848 -W /pathInDocker/passwordfile.txt _set-docker-container-id --instance ManagedContainer2-PerfectZiege --id af48bec58c144bad8ac83c9344dcebc4b9a6d528dd8673a6e6f5275e8b3ed2a2
Command _set-docker-container-id executed successfully.
...
payara/server-node   custom-{release-id}       386a996b3649        About an hour ago   511MB
payara/server-node   {release-id}              f5cf02e10dc2        4 weeks ago         525MB
...
* Whenever an `asadmin` subcommand is run. For more information, see xref:../general-administration/index.adoc#impact-of-configuration-changes[Impact of Configuration Changes]" in Payara Server General Administration section.
...
The `--sync` option of the subcommands for starting an instance controls the type of synchronization between the DAS and the instance's files when the instance is started. You can use this option to override the default synchronization behavior for the files and directories of an instance. For more information, see xref:administering-payara-server-instances.adoc#to-resynchronize-an-instance-and-the-das-online[To Resynchronize an Instance and the DAS Online].
...
+
For more information, see xref:administering-payara-server-instances.adoc#to-resynchronize-additional-configuration-files[To Resynchronize Additional Configuration Files].
...
+
For more information, see the xref:technical-documentation/payara-server-documentation/command-reference/create-virtual-server.adoc[`create-virtual-server`] help
  page.
+
...
+
The contents of these directories are loaded by the common class loader. For more information, see "xref:../../application-development/class-loaders.adoc[Using the Common Class Loader]" in the Payara Server Application Development section.
+
...
* xref:../command-reference/list-instances.adoc[`list-instances`]
* xref:../command-reference/start-instance.adoc[`start-instance`]
* xref:../command-reference/start-local-instance.adoc[`start-local-instance`]
...
a|`domain-dir/lib/classes`

|Application-specific libraries.
a|`domain-dir/lib/applibs`

|Optional packages for all applications in a domain.
a|`domain-dir/lib/ext`

|Library files for all applications that are deployed to a specific cluster or standalone instance.
a|[.breakable]`domain-dir/config/<config-name>/lib`

a|Optional packages for all applications that are deployed to a specific
cluster or standalone instance.
a|[.breakable]`domain-dir/config/<config-name>/lib/ext`
...
* xref:../command-reference/create-jvm-options.adoc[`create-jvm-options`]
* xref:../command-reference/delete-jvm-options.adoc[`delete-jvm-options`]
...
+
For instructions, see xref:administering-payara-server-instances.adoc#to-resynchronize-an-instance-and-the-das-online[To Resynchronize an Instance and the DAS Online].

*See Also*

* xref:administering-payara-server-instances.adoc#to-resynchronize-an-instance-and-the-das-online[To Resynchronize an Instance and the DAS Online]
...
Put the files in a subdirectory under the domain directory that is not a pre-defined directory, for example, `/payara7/glassfish/domains/domain1/myapp/myfile`.
...
+
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for exporting the configuration data, see the xref:technical-documentation/payara-server-documentation/command-reference/export-sync-bundle.adoc[`export-sync-bundle`] help page.
+
...
+
NOTE: Only the options that are required to complete this task are provided in this step. For information about all the options for importing the configuration data, see the xref:technical-documentation/payara-server-documentation/command-reference/import-sync-bundle.adoc[`import-sync-bundle`] help page.
+
[source,shell]
----
...
* xref:../command-reference/export-sync-bundle.adoc[`export-sync-bundle`]
* xref:../command-reference/import-sync-bundle.adoc[`import-sync-bundle`]
...
* xref:administering-payara-server-instances.adoc#to-enable-automatic-ejb-timer-migration-for-failed-clustered-instances[To Enable Automatic EJB Timer Migration for Failed Clustered Instances]
* xref:administering-payara-server-instances.adoc#to-migrate-ejb-timers-manually[To Migrate EJB Timers Manually]
...
For information about the GMS, see xref:administering-payara-server-clusters.adoc#group-management-service[Group Management Service].
...
For instructions on enabling delegated transaction recovery, see "xref:../general-administration/administering-transactions.adoc#administering-transactions[Administering Transactions]" in Payara Server General Administration section.
