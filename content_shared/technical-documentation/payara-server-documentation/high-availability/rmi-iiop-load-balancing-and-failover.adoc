...
:page-description: RMI-IIOP load balancing and failover in Azul Payara Server — configure IIOP request distribution and failover for remote EJB clients across clustered Payara instances.
:ordinal: 11
...
See xref:rmi-iiop-load-balancing-and-failover.adoc#enabling-rmi-iiop-hardware-load-balancing-and-failover[Enabling RMI-IIOP Hardware Load Balancing and Failover].
...
==== xref:rmi-iiop-load-balancing-and-failover.adoc#initialcontext-load-balancing[`InitialContext` Load Balancing]
...
==== xref:rmi-iiop-load-balancing-and-failover.adoc#per-request-load-balancing-prlb[Per-Request Load Balancing (PRLB)]
...
+
This utility produces an `appclient.jar` file. For more information on `package-appclient`, see xref:../command-reference/package-appclient.adoc#package-appclient[`package-appclient`].
...
+
For a list of the path variables to update, see xref:../command-reference/package-appclient.adoc[`package-appclient`].
...
+
You specify the IIOP listeners as endpoints in xref:rmi-iiop-load-balancing-and-failover.adoc#add-at-least-two-target-server-elements-in-the-sun-acc.xml-file[Add at least two `target-server` elements in the `sun-acc.xml` file.].
+
...
+
Use the endpoints that you obtained in xref:rmi-iiop-load-balancing-and-failover.adoc#find-the-iiop-listener-port-number-for-at-least-two-instances-in-the-cluster[Find the IIOP listener port number for at least two instances in the cluster].
+
...
+
The `target-server` element specifies one or more IIOP endpoints used for load balancing. The `address` attribute is an IPv4 address or hostname, and the `port` attribute specifies the port number. See "xref:../application-deployment/elements-of-deployment-descriptors.adoc#client-container[client-container]" in the Payara Server Application Deployment section.
+
...
See xref:configuring-ha-session-persistence-and-failover.adoc#enabling-the-high-availability-session-persistence-service[Enabling the High Availability Session Persistence Service] for instructions on enabling the session availability service for a cluster or for a Web, EJB, or JMS container running in a cluster.
...
+
* "xref:../../application-development/using-jakarta-enterprise-beans-technology.adoc#using-enterprise-javabeans-technology[Using Jakarta Enterprise Beans Technology]" in the Payara Server Application Development section
* "xref:../application-deployment/deploying-applications.adoc#ejb-module-deployment-guidelines[EJB Module Deployment Guidelines]" in the Payara Server Application Deployment section
* "xref:../application-deployment/deploying-applications.adoc#assembling-and-deploying-an-application-client-module[Assembling and Deploying an Application Client Module]" in the Payara Server Application Deployment section
. Set the `per-request-load-balancing` property to `true` in the `glassfish-ejb-jar.xml` deployment descriptor file for the EJB.
+
For more information about the `glassfish-ejb-jar.xml` deployment descriptor file, refer to "xref:../application-deployment/deployment-descriptor-files.adoc#the-payara-ejb-jar.xml-file[The glassfish-ejb-jar.xml File]" in the Payara Server Application Deployment section.
+
For example, the `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` file for a sample EJB named `TheGreeter` is listed below.
+
[source,xml]
----
<payara-ejb-jar>
...
* "xref:../application-deployment/deploying-applications.adoc#to-deploy-an-application-or-module[To Deploy an Application or Module]" in the Payara Server Application Deployment section
* "xref:../application-deployment/deploying-applications.adoc#to-redeploy-an-application-or-module[To Redeploy an Application or Module]" in the Payara Server Application Deployment section
