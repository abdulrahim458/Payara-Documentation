...
:page-description: Administer deployment groups in Azul Payara Server — group server instances for targeted application deployments without full clustering configuration.
:ordinal: 4

Deployment Groups were introduced in Azul Payara 5 as an effective replacement for xref:administering-payara-server-clusters.adoc[clusters]. Deployment groups are a complement to the xref:administering-the-domain-data-grid.adoc[Data Grid] whereas the grid is responsible to managing distributed application data across multiple instances, deployment groups manage how application deployment is targeted in general.

A Deployment Group is a loose grouping of Payara Server instances that can be used as a target for *deployments*. Deployment Groups consist of one or more Payara Server xref:administering-payara-server-instances.adoc#types-of-payara-server-instances[standalone instances] and each instance can belong to one or more deployment groups.
...
image:deployment-groups/deployment-group-example.png[alt="Deployment Group Example",width=600px]
...
+
image:deployment-groups/deployment-group-new.png[alt="Create New Deployment Group",width=100%]
+
IMPORTANT: Instances created in this screen will be created alongside a corresponding xref:../command-reference/configuration.adoc[configuration] object that is a copy of the `default-config` object. +
...
To create a new deployment group run the xref:../command-reference/create-deployment-group.adoc[`create-deployment-group`] command:
...
+
image:deployment-groups/delete-deployment-group.png[alt="Delete a Deployment Group",width=100%]

[#delete-using-asadmin-cli]
==== Using Asadmin Commands

To delete a deployment group use the xref:../command-reference/delete-deployment-group.adoc[`delete-deployment-group`] command:
...
+
image:deployment-groups/add-server-to-dg.png[alt="Add a Server to a Deployment Group",width=100%]

The same result can be achieved from the command line by using the xref:../command-reference/add-instance-to-deployment-group.adoc[`add-instance-to-deployment-group`] command:
...
+
image:deployment-groups/remove-server-from-dg.png[alt="Remove a Server from a Deployment Group",width=100%]

The same can be achieved using the Asadmin CLI by using the xref:../command-reference/remove-instance-from-deployment-group.adoc[`remove-instance-from-deployment-group`] command:
...
+
image:deployment-groups/lifecycle-dg.png[alt="Server Lifecycle Management",width=600px]
...
* xref:../command-reference/start-deployment-group.adoc[`start-deployment-group`]
* xref:../command-reference/stop-deployment-group.adoc[`stop-deployment-group`]
* xref:../command-reference/restart-deployment-group.adoc[`restart-deployment-group`]
...
image:deployment-groups/deploy-to-dg.png[alt="Deploy to a deployment group",width=100%]

To deploy an application from the command line by using the xref:../command-reference/deploy.adoc[`deploy`] command, just specify the deployment group as a *target* like this:
...
* xref:administering-the-domain-data-grid.adoc[Domain Data Grid]
* xref:../command-reference/create-deployment-group.adoc[`create-deployment-group`]
* xref:../command-reference/delete-deployment-group.adoc[`delete-deployment-group`]
* xref:../command-reference/start-deployment-group.adoc[`start-deployment-group`]
* xref:../command-reference/stop-deployment-group.adoc[`stop-deployment-group`]
* xref:../command-reference/restart-deployment-group.adoc[`restart-deployment-group`]
* xref:../command-reference/remove-instance-from-deployment-group.adoc[`remove-instance-from-deployment-group`]
* xref:../command-reference/add-instance-to-deployment-group.adoc[`add-instance-to-deployment-group`]
