...
:page-description: Deploy applications to Azul Payara Server — deploy WAR, EAR, and JAR archives using the Admin Console, asadmin CLI, Maven plugin, or REST management interface.
:ordinal: 1
...
You can see the enabled or disabled status of an application or module by using the xref:../command-reference/show-component-status.adoc[`show-component-status`] subcommand.

For information about how the application or module name is derived, see xref:../../../index.adoc#naming-standards[Naming Standards].
...
For more information about deployment descriptors associated with Payara Server, see xref:../../../index.adoc#deployment-descriptors-and-annotations[Deployment Descriptors and Annotations].
...
. List deployed applications by using the xref:../command-reference/list-applications.adoc[`list-applications`] subcommand.
. Deploy the application or module by using the xref:../command-reference/deploy.adoc[`deploy`] subcommand.
+
...
xref:../command-reference/create-application-ref.adoc[`create-application-ref`] and
xref:../command-reference/delete-application-ref.adoc[`delete-application-ref`] subcommands.
...
. Ensure that the server is running. Remote commands require a running server.
. List the desired applications by using the xref:../command-reference/list-applications.adoc[`list-applications`] subcommand or
the xref:../command-reference/list-sub-components.adoc[`list-sub-components`] subcommand.
+
Information about these commands is included in these help pages.
. Show the status of a deployed component by using the xref:../command-reference/show-component-status.adoc[`show-component-status`] subcommand.
...
xref:../command-reference/redeploy.adoc[`redeploy`] subcommand or the xref:../command-reference/deploy.adoc[`deploy`] subcommand with the `--force` option.
+
...
To list deployed applications or modules, use the xref:../command-reference/list-applications.adoc[`list-applications`] subcommand.
...
To see the status of deployed components, use the xref:../command-reference/show-component-status.adoc[`show-component-status`] subcommand.
. Deactivate the application or module by using the xref:../command-reference/disable.adoc[`disable`] subcommand.
+
Information about the options and properties of the subcommand is included in this help page.
...
. Ensure that the server is running. Remote commands require a running server.
. Enable the application or module by using the xref:../command-reference/enable.adoc[`enable`] subcommand.
+
...
+
To see the status of deployed components, use the xref:../command-reference/show-component-status.adoc[`show-component-status`] subcommand.
. Undeploy the application or module by using the xref:../command-reference/undeploy.adoc[`undeploy`] subcommand.
+
Information about the options and properties of the subcommand is included in this help page.
...
. Use the xref:../command-reference/set.adoc[`set`] subcommand to adjust the auto-deployment interval.
+
...
. Use the xref:../command-reference/set.adoc[`set`] subcommand to enable JSP precompilation.
...
In the deployment plan for an EAR file, the `payara-application.xml` file is located at the root. The deployment descriptor for each module is stored according to this syntax: `module-name.gf-dd-name`, where the `gf-dd-name` depends on the module type.

Each `/` (forward slash) is replaced by a `#` (pound sign).

. Ensure that the server is running. Remote commands require a running server.
. Deploy the application or module by using the xref:../command-reference/deploy.adoc[`deploy`] subcommand with the `--deploymentplan` option.
+
NOTE: Deployment directories might change between Payara Server releases.
...
. Deploy the directory by using the xref:../command-reference/deploy.adoc[`deploy`] subcommand and specifying the path to the expanded directory.
+
NOTE: Deployment directories might change between Payara Server releases.
...
Payara Server also has the ability to deploy applications whose archives are hosted on a remote location. To deploy an application archive that can be reached via its URL, use the xref:../command-reference/deploy-remote-archive.adoc[`deploy-remote-archive`] subcommand, which works similarly to the standard `deploy` command, and can be used in the same way.
...
Payara Server also has the ability to deploy applications whose archives are hosted as Maven artifacts on remote repositories. To deploy an application archive in this manner, use the xref:../command-reference/deploy-remote-archive.adoc[`deploy-remote-archive`] subcommand, which works similarly to the standard `deploy` command, and point it to a set of Maven GAV (Group ID/Artifact ID/Version) coordinates where the artifact is located.
...
+
Remote commands require a running server.
. Set a servlet context-initialization parameter by using the xref:../command-reference/set-web-context-param.adoc[`set-web-context-param`] subcommand.
+
...
Instead, use the xref:../command-reference/set-web-context-param.adoc[`set-web-context-param`] subcommand for this purpose.
...
+
Remote commands require a running server.
. Unset an environment entry by using the xref:../command-reference/unset-web-context-param.adoc[`unset-web-context-param`] subcommand.
+
Information about the options for the subcommand is included in this help page.
...
Use the `list-web-context-param` subcommand in remote mode to list the parameters that have previously been set by using the xref:../command-reference/set-web-context-param.adoc[`set-web-context-param`] subcommand.
...
+
Remote commands require a running server.
. List servlet context-initialization parameters by using the xref:../command-reference/list-web-context-param.adoc[`list-web-context-param`] subcommand.
...
+
Remote commands require a running server.
. Set an environment entry for a deployed web application or module by using the xref:../command-reference/set-web-env-entry.adoc[`set-web-env-entry`] subcommand.
+
Information about the options for the subcommand is included in this help page.
...
+
Remote commands require a running server.
. Unset a web environment entry by using the xref:../command-reference/unset-web-env-entry.adoc[`unset-web-env-entry`] subcommand.
+
Information about the options for the subcommand is included in this help page.
...
+
Remote commands require a running server.
. List the environment entries by using the xref:../command-reference/list-web-env-entry.adoc[`list-web-env-entry`] subcommand.
...
+
For information about HTTP session persistence, see the xref:../high-availability/index.adoc[Payara Server High Availability section].
...
+
For more information about JSP precompilation, see xref:elements-of-deployment-descriptors.adoc#jsp-config[`jsp-config`].
...
* *JNDI Name*. — If no JNDI name for the EJB JAR module is specified in the `jndi-name` element immediately under the `ejb` element in `payara-ejb-jar.xml`, or there is no `glassfish-ejb-jar.xml` file, a default, non-clashing JNDI name is derived.
+
...
* *Stateful Session Bean and Timer State*. — Use the `--keepstate` option of the xref:../command-reference/redeploy.adoc[`redeploy`] subcommand or the `<keepstate>` element in the `glassfish-ejb-jar.xml` file to retain stateful session bean instances and persistently created EJB timers across re-deployments.
+
...
* *EJB Singletons*. — EJB Singletons are created for each server instance in a cluster, and not once per cluster or deployment group. Check the xref:../../public-api/clustered-singleton.adoc[Clustered Singleton] proprietary feature of the Public API on how to achieve this.

* *Stubs and Ties*. — Use the xref:../command-reference/get-client-stubs.adoc[`get-client-stubs`] subcommand in remote mode to retrieve stubs and ties.

* *Compatibility of JAR Visibility Requirements*. — Use the `compatibility` element of the `payara-application.xml` or `payara-ejb-jar.xml` file to specify the Payara Server release with which to be backward compatible in terms of JAR visibility requirements for applications.
+
...
For more information, see xref:deploying-applications.adoc#application-scoped-resources[Application-Scoped Resources].

. Ensure that the server is running.
+
Remote commands require a running server.
. Deploy the connector module by using the xref:../command-reference/deploy.adoc#deploy[`deploy`] subcommand.
. Configure connector connection pools for the deployed connector module.
+
Use the `create-connector-connection-pool` subcommand. For procedures, see "xref:../general-administration/administering-eis-connectivity.adoc#to-create-a-connector-connection-pool[To Create a Connector Connection Pool]" in the __General Administration section__.

. Configure connector resources for the connector connection pools.
+
Use the `create-resource-adapter-config` subcommand. For procedures, see "xref:../general-administration/administering-eis-connectivity.adoc#to-create-configuration-information-for-a-resource-adapter[To Create Configuration Information for a Resource Adapter]" in the __General Administration section__.
+
...
"xref:../general-administration/administering-eis-connectivity.adoc#to-create-configuration-information-for-a-resource-adapter[To Create Configuration Information for a Resource Adapter]" in the __ General Administration section__.
+
If needed, you can override the default configuration properties of a resource adapter.
...
"xref:../general-administration/administering-eis-connectivity.adoc#to-create-an-administered-object[To Create an Administered Object]" in the __General Administration section__.
...
For more information about class loaders, see "xref:../../application-development/class-loaders.adoc#class-loaders[Class Loaders]" in the Application Development section.
...
+
Instructions are contained in xref:deploying-applications.adoc#to-deploy-an-application-or-module[To Deploy an Application or Module].
...
* Use the xref:../command-reference/deploy.adoc[`deploy`] subcommand with the `--retrieve` option to retrieve the client files as part of deploying the application.
* Use the xref:../command-reference/get-client-stubs.adoc[`get-client-stubs`] subcommand to retrieve client files for a previously-deployed application.
...
+
TIP: For more detailed information about the `appclient` script, see xref:../command-reference/appclient.adoc[`appclient`].

For more detailed information about creating application clients, see
"xref:../../application-development/developing-java-clients.adoc[Developing Java Clients]" in the Application Development section. This chapter includes information on the following topics:
...
For details about naming, see xref:../../../index.adoc#naming-standards[Naming Standards].
...
TIP: For more detailed information about the `package-appclient` script, see xref:../command-reference/appclient.adoc[`appclient`(1M)].
...
+
The Payara Server specific deployment descriptor files, `glassfish-web.xml`, `payara-web.xml` and `glassfish-ejb-jar.xml`, provide optional web service enhancements in their xref:elements-of-deployment-descriptors.adoc#webservice-endpoint[`webservice-endpoint`] and xref:elements-of-deployment-descriptors.adoc#webservice-description[`webservice-description`] elements.
...
For more information about web services, see "xref:../../application-development/developing-soap-webservices.adoc#developing-web-services[Developing Web Services]" in the Application Development section.
...
You can set this property in the `glassfish-resources.xml` or `payara-resources.xml` file. For more information, see the property descriptions under xref:elements-of-deployment-descriptors.adoc#jdbc-connection-pool[`jdbc-connection-pool`].
...
For more information about the `glassfish-resources.xml` and `payara-resources.xml` file, see xref:deployment-descriptor-files.adoc#the-payara-resources.xml-file[Payara Server Deployment Descriptor Files] and xref:elements-of-deployment-descriptors.adoc[Elements of the Payara Server Deployment Descriptors].
...
For more information, see xref:../command-reference/redeploy.adoc[`redeploy`] and xref:../command-reference/deploy.adoc[`deploy`].
...
For more information, see xref:../command-reference/list-applications.adoc[`list-applications`] and xref:../command-reference/list-sub-components.adoc[`list-sub-components`].
