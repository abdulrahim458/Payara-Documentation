...
|zero or one
|Allows you to change the properties of the default, built-in  xref:elements-of-deployment-descriptors.adoc#cache-helper[`cache-helper`] class.
...
|`timeout-in-seconds` |`30` |(optional) Specifies the maximum amount of time in seconds that an entry can remain in the cache after it is created or refreshed. Can be overridden by a xref:elements-of-deployment-descriptors.adoc#timeout[`timeout`] element.
...
|`cacheClassName` |`com.sun.appserv.web.cache.LruCache` |Specifies the fully qualified name of the class that implements the cache functionality. See xref:elements-of-deployment-descriptors.adoc#cache-class-names[Cache Class Names] for possible values.
...
|`name` |`default` |Specifies a unique name for the helper class, which is referenced in the  xref:elements-of-deployment-descriptors.adoc#cache-mapping[`cache-mapping`] element.
...
|Contains the `name` of the  xref:elements-of-deployment-descriptors.adoc#cache-helper[`cache-helper`] used by the parent `cache-mapping` element.
...
|zero or one if `cache-helper-ref` is not used
|Contains the xref:elements-of-deployment-descriptors.adoc#cache-mapping[`cache-mapping`] specific maximum amount of time in seconds that an entry can remain in the cache after it is created or refreshed.
...
|`name` |none |Specifies the name of the connection pool. A xref:elements-of-deployment-descriptors.adoc#connector-resource[`connector-resource`] element's `pool-name` attribute refers to this `name`.
...
|`cacheTTL` |`5000` |This property sets the time to live (TTL), in milliseconds, for the virtual server cache used when serving files.
...
|`pool-name` |none |Specifies the `name` of the associated xref:elements-of-deployment-descriptors.adoc#jdbc-connection-pool[`jdbc-connection-pool`].
