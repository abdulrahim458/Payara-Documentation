...
:page-description: Security in high-availability environments for Azul Payara Server — configure SSL/TLS between clustered instances, secure JMS replication, and HA-specific security settings.
:ordinal: 4
...
This chapter assumes that you are familiar with security features such as authentication, authorization, and certificates. If you are not, see xref:administering-system-security.adoc#administering-system-security[Administering System Security].

[#configuring-certificates-in-cluster-mode]
== Configuring Certificates in Cluster Mode

The sections xref:administering-system-security.adoc#certificates-and-secure-transport[Certificates and Secure Transport] and xref:administering-system-security.adoc[Administering JSSE Certificates] describe the relevant concepts and use of certificates in Payara Server.
...
You can instead use your own certificates, as described in xref:managing-administrative-security.adoc[Using Your Own Certificates].
...
NOTE: It is recommended that you enable secure admin as described in xref:managing-administrative-security.adoc#managing-administrative-security[Managing Administrative Security] so that Payara Server securely transfers these files on the network.
...
As described in "xref:../high-availability/administering-payara-server-instances.adoc#resynchronizing-payara-server-instances-and-the-das[Resynchronizing Payara Server Instances and the DAS]" in the High Availability section, configuration data for a Payara Server instance is stored in the repository of the DAS and in a cache on the host that is local to the instance.
