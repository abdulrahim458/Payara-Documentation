...
:page-description: Administer system security in Azul Payara Server — configure certificates, keystores, SSL/TLS settings, and harden Payara Server for production environments.
:ordinal: 1

This chapter describes general information about administering system security.

Information on application security is contained in "xref:../../application-development/securing-applications.adoc#securing-applications[Securing Applications]" in the Application Development section.
...
Passwords are your first line of defense against unauthorized access to the components and data of Payara Server. For Information about how to use passwords for Payara Server, see xref:administering-system-security.adoc#administering-passwords[Administering Passwords].
...
For instructions, see xref:administering-system-security.adoc#to-change-an-administration-password[To Change an Administration Password].
...
For instructions, see xref:administering-system-security.adoc#to-set-a-password-from-a-file[To Set a Password From a File].
...
To avoid storing passwords in the domain configuration file in clear text, you can create an alias for a password. This process is also known as encrypting a password. For more information, see xref:administering-system-security.adoc#administering-password-aliases[Administering Password Aliases].
...
.Role Mappings
image:security/securityrolemapping.png[alt=""Shows how users are assigned to groups",how users and groups are assigned to roles, and how applications use groups and roles.",width=600px]
...
To use SSL, Payara Server must have a certificate for each external interface or IP address that accepts secure connections. The HTTPS service of most web servers will not run unless a certificate has been installed. For instructions on applying SSL to HTTP listeners, see "xref:../general-administration/administering-http-connectivity.adoc#configure-an-http-listener-for-ssl[To Configure an HTTP Listener for SSL]" in the Payara Server General Administration section.
...
. Change the master password for the domain by using the xref:../command-reference/change-master-password.adoc[`change-master-password`] subcommand. +
...
If you have already logged into the domain using the xref:../command-reference/login.adoc[`login`] subcommand, you are prompted for the new master password:
...
. Change the admin password by using the xref:../command-reference/change-admin-password.adoc#change-admin-password[`change-admin-password`] subcommand.
...
For a complete list of the types of passwords that can be specified, see the xref:../command-reference/asadmin.adoc[`asadmin`] command help page. Here are a couple of examples of password names used by the file.
...
You can also use the xref:../command-reference/set.adoc[`set`] subcommand to remove and replace the password in the configuration file. For example:
...
. Run the xref:../command-reference/create-password-alias.adoc[`create-password-alias`] subcommand.
. Type the password for the alias when prompted.
. Add the alias to a password file.
+
For example, assume the use of a password file such as `passwords.txt`. Assume further that you want to add an alias for the `AS_ADMIN_USERPASSWORD` entry that is read by the xref:../command-reference/create-file-user.adoc[`create-file-user`] subcommand.
+
...
. Now execute the corresponding command that requires the alias in the password file. Here's an example that uses the xref:../command-reference/create-file-user.adoc[`create-file-user`] subcommand:
+
[source, shell]
----
...
image::password-aliases/password-aliases-unused.png[Password in plain text,width=100%]

In place of the password, you can enter an alias which corresponds to the password in the form: `${ALIAS=<password-alias-name>}`:

image::password-aliases/password-aliases-using.png[Placeholder for Password Alias,width=100%]
...
+
image::password-aliases/password-aliases-new.png[Create new password alias,width=100%]

. On the next page, enter the name of the alias, and the password, twice:
+
image::password-aliases/password-aliases-creation.png[Password alias creation,width=100%]

. Finally, press the *OK* button to create your new alias:
+
image::password-aliases/password-aliases-created.png[Password alias created,width=100%]
...
image::password-aliases/password-aliases-modifying.png[Modifying password alias,width=100%]
...
image::password-aliases/password-aliases-deleting.png[Deleting password alias,width=100%]


[#using-password-alias-microprofile]
=== Microprofile Config Support

Password aliases can also be accessed using MicroProfile Config, as detailed xref:../../microprofile/config/index.adoc[here].
...
+
image:security/jacc-providers.png[alt="Configured JACC Providers",width=100%]
+
...
IMPORTANT: To access this API, you will need to add a dependency to the xref:../../public-api/index.adoc[Payara Public API] first.
