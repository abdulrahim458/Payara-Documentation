...
:page-description: Request Tracing in Payara Micro — configure and use the Request Tracing Service in Payara Micro to identify slow requests and trace distributed operations.

The Request Tracing service works the same on Payara Micro and will trace the same events described in its xref:../../application-development/using-request-tracing-in-applications.adoc#request-tracing[documentation].
...
a| Enables the service when present.
...
a| `requestTracingThresholdUnit`
a| Accepts a String value determining the time unit, defaulting to seconds if no
...
a| `requestTracingThresholdValue`
a| Accepts an integer value to determine the threshold value before a request is traced,
...
a| `enablerequesttracingadaptivesampling`
a| Enables the _Adaptive Sampling_ option when present. When enabled, this allows the sample rate to change to attempt to
...
a| Accepts a String value determining the time unit, defaulting to minutes if no
value is given. The accepted strings are (case-insensitive):
