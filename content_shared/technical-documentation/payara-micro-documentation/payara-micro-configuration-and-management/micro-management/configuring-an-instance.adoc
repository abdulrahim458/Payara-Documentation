...
:page-description: Configure a Payara Micro instance — set command-line options, load a pre-configured domain.xml, apply asadmin post-boot scripts, and tune JVM and network settings.
:ordinal: 2
...
As described in
xref:deploying-applications/deploy-applications.adoc[Deploying From the Command Line],
...
xref:command-line-options/command-line-options.adoc[Payara Micro Command Line Options]
section.
...
An instance can be configured separately, but only when xref:jar-structure-and-configuration/root-directory.adoc[persistent root configuration directory] is specified by means of command line argument `--rootDir`.
...
[source,shell]
----
# Open JDK; launcher needs to be used because of simpler classpath
...
CAUTION: If using a CRaC JDK, the warmup option is instead used to activate creating a checkpoint during the bootstrap of Payara Micro.
...
xref:../../api/payara-micro-api/using-the-payara-micro-api.adoc[Payara Micro Configuration Methods]
section.

As noted in the
xref:deploying-applications/deploy-applications-programmatically.adoc[Deploying an Application Programmatically]
...
xref:jar-structure-and-configuration/adding-jars.adoc[Adding Third Party JARs section]
...
[#micro-ip-settings]
== Payara Micro IPv4 and IPv6

Payara Micro instances automatically configure `java.net.preferIPv4Stack` to true shortly after launching unless
configured otherwise on the command line. This is to match the default Hazelcast configuration of preferring IPv4, configured
via `hazelcast.prefer.ipv4.stack`.

There may be situations where Payara Micro doesn't configure the `java.net.preferIPv4Stack` early enough, in which case it
may be beneficial to define the JVM option on the command line yourself:

[source, shell]
----
java -Djava.net.preferIPv4Stack=true -jar payara-micro.jar
----

If you wish Payara Micro and Hazelcast to prefer IPv6 addresses and an IPv6 stack, you can configure this using:

[source, shell]
----
java -Djava.net.preferIPv4Stack=false -Djava.net.preferIPv6Addresses=true -Dhazelcast.prefer.ipv4.stack=false -jar payara-micro.jar
----

Alternatively to return everything to the Java default (which isn't the Hazelcast default) of preferring an IPv6 stack and IPv4 address,
you can configure Payara Micro and Hazelcast using:

[source, shell]
----
java -Djava.net.preferIPv4Stack=false -Dhazelcast.prefer.ipv4.stack=false -jar payara-micro.jar
----