...
:page-description: Pre-boot and post-boot asadmin scripts in Payara Micro — run asadmin configuration commands automatically before and after Payara Micro starts using script files.
...
. Each _asadmin_ command may contain variable references (e.g. to environment variables or system properties) as described in xref:../../../../payara-server-documentation/general-administration/configuration-variables-reference.adoc#using-variable-references[Using variables]
...
image:payara-micro/uber-jar-command-scripts-structure.png[alt="Uber JAR Command scripts structure",width=100%]
...
By utilizing xref:../command-line-options/command-line-options.adoc[--warmup], these commands can be run as part of preparation of root directory rather than run on every startup. That also spares startup time for productive instance.

When a command uses xref:../../../../payara-server-documentation/general-administration/configuration-variables-reference.adoc#using-variable-references[variable references], the values for variables are determined during execution of the script, before executing the command and changing the configuration. If a variable value should be evaluated each time Payara Micro starts, then executing a script in `--warmup` mode will not suffice and the script needs to be passed as argument also for productive runs.
