...
:page-description: Start a Payara Micro instance — launch Payara Micro from the command line with configuration options, deploy applications, and configure the embedded server at startup.
...
When using xref:../jar-structure-and-configuration/root-directory.adoc[persistent root configuration directory], it is possible to generate a launcher bound to that directory using the command:
...
You need to import two classes from the `fish.payara.micro` package (contained in the Payara Micro JAR, see section xref:../../../maven-support.adoc[Maven Support] for instructions on importing this JAR with **Maven**):
