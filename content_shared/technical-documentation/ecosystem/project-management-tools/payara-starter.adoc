...
:page-description: Payara Starter — web-based Jakarta EE project generator for Azul Payara. Create and download Maven projects configured for Payara Server or Payara Micro.
...
* https://start.payara.fish[Website^]
* https://github.com/payara/ecosystem-starter[GitHub Repository^]
* https://mvnrepository.com/artifact/fish.payara.starter/payara-starter-archetype[Maven Central Repository for Payara Starter Archetype^]
...
* Azul Payara Compatibility: Effortlessly works with Azul Payara, ensuring readiness for deployment.
...
[cols="5,5,12",options="header"]
|===
| Element | Default Value | Notes

a| `build`
a| `maven`
a| Specifies the preferred build system for the project.
...
a| `javaVersion`
a| `21`
a| Sets the Java version for the project.
...
WARNING: Keep in mind that versions older than `11` are not supported in Azul Payara {release-id}

a| `jakartaEEVersion`
a| `10`
a| Specifies the targeted Jakarta EE version.
...
WARNING: For compatibility with Azul Payara {release-id}, versions older than `10` are not supported.
...
a| `platform`
a| `server`
a| Specifies the target distribution where the application will be deployed
...
a| `addConcurrentApi`
a| `false`
a| Includes support for the Concurrent API if `true`.

a| `addResourceApi`
a| `false`
a| Includes support for the Resource API if `true`.

a| `addJBatchApi`
a| `false`
a| Includes support for the JBatch API if `true`.

a| `addMicroprofileApi`
a| `true`
a| Includes support for the MicroProfile API if `true`.

a| `addJcache`
a| `false`
a| Includes support for the JCache if `true`.

a| `addPayaraApi`
a| `true`
a| Includes support for the Payara Public API if `true`.

a| `deployWar`
a| `true`
a| Automatically deploys the WAR file if `true`.

NOTE: Applicable only for Payara Micro deployments.

a| `autoBindHttp`
a| `true`
a| Enables automatic HTTP binding.

NOTE: Applicable only for Payara Micro deployments.

a| `contextRoot`
a| `/`
a| Sets the web context root for the deployed artifact.

NOTE: Applicable only for Payara Micro deployments.

a| `includeTests`
a| `false`
a| Includes a set of generated Unit tests if `true`.
...
a| `auth`
a| `none`
a| Specifies the authentication type for the application.
...
+
image::payara-starter/project-details.png[Project Details,width=600px]
...
+
image::payara-starter/jakartaee.png[Jakarta EE version,width=600px]

. Configure any *Additional Options*:
* The web application allows you to configure additional options such as MicroProfile integration, Azul Payara version, and more.
* Customize these options based on your project needs.
+
image::payara-starter/payara-platform.png[Azul Payara,width=600px]
...
image::payara-starter/er-diagram.png[ER Diagram,width=600px]
...
image::payara-starter/existing-er-diagram.png[Existing ER Diagram,width=600px]
...
image::payara-starter/free-text-support-0.png[Default ER Diagram,width=100%]
...
+
image::payara-starter/free-text-support-1.png[Add a Field,width=100%]
...
+
image::payara-starter/free-text-support-2.png[Change Entity Name,width=100%]
...
+
image::payara-starter/free-text-support-3.png[Modify Field,width=100%]
...
+
image::payara-starter/free-text-support-4.png[Enhance diagram by adding more entities and relationship,width=100%]
...
image::payara-starter/create-er-diagram.png[Create ER Diagram,width=100%]
...
image::payara-starter/update-er-diagram.png[Update ER Diagram,width=100%]
...
image::payara-starter/generate-application.png[Generate Application,width=600px]
image::payara-starter/final-application.png[Final Application,width=600px]
