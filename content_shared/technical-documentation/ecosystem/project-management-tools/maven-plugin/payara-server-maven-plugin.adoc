...
:page-description: Payara Server Maven Plugin — deploy, undeploy, and manage Jakarta EE applications on Payara Server from Maven builds with hot-deploy and administration support.
:ordinal: 2
...
|false
a|If Payara Server is executed in daemon mode, the executor thread will wait for the ready message before shutting down its process. By setting `immediateExit` to `true` you can skip this and instantly interrupt the executor thread.
...
|payara.java.home
|PAYARA_JAVA_HOME
a|Environment Variable `JAVA_HOME` or `JDK_HOME`
...
a|`6.2025.6`
a|By default, start mojo fetches payara-server with version `6.2025.6`.
...
|_none_
a|Endpoint or URL of the AI provider. For example, for `OLLAMA`, the location is `http://localhost:11434`.

|Provider
|payara.ai.provider
|PAYARA_AI_PROVIDER
a|`OPEN_AI`
a|Name of the AI service provider. Supported values:
...
|PAYARA_AI_MODEL
a|`gpt-4o-mini`
a|The AI model to be used (e.g., `gpt-4`, `claude-3-opus`, etc.).
