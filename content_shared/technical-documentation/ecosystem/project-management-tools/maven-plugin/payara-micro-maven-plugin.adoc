...
:page-description: Payara Micro Maven Plugin — start, stop, deploy, and bundle Payara Micro instances from Maven builds for local development, testing, and CI/CD pipelines.
:ordinal: 2
...
a|`autoDeployArtifact`
|payara.auto.deploy.artifact
|PAYARA_AUTO_DEPLOY_ARTIFACT
|true
a|If the extension of the produced artifact is `WAR`, it will be copied automatically to the `MICRO-INF/deploy` directory when this property is set to `true`.

a|`autoDeployContextRoot`
|payara.auto.deploy.context.root
|PAYARA_AUTO_DEPLOY_CONTEXT_ROOT
|_none_
a|Sets context root of the deployed artifact if `autoDeployArtifact` is `true`. `autoDeployContextRoot` has the higher precedence compared to the `autoDeployEmptyContextRoot`.

a|[.breakable]`autoDeployEmptyContextRoot`
|payara.auto.deploy.empty.context.root
|PAYARA_AUTO_DEPLOY_EMPTY_CONTEXT_ROOT
a|`true`
a|Sets context root of the deployed artifact to `/`  if `autoDeployArtifact` is `true`. If set to false, context root is derived from `project.build.finalName`. If `autoDeployContextRoot` is specified, `autoDeployEmptyContextRoot` will be ignored.

a|`startClass`
|payara.start.class
|PAYARA_START_CLASS
|(optional):
a|Replaces the `Start-Class` definition that resides in the `MANIFEST.MF` file with the provided class.

a|`appendSystemProperties`
|payara.append.system.properties
|PAYARA_APPEND_SYSTEM_PROPERTIES
a|`true`
a|Appends all system properties defined into the `payara-boot.properties` file.

a|`payaraMicroVersion`
|payara.micro.version
|PAYARA_MICRO_VERSION
a|`{release-id}`
a|By default, bundle mojo fetches Payara Micro with version {release-id}. Note: `payaraVersion` is deprecated and has been replaced by `payaraMicroVersion` to stay in sync with the Payara Server Maven Plugin.
...
a|`uberJarClassifier`
|payara.uber.jar.classifier
|PAYARA_UBER_JAR_CLASSIFIER
a|`microbundle`
...
<plugin>
    ...
    <configuration>
        ...
        <payaraVersion>{release-id}</payaraVersion>
        <artifactItem>
...
|PAYARA_USE_UBER_JAR
|false
a|Use the created uber-jar that resides in the target folder. The name of the JAR artifact will be resolved automatically by evaluating its final name, artifact ID and version. This configuration element has the higher precedence compared to `payaraMicroAbsolutePath`, `payaraVersion` and `artifactItem`.
...
|PAYARA_MICRO_PATH
|_none_
a|Absolute path to the Payara Micro executable. Note: Parameter `payaraMicroAbsolutePath` is deprecated and has been replaced by `payaraMicroPath` to stay in sync with the Payara Server Maven Plugin.
...
|false
a|If Payara Micro is executed in daemon mode, the executor thread will wait for the ready message before shutting down its process. By setting `immediateExit` to `true` you can skip this and instantly interrupt the executor thread.

a|`javaHome`
|payara.java.home
|PAYARA_JAVA_HOME
a|`java`
a|Absolute path to the java executable. This has higher priority to the java executable identified via the Maven toolchain. Note: Parameter `javaPath` is deprecated and has been replaced by `javaHome` to stay in sync with the Payara Server Maven Plugin.

a|`deployWar`
|payara.deploy.war
|PAYARA_DEPLOY_WAR
|false
a|If the attached project is of type WAR, it will automatically be deployed if `deployWar` is set to `true`.
...
a|`javaCommandLineOptions`
|
|
|_none_
a|Defines a list of command line options that will be passed to `java` executable. Command line options can either be defined as key-value pairs or just as list of values. key-value pairs will be formatted as `key=value`.
...
a|`maxStopTimeoutMillis`
|payara.max.stop.timeout.millis
|PAYARA_MAX_STOP_TIMEOUT_MILLIS
|5000
...
a|`sourcesChanged`
|payara.sources.changed
|PAYARA_SOURCES_CHANGED
|_none_
...
a|`metadataChanged`
|payara.metadata.changed
|PAYARA_METADATA_CHANGED
|false
