...
|`{version}`
|By default, bundle mojo fetches Payara Micro with version {version}. Note: `payaraVersion` is deprecated and has been replaced by `payaraMicroVersion` to stay in sync with the Payara Server Maven Plugin.
...
<plugin>
    ...
    <configuration>
        ...
        <payaraVersion>{version}</payaraVersion>
        <artifactItem>
