...
:page-description: Payara Platform Maven BOM — Bill of Materials artifact for consistent dependency version management across Payara Server and Payara Micro Jakarta EE projects.
:ordinal: 1

Azul Payara consists of many artifacts and it depends on lots of APIs as well as their implementations. Having matching versions of these dependencies will prevent classloading issues at runtime. To this effect, Azul Payara's Bill Of Materials (BOM) artifact collects all of these versions for the following categories:

* Azul Payara Distribution artifacts
* Azul Payara Client libraries
...
----
a| Also available are `payara-ml`, `payara-web`, `payara-web-ml`
...
----
a| Also available `payara-embedded-web`
...
----
a|Also available `jakarta.jakartaee-web-api`, and all individual API artifacts.
...
----
a| Also available are `org.cmpn`, `osgi.annotation`, `org.osgi.dto`, and `osgi.enterprise`
...
Arquillian is the recommended way to run integration tests written for Payara Server. Azul Payara provides multiple connectors fitting the deployment scenarios. These artifacts are declared in scope `test`.
...
----
a| The only dependency that is `provided` scoped, as it is an annotation processor.
...
----
a| The `hazelcast-kubernetes` dependency is also available.
