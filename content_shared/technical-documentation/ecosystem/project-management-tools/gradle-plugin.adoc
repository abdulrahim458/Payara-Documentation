...
:page-description: Payara Micro Gradle Plugin — build, run, and bundle Payara Micro instances from Gradle. Supports hot deploy, exploded WAR mode, and artifact bundling for Jakarta EE apps.
:ordinal: 4
...
a|`payaraVersion`
|{release-id}
a|By default `microBundle` task fetches payara-micro with version {release-id}
...
a|`useUberJar`
|false
a|Use created uber-jar that resides in target folder. The name of the JAR artifact will be resolved automatically by evaluating its final name, artifact ID and version. This configuration element has the higher precedence compared to `payaraMicroAbsolutePath` and `payaraVersion`.

a|`payaraMicroAbsolutePath`
|_none_
|Absolute path to payara-micro executable.

a|`payaraVersion`
|{release-id}
a|By default `microStart` task fetches Payara Micro with version {release-id}.
...
a|`deployWar`
|false
a|If the attached project is of type WAR, it will automatically be deployed to Payara Micro if `deployWar` is set to `true`.
...
a|`sourcesChanged`
|_none_
a|Defines a list(comma separated) of the absolute paths to the source file which need
