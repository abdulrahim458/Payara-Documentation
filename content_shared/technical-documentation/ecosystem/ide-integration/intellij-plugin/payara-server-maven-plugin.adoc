...
:page-description: Payara Server Maven Plugin in IntelliJ IDEA — configure and run Payara Server Maven Plugin goals directly from the IntelliJ IDEA Maven integration.
:ordinal: 1
...
image::intellij-plugin/payara-server-maven-plugin/create-configuration.png[Create Configuration,width=100%]

. In the *Run/Debug Configurations* dialog, click *+*, select the *Payara Server*:
image::intellij-plugin/payara-server-maven-plugin/create-server-configuration.png[Create Server configuration,width=100%]
...
image::intellij-plugin/payara-server-maven-plugin/app-configuration.png[App configuration,width=100%]
...
image::intellij-plugin/payara-server-maven-plugin/payara-configuration.png[Payara configuration,width=100%]

. Configure environment variables and system properties required by the server and Maven plugin.
image::intellij-plugin/payara-server-maven-plugin/env-server-properties.png[Env and Server properties,width=100%]

. Define additional Maven properties used by the Payara Server Maven Plugin during the build and deployment process.
image::intellij-plugin/payara-server-maven-plugin/maven-properties.png[Maven properties,width=100%]

. Specify Maven tasks to run before launching the server.
image::intellij-plugin/payara-server-maven-plugin/before-launch-task.png[Before Launch Task,width=100%]
...
image::intellij-plugin/payara-server-maven-plugin/run-server.png[Start the Payara Server,width=100%]

. You can see the server log in the *application build console* window.
image::intellij-plugin/payara-server-maven-plugin/server-log.png[Payara Server log,width=100%]
