...
:page-description: Build the Payara VS Code extension from source — steps for cloning, building, and installing the Payara Tools VS Code extension for local development or contribution.
...
* *Clone the ecosystem-vscode-plugin repository*: You must first clone the repository where the extension source code is stored. https://github.com/payara/ecosystem-vscode-plugin[VS Code Extension repository^]
* *Install Yarn*: Yarn is the build tool for the Payara Tools VS Code IDE Plugin and must be configured locally. https://classic.yarnpkg.com/lang/en/docs/install/[Install Yarn^]
* *Install vsce*: vsce is the tool to package VS Code extensions. https://www.npmjs.com/package/vsce[Install vsce^]
...
image::vscode-extension/building-extension/install-vsix.png[Install VSIX File,width=100%]
...
image::vscode-extension/building-extension/uninstall-extension.png[Uninstall VS Code Extension,width=600px]