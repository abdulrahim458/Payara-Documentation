...
:page-description: Payara Server tools in Eclipse IDE — configure a Payara Server runtime, deploy Jakarta EE applications, manage domains, and use hot deploy from the Eclipse IDE.
:ordinal: 1
...
+
image::eclipse-plugin/payara-server/new-server-menu.png[New Server menu,width=600px]

. Select *Payara -> Payara* and set the *Host name* and some comprehensible *Server name*.
+
image::eclipse-plugin/payara-server/new-server-window.png[New Server window,width=600px]

. Then click *Next* to another window with server runtime configuration.
+
image::eclipse-plugin/payara-server/new-server-runtime.png[New Server Runtime,width=600px]
...
+
image::eclipse-plugin/payara-server/new-server-instance.png[New Server Instance,width=600px]
...
+
image::eclipse-plugin/payara-server/new-server-resources.png[New Server Resources,width=600px]
...
+
image::eclipse-plugin/payara-server/add-resource-menu.png[Add and Remove menu,width=100%]
. Select resources you want to be managed by the *Payara Tools*, add them to the right panel and click *Finish*.
+
image::eclipse-plugin/payara-server/add-resource-window.png[Add and Remove window,width=100%]
. Now you can watch the deployment of the application in the *Console* view.
+
image::eclipse-plugin/payara-server/add-resource-console.png[Add and Remove window,width=100%]
...
+
image::eclipse-plugin/payara-server/add-resource-browser.png[Request and Response in Browser,width=100%]
...
+
image::eclipse-plugin/payara-server/add-resource-admin-1.png[Admin Console,width=100%]
+
image::eclipse-plugin/payara-server/add-resource-admin-2.png[Admin Console,width=100%]
...
+
image::eclipse-plugin/payara-server/reconfig-overview.png[Server Overview,width=100%]
...
+
image::eclipse-plugin/payara-server/reconfig-runtime.png[Runtime Configuration,width=600px]
...
[source, shell]
----
docker run -p 8080:8080 -p 4848:4848 -p 9009:9009 -v HOST_PATH:CONTAINER_PATH  payara/server-full:{release-id}
----
+
...
image::hot-auto-deploy/eclipse-payara-auto-deploy.png[Auto Publish applications,width=600px]
...
image::hot-auto-deploy/eclipse-payara-hot-deploy.png[Payara Server Hot Deploy,width=600px]
