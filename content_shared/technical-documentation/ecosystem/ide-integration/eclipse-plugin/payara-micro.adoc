...
:page-description: Payara Micro tools in Eclipse IDE — create, run, and debug Payara Micro projects from Eclipse with hot deploy and MicroProfile project support.
:ordinal: 2
...
+
image::eclipse-plugin/payara-micro/create-new-project.png[Generate Payara Micro Application,width=100%]

. Now, select the Payara Micro Project type to create the application:
+
image::eclipse-plugin/payara-micro/payara-micro-project.png[Payara Micro Project,width=600px]

. Select the destination folder, where the project will be generated. You may also use the default workspace location:
+
image::eclipse-plugin/payara-micro/project-location.png[Project location,width=600px]

. Enter the groupId, artifactId, etc to set up the project, and then click *Next*:
+
image::eclipse-plugin/payara-micro/project-gav-details.png[Enter GAV,width=600px]
...
+
image::eclipse-plugin/payara-micro/micro-settings.png[Micro settings,width=600px]
...
+
image::eclipse-plugin/payara-micro/project-pom.png[Micro project generated,width=100%]
...
+
image::eclipse-plugin/payara-micro/micro-run-config-1.png[Micro run configuration,width=100%]
...
+
image::eclipse-plugin/payara-micro/micro-run-config-2.png[New run configuration,width=100%]

. Associate the workspace project with the run configuration:
+
image::eclipse-plugin/payara-micro/micro-run-config-3.png[Associate project,width=100%]

. Click the browse button and select the project from the *Project Selection* dialog:
+
image::eclipse-plugin/payara-micro/micro-run-config-4.png[Select the project,width=600px]

. Enter the configuration name and choose the *Build Artifact* type
+
image::eclipse-plugin/payara-micro/micro-run-config-5.png[Add the Run and Debug configuration,width=600px]
+

. Add the configuration to *Run and Debug* menu and press the *Apply* button to save the configuration:
+
image::eclipse-plugin/payara-micro/micro-run-config-6.png[Add the Run and Debug configuration,width=100%]

. To start the Payara Micro instance, run the *Payara Micro Sample* configuration from the toolbar or main menu:
+
image::eclipse-plugin/payara-micro/micro-run-config-7.png[Invoke run configuration,width=100%]

. After running the Micro application, you may see the log in the OUTPUT window:
+
image::eclipse-plugin/payara-micro/micro-run-1.png[Payara Micro log,width=100%]
+
image::eclipse-plugin/payara-micro/micro-run-2.png[Payara Micro log,width=100%]
...
image::hot-auto-deploy/eclipse-micro-hot-auto-deploy.png[Configuration name,width=100%]
