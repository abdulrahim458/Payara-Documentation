...
:page-description: Payara Server Managed Arquillian container adapter — run Jakarta EE integration tests with Arquillian by controlling a full Payara Server instance lifecycle.
...
The Payara Server Managed Arquillian container adapter can be found on https://central.sonatype.com/artifact/fish.payara.arquillian/arquillian-payara-server-managed[Maven Central^], and can be included in your project using the following Maven GAV coordinates:
...
a| `adminHttps`
a| You can use it to specify whether the HTTP or HTTPS protocol shall be used to access the DAS. The property value can be `true` or `false`.
a| `false`
...
a| `adminUser`
a| The name of the admin user of your DAS.
...
a| `adminPassword`
a| The password of the admin user of your DAS.
...
a| `allowConnectingToRunningServer`
a| Allow Arquillian to use an already running Payara Server instance and skips `start-domain` command invocation.
a| `false`

a| `authorisation`
a| If true, basic access authentication is enabled.
...
a| `ignoreCertificates`
a| If set to `true`, SSL certificate correctness is ignored.
...
a| `libraries`
a| A comma-separated list of library JAR files. Specify the library JAR files by their relative or absolute paths.
...
a| Value of the `payara.home` system property.

a| `properties`
a| Optional keyword-value pair that specify additional properties for the deployment.
...
a| `type`
a| If set to `osgi`, the component is packaged as an OSGi bundle.
...
IMPORTANT: glassfish-web.xml and sun-web.xml are deprecated and support will be removed in a future release.
