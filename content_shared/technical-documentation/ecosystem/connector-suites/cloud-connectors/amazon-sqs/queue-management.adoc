...
:page-description: Programmatic SQS queue management in Azul Payara — create, delete, and configure Amazon SQS queues from Jakarta EE applications using the Payara SQS Cloud Connector API.
...
To simplify the setup needed to use the SQS API from a standard Jakarta EE application and manage SQS resources, the Azul Payara SQS connector offers easy access to the `com.amazonaws.services.sqs.AmazonSQSConnectionFactory` class through the Java Connector Architecture (JCA) API.
...
For more detailed information on the Amazon SQS Java SDK, including additional features and advanced usage, refer to the https://sdk.amazonaws.com/java/api/latest/software/amazon/awssdk/services/sqs/SqsClient.html[official Javadocs^].
