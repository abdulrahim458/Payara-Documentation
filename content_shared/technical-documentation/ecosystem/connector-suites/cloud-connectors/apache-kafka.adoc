...
:page-description: Apache Kafka Cloud Connector for Azul Payara — JCA resource adapter for producing and consuming Kafka messages from Payara Server and Payara Micro Jakarta EE applications.
...
In order to connect to Apache Kafka, the `kafka-rar-0.8.0.rar` has to be deployed as shown in the xref:../../../../index.adoc#installing-a-connector[Installing a connector section] of the Cloud Connectors overview.
...
a|`valueSerializer`
|String
a|[.breakable]`org.apache.kafka.common .serialization.StringSerializer`
...
a|`connectionsMaxIdle`
|Long
|540000
|Close Idle Kafka Connections

a|`maxInflightConnections`
|Integer
|5
