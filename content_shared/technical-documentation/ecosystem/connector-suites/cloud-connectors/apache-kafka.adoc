...
In order to connect to Apache Kafka, the `kafka-rar-0.8.0.rar` has to be deployed as shown in the xref:technical-documentation/ecosystem/connector-suites/cloud-connectors/index.adoc#installing-a-connector[Installing a connector section] of the Cloud Connectors overview.
