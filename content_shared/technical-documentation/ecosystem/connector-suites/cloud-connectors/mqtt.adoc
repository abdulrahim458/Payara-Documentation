...
:page-description: MQTT Cloud Connector for Azul Payara — JCA resource adapter for publishing and subscribing to MQTT topics from Payara Server and Payara Micro applications.
...
In order to connect to a MQTT broker, the `mqtt-rar-0.8.0.rar` has to be deployed as shown in the xref:../../../../index.adoc#installing-a-connector[Installing a connector section] of the Cloud Connectors overview.
...
|Boolean
|true
a|Sets whether the client will automatically reconnect to the server if the
connection is lost
