...
:page-description: Azure Service Bus Cloud Connector for Azul Payara — JCA resource adapter for sending and receiving messages via Azure Service Bus queues and topics in Jakarta EE apps.
...
be deployed as shown in the xref:../../../../index.adoc#installing-a-connector[Installing a connector section] of the Cloud Connectors overview.
...
|String
|none
a|The SAS Key. Environment variable replacement can be used so that this is not
exposed in the code
...
|Integer
|1
a|The Initial Poll Delay (in seconds) before the Adapter starts polling for
messages after deployment (MDB Property Only)

|pollInterval
|Integer
|1
a|The Poll interval (in seconds). This is how often the MDB polls for new
messages (MDB Property Only)

|pollTimeout
|Integer
|1
a|The Poll Timeout (in seconds). This is how long the MDB should wait for
