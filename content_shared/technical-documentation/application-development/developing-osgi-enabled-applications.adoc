...
:page-description: Develop OSGi-enabled Jakarta EE applications on Azul Payara — combine OSGi modularity with Jakarta EE component model using Payara's integrated OSGi container.
:ordinal: 15
...
a|`jakarta.jms.Queue`
a|`jndi-name`
a|`jndi-name` is set to the JNDI name of the queue

|JMS Topic destination
a|`jakarta.jms.Topic`
a|`jndi-name`
a|`jndi-name` is set to the JNDI name of the topic
...
For instruction on deploying OSGi bundle, see "xref:../payara-server-documentation/application-deployment/deploying-applications.adoc#osgi-bundle-deployment-guidelines[OSGi Bundle Deployment Guidelines]" in the Payara Server Application Deployment section.
