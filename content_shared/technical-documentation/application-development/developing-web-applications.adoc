...
:page-description: Develop web applications in Azul Payara — build Jakarta Servlet, Jakarta Faces, and Jakarta REST applications with Payara-specific configuration and extensions.
:ordinal: 7

This chapter describes how web applications are supported in Azul Payara.
...
This section describes how to create effective servlets to control application interactions running on Payara Server or Micro, including standard-based servlets. In addition, this section describes additional features introduced in Azul Payara.
...
Azul Payara has the following web application response caching capabilities:
...
* Results produced by resources that are the target of a `RequestDispatcher.include()` or `RequestDispatcher.forward()` call are cached if caching has been enabled for those resources. For details, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#cache-mapping[cache-mapping]" and "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#dispatcher[dispatcher] in the Payara Server Application Deployment section.
...
For more information about the `glassfish-web.xml`/`payara-web.xml` caching settings, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#cache[cache]" in the Payara Server Application Deployment section.
...
For more information, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#default-helper[default-helper]" in the Payara Server Application Deployment section.
...
a|`timeout`
a|`60s`
a|(optional) The time in seconds after which the body of the tag is executed and the cache is refreshed. By default, this value is interpreted in seconds.
...
a|`nocache`
a|`false`
a|(optional) If set to `true`, the body content is executed and served as if there were no `cache` tag. This offers a way to programmatically decide whether the cached response is sent or whether the body has to be executed, though the response is not cached.

a|`refresh`
a|`false`
a|(optional) If set to `true`, the body content is executed and the response is cached again. This lets you programmatically refresh the cache immediately regardless of the `timeout` setting.

a|`scope`
a|`application`
a|(optional) The scope of the cache. Can be `request`, `session`, or `application`.
...
a|`key`
a|`ServletPath_Suffix`
a|(optional) The name used by the container to access the cached entry. The cache key is suffixed to the servlet path to generate a key to access the cached entry.
...
To configure whether and how HTTP sessions use cookies and URL rewriting, edit the `session-properties` and `cookie-properties elements in the `glassfish-web.xml` or `payara-web.xml` file for an individual web application. For more details about the properties you can configure, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#session-properties[session-properties]" "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#cookie-properties[cookie-properties]" and in the Payara Server Application Deployment section.
...
For more information about class loaders, see xref:class-loaders.adoc[Class Loaders].
...
a|Co-located or distributed stateless session, stateful session, or entity
bean references
|Yes

a|JNDI context
a|Yes, for `InitialContext` and `java:comp/env` namespace references

a|`UserTransaction`
a|Yes, but if the instance that fails is never
...
Azul Payara offers these session management options, determined by the `session-manager` element's `persistence-type` attribute in the `glassfish-web.xml`/`payara-web.xml` file:
...
For more information, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#session-manager[session-manager]" in the Payara Server Application Deployment section.
...
`sessionFilename`, which is listed under "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#manager-properties[manager-properties]" in the Payara Server Application Deployment section.
...
NOTE: Make sure the `delete` option is set in the `server.policy` file, or expired file-based sessions might not be deleted properly. For more information about `server.policy`, see xref:securing-applications.adoc#the-server.policy-file[The`server.policy` File].
...
The `file` persistence type supports all the manager properties listed under "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#manager-properties[manager-properties]" in the Payara Server Application Deployment section except `sessionFilename`, and supports the `directory` store property listed under "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#store-properties[store-properties]" in the same document.
...
Azul Payara includes the Grizzly HTTP Engine, which enables asynchronous request processing (ARP) by avoiding blocking connections. Grizzly's ARP implementation accomplishes this by using the Java NIO API.
...
For details about the `parameter-encoding` element, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#parameter-encoding[parameter-encoding]" in the Payara Server Application Deployment section.
...
----
+
For more details and a complete list of virtual server properties, see xref:../payara-server-documentation/command-reference/create-virtual-server.adoc[`create-virtual-server`].
...
IMPORTANT: *Catalina* is the internal implementation of the Servlet implementation used in Azul Payara, originated from Apache Tomcat's own implementation. +
...
NOTE: Not all context definitions are supported by Azul Payara.
...
Enable the security manager as described in xref:securing-applications.adoc#enabling-and-disabling-the-security-manager[Enabling and Disabling the Security Manager].
...
a|`debug`
a|`int`
a|`0` (for no debugging)
|Specifies the debugging level.

a|`expires`
a|`Long`
a|`Expires` header in HTTP response not set
...
a|`isVirtualWebappRelative`
a|`boolean`
a|`false` (relative to the given SSI file)
a|Specifies whether the virtual path of the `#include` directive is relative to the `content-root`.
...
a|`parameterEncoding`
a|`String`
a|`System.getProperty("file.encoding",  "UTF-8")`
|Specifies the parameter's encoding.

a|`passShellEnvironment`
a|`boolean`
a|`false`
...
For more information about this element, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#payara-web-app[payara-web-app]" in the Payara Server Application Deployment section.
