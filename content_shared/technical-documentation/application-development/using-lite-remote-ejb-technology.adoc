...
:page-description: Use Lite Remote EJB in Azul Payara — invoke EJBs from remote clients using the Payara Lite Remote EJB implementation over HTTP without IIOP.
:ordinal: 10
...
a|[.breakable]`fish.payara.connectTimeout`
...
a| `Number` (from which it's `Long` value is taken) or a `String` that can be converted to a `Long` value.
a|[.breakable]`fish.payara.readTimeout`

a| The timeout to read a response. If the remote Payara doesn't respond within the defined time a ProcessingException is thrown with a `TimeoutException` as its cause. A value of *0* represents that the wait is indefinite. Negative values are not allowed. Unit is microseconds.
a| `Number` (from which it's `Long` value is taken) or a `String` that can be converted to a `Long` value.

a|[.breakable]`fish.payara.keyStore`
...
a| Instance of `java.security.KeyStore` or a `String` representing its fully qualified classname.
a|[.breakable]`fish.payara.trustStore`
...
a|[.breakable]`fish.payara.sslContext`
a| The SSL context that will be used by the proxy for creating secured connections to the Payara remote server. This context *must* be fully initialized, including the trust and key managers. Should not be used in conjunction with the `fish.payara.keyStore` and/or `fish.payara.trustStore` properties.

a| Instance of `javax.net.ssl.SSLContext` or a `String` representing its fully qualified classname.
a|[.breakable]`fish.payara.hostnameVerifier`
...
a| Instance of a `javax.net.ssl.HostnameVerifier` or a `String` representing its fully qualified classname.

a|[.breakable]`fish.payara.provider.principal`
| The principal to be used by the proxy to access the secured endpoint.

a| Instance of a `String`.
a|[.breakable]`fish.payara.provider.credentials`

| The credentials to be used by the proxy to access the secured endpoint.
a| Instance of a `String`.

a|[.breakable]`fish.payara.provider.authType`
a| To specify the authentication type of the secured endpoint. By default `BASIC` is defined.

a| Instance of a `String`.
a|[.breakable]`fish.payara.requestFilter`

| To register the custom filter for the client request invoked by proxy.
a| Instance of a `jakarta.ws.rs.client.ClientRequestFilter` or a `Class` implementing `jakarta.ws.rs.client.ClientRequestFilter`.

a|[.breakable]`fish.payara.responseFilter`
| To register the custom filter for the client response received by proxy.

a| Instance of a `jakarta.ws.rs.client.ClientResponseFilter ` or a `Class` implementing `jakarta.ws.rs.client.ClientResponseFilter`.
a|[.breakable]`fish.payara.executorService`

| The executor service that will be used for executing asynchronous tasks. _(for future use)_
a| Instance of `java.util.concurrent.ExecutorService` or a `String` representing its fully qualified classname.

a|[.breakable]`fish.payara.scheduledExecutorService`
| The executor service that will be used for executing scheduled asynchronous tasks. _(For future use)_

a| Instance of `java.util.concurrent.ScheduledExecutorService` or string representing fully qualified classname.
a|[.breakable]`fish.payara.withConfig`

| The configuration for the internal JAX-RS/Jersey REST client.
a| Instance of `jakarta.ws.rs.core.Configuration` or a `String` representing its fully qualified classname.

a|[.breakable]`fish.payara.clientAdapter`
| Implementation of client side adapter to use for intercepting JNDI lookups (see below)

a| Instance of `fish.payara.ejb.http.client.adapter.ClientAdapter`

|===

The constants are also exposed as static attributes of the `fish.payara.ejb.http.client.RemoteEJBContextFactory` class.
...
* xref:../payara-server-documentation/command-reference/get-ejb-invoker-configuration.adoc#get-ejb-invoker-configuration[`get-ejb-invoker-configuration`]
* xref:../payara-server-documentation/command-reference/set-ejb-invoker-configuration.adoc#set-ejb-invoker-configuration[`set-ejb-invoker-configuration`]
