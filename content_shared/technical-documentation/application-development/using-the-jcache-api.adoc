...
:page-description: Use the JCache API in Azul Payara applications — store and retrieve data from the Payara Domain Data Grid using the JCache (JSR 107) API and CDI injection.
:ordinal: 22

This chapter covers how to use the JCache functionality in Azul Payara.
...
Its inclusion in Azul Payara is a product choice to leverage the capabilities of a caching API as the Jakarta Platform lacks an official API.
...
IMPORTANT: This annotation is part of the xref:../public-api/index.adoc[Azul Payara Public API]
...
Azul Payara supports cache definitions and operations on caches by using the standard set of annotations that are part of the JCache API:
