...
:page-description: Use the Transaction Service in Azul Payara — manage Jakarta Transactions (JTA), configure XA data sources, and handle distributed transactions in Payara applications.
:ordinal: 17

The Jakarta EE platform provides several abstractions that simplify development of dependable transaction processing for applications. This chapter discusses Jakarta EE transactions and transaction support in Azul Payara.

For more information about the Jakarta Transactions API (JTA) and Java Transaction Service (JTS), see "xref:../payara-server-documentation/general-administration/administering-transactions.adoc#administering-transactions[Administering Transactions]" in Payara Server General Administration section and in the https://jakarta.ee/specifications/transactions[Jakarta Transactions specification^].
...
|===
|Transaction Isolation Level a|`getTransactionIsolation` Return Value |Description
...
You can set the default transaction isolation level for a JDBC connection pool. For details, see "xref:../payara-server-documentation/general-administration/administering-database-connectivity.adoc#to-create-a-jdbc-connection-pool[To Create a JDBC Connection Pool]" in the Payara Server General Administration section.
...
* Use Azul Payara's `DataSource` implementation, which provides a `getNonTxConnection` method. This method retrieves a JDBC connection that is not in the scope of any transaction. There are two variants:
+
[source,java]
----
...
+
IMPORTANT: Commit option A is not supported in Azul Payara's EJB implementation.
...
+
NOTE: This is the default commit option set in Azul Payara.
+
...
The `payara-ejb-jar.xml` deployment descriptor has an element, `commit-option`, that specifies the commit option to be used. Based on the specified commit option, the appropriate handler is instantiated.
...
To override this timeout for an individual bean, use the optional `cmt-timeout-in-seconds` element in `payara-ejb-jar.xml`. The default value, `0`, specifies that the Transaction Service timeout is used.
...
The default setting is `false`. For more information about non-component callers, see xref:using-the-jdbc-api-for-database-access.adoc#allowing-non-component-callers[Allowing Non-Component Callers].
...
If accessing the `jakarta.transaction.TransactionManager` implementation is absolutely necessary, you can look up Azul Payara's implementation of this interface using the JNDI name `java:appserver/TransactionManager`.
