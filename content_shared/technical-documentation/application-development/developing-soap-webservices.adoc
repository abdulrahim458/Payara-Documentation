...
:page-description: Develop SOAP web services in Azul Payara — implement Jakarta XML Web Services (JAXWS) services and clients, configure WSDL endpoints, and apply WS-Security.
:ordinal: 5
...
For information about web services security, see xref:securing-applications.adoc#configuring-message-security-for-web-services[Configuring Message Security for Web Services].
...
The Payara Server deployment descriptor files `glassfish-web.xml`/`payara-web.xml` and `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` provide optional web service enhancements in the `webservice-endpoint` and `webservice-description` elements, including a `debugging-enabled` sub-element that enables the creation of a test page.
...
The context-root is defined in the `application.xml` or `web.xml` file, and can be overridden in the `glassfish-application.xml`/`payara-application.xml` or
...
`debugging-enabled` element in the `glassfish-web.xml`/`payara-web.xml` and `glassfish-ejb-jar.xml`/payara-ejb-jar.xml` deployment descriptor to `false`.
