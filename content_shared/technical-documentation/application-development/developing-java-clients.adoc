...
:page-description: Develop Java clients for Azul Payara — create standalone Java applications accessing EJBs, JMS, and other Jakarta EE resources from outside the Payara container.
:ordinal: 12
...
JMS resources are supported only in the Full Profile, not in the Web Profile. See xref:using-jakarta-messaging.adoc#using-jakarta-messaging[Using the Java Message Service].
...
For information about debugging application clients, see xref:debugging-applications.adoc#application-client-debugging[Application Client Debugging].

IMPORTANT: Interoperability between application clients and Payara Servers running under different Azul Payara major versions is not supported. The same can be said if both server and client are running under different major Java versions.
...
The ACC integrates with Payara Server's authentication system. It also supports SSL (Secure Socket Layer)/IIOP if configured and when necessary; see xref:developing-java-clients.adoc#using-rmiiiop-over-ssl[Using RMI/IIOP Over SSL].
...
Application clients can use a programmatic login mechanism by xref:securing-applications.adoc#programmatic-login-using-the-programmaticlogin-class[using the `ProgrammaticLogin` Class].
...
Annotation is supported for the main class and the optional callback handler class in application clients. For more information, see "xref:../payara-server-documentation/application-deployment/index.adoc#deployment-descriptors-and-annotations[Deployment Descriptors and Annotations]" in the Payara Server Application Deployment section.
...
For more information about naming and lookups, see xref:using-the-java-naming-and-directory-interface.adoc#accessing-the-naming-context[Accessing the Naming Context]. +
...
----
. Define the `@EJB` annotations or the `ejb-ref` elements in the `application-client.xml` file. Define the corresponding `ejb-ref` elements in the`payara-application-client.xml` file. +
For more information on the `payara-application-client.xml` file, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-application-client.xml-file[The payara-application-client.xml file]" in the Payara Server Application Deployment section. For a general explanation of how to map JNDI names using reference elements, see xref:using-the-java-naming-and-directory-interface.adoc#mapping-references[Mapping References].
...
* `payara-application-client.xml` - (optional) Payara Server specific client deployment descriptor. For information on the
`payara-application-client.xml` file, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-application-client.xml-file[The payara-application-client.xml file]" in the Payara Server Application Deployment section.
...
* Define the `target-server` element's `address` and `port` attributes to reference the remote server machine and its ORB port. See "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#target-server[target-server]" in the Payara Server Application Deployment section.
...
+
See "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#client-container[client-container]" in the Payara Server Application Deployment section.
+
...
* For more information about RMI-IIOP load balancing and failover, see "xref:../payara-server-documentation/high-availability/rmi-iiop-load-balancing-and-failover.adoc#rmi-iiop-load-balancing-and-failover[RMI-IIOP Load Balancing and Failover]" in the Payara Server High Availability section.
...
+
For information on configuring JMS resources, see "xref:../payara-server-documentation/general-administration/administering-the-java-message-service.adoc#administering-jms-connection-factories-and-destinations[Administering JMS Connection Factories and Destinations]" in the Payara Server General Administration section.

. Define the `@Resource` or `@Resources` annotations or the `resource-ref` elements in the `application-client.xml` file. Define the corresponding `resource-ref` elements in the `payara-application-client.xml` file. +
For more information on the `payara-application-client.xml` file, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-application-client.xml-file[The payara-application-client.xml file]" in the Payara Server Application Deployment section. For a general explanation of how to map JNDI names using reference elements, see xref:using-the-java-naming-and-directory-interface.adoc#mapping-references[Mapping References].

. Ensure that the client JAR file includes the following files:
* A Java class to access the resource.
* `application-client.xml` - (optional) Jakarta EE application client deployment descriptor.
* `payara-application-client.xml` - (optional) Payara Server specific client deployment descriptor. For information on the `payara-application-client.xml` file, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-application-client.xml-file[The payara-application-client.xml file]" in the Payara Server Application Deployment section.
* The `MANIFEST.MF` file. This file contains a reference to the `main` class, which states the complete package prefix and class name of the Java client.
.  Prepare the client machine.
+
...
The application developer or deployer can specify that Java Web Start is always disabled for an application client by setting the value of the `eligible` element to `false` in the `payara-application-client.xml` file.
...
For details about naming, see "xref:../payara-server-documentation/application-deployment/index.adoc#naming-standards[Naming Standards]" in the Payara Server Application Deployment section.

To set a different URL for an application client, use the `context-root` sub-element of the `java-web-start-access` element in the `payara-application-client.xml` file. This overrides the `appclient-module-id` or `application-id`.
...
+
For more information about importing a trusted certificate into the domain keystore, see "xref:../payara-server-documentation/security-guide/administering-system-security.adoc#administering-jsse-certificates[Administering JSSE Certificates]" in the Payara Server Security Guide.
...
To specify a vendor-specific icon, splash screen, text string, or a combination of these for Java Web Start download and launch screens, use the `vendor`element in the `payara-application-client.xml` file. The complete format of this element's data is as follows:
...
To specify a custom JNLP file for Java Web Start, use the `jnlp-doc` element in the `payara-application-client.xml` file. If `none` is specified, a default JNLP file is generated.
...
For more information about the `payara-application-client.xml` file, see the Payara Server Application Deployment section.
...
Note that these formats are not equivalent to the format of the `jnlp-doc` element in the `payara-application-client.xml` file.
...
----
a|Payara Server sets the `main-class` and the arguments to be passed to the client.
...
----
a|You can specify one or more of the `os`, `arch`, `platform`, and `locale` attributes for the `<information>` element. You can also specify child elements; Payara Server provides no default children.
...
----
a|You can specify one or more of the `os`, `arch`, `platform`, and `locale` attributes for the `<resources>` element. You can also specify child elements; Payara Server provides no default children.
...
----
a|Adds native libraries to be included in the application. Each entry in a JAR listed in a `<nativelib>` element must be a native library for the correct platform. The full syntax of the `<nativelib>` element lets the developer specify the platform for that native library.
...
To use a username and password, configure the `ior-security-config` element in the `payara-ejb-jar.xml` file. The following configuration establishes SSL between an application client and an EJB component using a username and password.

The user has to log in to the ACC using either the `sun-acc.xml` mechanism or the xref:securing-applications.adoc#programmatic-login-using-the-programmaticlogin-class[Programmatic Login Using the `ProgrammaticLogin` Class] mechanism.
...
To use a client certificate, configure the `ior-security-config` element in the `payara-ejb-jar.xml` file.
...
These apply only to applications and modules deployed to the server. For more information, see xref:class-loaders.adoc#class-loaders[Class Loaders].
...
+
For more information on deployment, see "xref:../payara-server-documentation/application-deployment/index.adoc#about-deployment-tools[About Deployment Tools]" in the Payara Server Application Deployment section.
...
. Deploy the EJB component to be accessed. For more information on deployment, see "xref:../payara-server-documentation/application-deployment/index.adoc#about-deployment-tools[About Deployment Tools]" in the Application Deployment section.
...
.  Deploy the module.
+
For more information on deployment, see "xref:../payara-server-documentation/application-deployment/index.adoc#about-deployment-tools[About Deployment Tools]" in Payara Server Application Deployment section.
