...
If using Java 17 or higher, you will need to configure the environment variable `VMARGS` with additional options to allow the required reflective access. An example set is as follows:

[source,shell]
----
export VMARGS="--add-opens=java.base/java.io=ALL-UNNAMED
