...
:page-description: Use Request Tracing in applications on Azul Payara — instrument Jakarta EE application code with the Request Tracing API to capture and trace application requests.
:ordinal: 23

Request Tracing in Azul Payara is done either automatically, by using the MicroProfile OpenTracing `@Traced` annotation or by utilizing OpenTelemetry/OpenTracing APIs. This chapter covers manually decorating your applications to enable explicit Request Tracing.
...
CAUTION: You can only inject a Tracer when your application is running inside an Azul Payara runtime you cannot do so from a Java SE client. From a Java SE client you must use the OpenTracing.io `GlobalTracer` class, see xref:using-jakarta-enterprise-beans-technology.adoc#getting-a-tracer[here] for more details.
