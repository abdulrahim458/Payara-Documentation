...
:page-description: Secure Jakarta EE applications on Azul Payara — configure realms, roles, security constraints, and Jakarta Security for authentication and authorization.
:ordinal: 3
...
* Support for https://openid.net/connect/[OpenID Connect^] compatible security features (such as OAuth2).
...
These role names are scoped to the EJB XML deployment descriptors (`ejb-jar.xml` and `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` files) and to the servlet XML deployment descriptors (`web.xml` and `payara-web.xml` files).
...
To map roles to principals and groups, define matching `security-role-mapping` elements in the `glassfish-application.xml`/`payara-application.xml`, `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml`, or `glassfish-web.xml`/`payara-web.xml` file for each `role-name` used by the application.
...
The EJB container is responsible for authorizing access to a bean method by using the security policy laid out in the EJB XML deployment descriptors (`ejb-jar.xml` and `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` files).
...
For applications, you define roles in `@DeclareRoles` annotations or the Jakarta EE deployment descriptor file `application.xml`. You define the corresponding role mappings in the Payara Server deployment descriptor file `glassfish-application.xml`/`payara-application.xml`.

For individually deployed web or EJB modules, you define roles in `@DeclareRoles` annotations or the Jakarta EE deployment descriptor files `web.xml` or `ejb-jar.xml`. You define the corresponding role mappings in the Payara Server deployment descriptor files `glassfish-web.xml`/`payara-web.xml` or `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml`.

For more information regarding Jakarta EE deployment descriptors, see the Jakarta EE Specification. For more information regarding Payara Server deployment descriptors, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#elements-of-the-payara-platform-deployment-descriptors[Elements of the Payara Server Deployment Descriptors]" in the Application Deployment section.

Each `security-role-mapping` element in the `glassfish-application.xml`/`payara-application.xml`, `glassfish-web.xml`/`payara-web.xml`, or `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` file maps a role name permitted by the application or module to principals and groups.
...
* `glassfish-application`/`payara-application` element in `glassfish-application.xml`/`payara-application.xml`
* `web-app` element in `web.xml`
* `as-context` element in `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml`
* `client-container` element in `sun-acc.xml`
* `client-credential` element in `sun-acc.xml`

If modules within an application specify realms, these are ignored. If present, the realm defined in `glassfish-application.xml`/`payara-application.xml` is used, otherwise the domain's default realm is used.

For example, a realm is specified in `payara-application.xml` as follows:
...
For more information about the deployment descriptor files and elements, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#elements-of-the-payara-platform-deployment-descriptors[Elements of the Payara Server Deployment Descriptors]" in the Payara Server Application Deployment section.
...
For more information about class loading in the Payara Server, see xref:class-loaders.adoc#class-loaders[Class Loaders].
...
For more information about web services, see xref:developing-soap-webservices.adoc#developing-web-services[Developing Web Services].
...
a|`security.config`
a|`domain-dir/config/wss-server-config-1.0.xml`
a|
...
a|`debug`
a|`false`
a|If `true`, enables dumping of server provider debug messages to the server log.

a|`dynamic.username.password`
a|`false`
a|If `true`, signals the provider runtime to collect the username and password from the `CallbackHandler` for each request.
...
a|`encryption.key.alias`
a|`s1as`
a|Specifies the encryption key used by the provider. The key is identified by its `keystore` alias.

a|`signature.key.alias`
a|`s1as`
a|Specifies the signature key used by the provider. The key is identified by its `keystore` alias.
...
For more details on message security binding for EJB web services,servlet web services, and clients, see the XML file descriptions in "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#elements-of-the-payara-platform-deployment-descriptors[Elements of the Payara Server Deployment Descriptors]" in the Payara Server Application Deployment section.

[#using-a-signature-to-enable-message-protection-for-all-methods]
==== Using a Signature to Enable Message Protection for All Methods

To enable message protection for all methods using digital signature, update the `message-security-binding` element for the EJB web service endpoint in the application's `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` file.
...
. Modify the `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` file by adding the `message-security-binding` element as shown:
+
[source,xml]
----
<payara-ejb-jar>
...
As in the example discussed in xref:#using-a-signature-to-enable-message-protection-for-all-methods[Using a Signature to Enable Message Protection for All Methods], to enable message protection for a specific method, update the `message-security-binding` element for the EJB web service endpoint in the application's `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` file.

Add the `request-protection` and `response-protection` elements, which are analogous to the `request-policy` and `response-policy` elements discussed in the Azul Payara Security Guide.
...
* Single sign-on applies to web applications configured for the same security realm and virtual server. The realm is defined by the `realm-name` element in the `web.xml` file. For information about virtual servers, see "xref:../payara-server-documentation/general-administration/administering-http-connectivity.adoc#administering-http-connectivity[Administering Internet Connectivity]" in the Payara Server General Administration section.
...
=== Azul Payara and Jakarta Security
...
NOTE: This behaviour is specific to Azul Payara and not compatible with the Jakarta Security specification.
...
a|`Basic`
|Use _BasicAuthenticationMechanism_ as mechanism.

a|`Form`
|Use _FormAuthenticationMechanism_ as mechanism.

a|`CustomForm`
| Use _CustomFormAuthenticationMechanism_ as mechanism.

a|`JWT`
| Use the custom Payara _JWTAuthenticationMechanism_ as mechanism.

a|`Certificate`
| Use the custom Payara _CertificateAuthenticationMechanism_ as mechanism.

a|`Azure`
| Use the custom Payara _AzureOpenIdAuthenticationMechanism_ as mechanism.

a|`Google`
| Use the custom Payara _GoogleOpenIdAuthenticationMechanism_ as mechanism.

a|`OAuth2`
|Use the custom Payara _OAuth2AuthenticationMechanism_ as mechanism.

a|<any Fully Qualified Class Name>
a|Use the `HttpAuthenticationMechanism` indicated by the class name.
...
+
image:security/common-name-as-principal-name.png[alt="Common Name As Principal Name Setting",width=100%]
...
+
image:security/default-certificate-expiration-validation.png[alt="Certificate Expiration Validation Setting",width=100%]
+
IMPORTANT: After setting the value of the property, make sure that you **restart the server instance** for the changes to take effect.
