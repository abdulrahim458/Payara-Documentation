...
:page-description: Debug Jakarta EE applications on Azul Payara — enable JPDA remote debugging, configure debug ports, and attach debuggers to Payara Server and Payara Micro.
:ordinal: 2
...
. To add JPDA options, add any desired JPDA debugging options in Debug Options. See xref:debugging-applications.adoc#jpda-options[JPDA Options].
...
For more information about the `asadmin generate-jvm-report` command, see xref:../payara-server-documentation/command-reference/generate-jvm-report.adoc[`generate-jvm-report`].
...
You can also include JVM options in the `appclient` script directly. For information about the `appclient` script, see xref:../payara-server-documentation/command-reference/appclient.adoc[`appclient`].

NOTE: The Application Client Container is supported only in the Full Profile, not in the Web Profile. See xref:developing-java-clients.adoc#developing-java-clients[Developing Java Clients].
...
NOTE: JMS resources are supported only in the Full Profile, not in the Web Profile. See xref:using-jakarta-messaging.adoc#using-jakarta-messaging[Using Jakarta Messaging].
