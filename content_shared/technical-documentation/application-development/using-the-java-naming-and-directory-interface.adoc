...
:page-description: Use JNDI in Azul Payara applications — look up Jakarta EE resources, EJBs, and custom objects using the Java Naming and Directory Interface in Payara.
:ordinal: 18

A naming service maintains a set of bindings, which relate names to objects. Azul Payara's naming service is based on the Java Naming and Directory Interface (JNDI) API.
...
. A global JNDI name assigned in the `payara-ejb-jar.xml`, `payara-web.xml`, or `payara-application-client.xml` deployment descriptor file has the highest precedence. See xref:#mapping-references[Mapping References].
...
* *`false`*: Enables the automatic use of Payara Server specific JNDI names in addition to portable global JNDI names. This is the default setting.
* *`true`*: Disables the automatic use of specific JNDI names. In all cases, only portable global JNDI names are used.
...
It is possible to create custom resources by using a `payara-resources.xml` or `glassfish-resources.xml` deployment descriptor. To this effect, several template files for these built-in factories and a `README` file are available at `as-install/lib/install/templates/resources/custom/`.
...
For details, see "xref:../payara-server-documentation/application-deployment/deploying-applications.adoc#application-scoped-resources[Application-Scoped Resources]" in the Application Deployment section.

[#using-a-custom-jndi.properties-file]
== Using a Custom `jndi.properties` File

To use a custom `jndi.properties` file, JAR it and place it in the `domain-dir/lib` directory. This adds the custom `jndi.properties` file to the Common class loader. For more information about class loading, see xref:class-loaders.adoc[Class Loaders].
...
These elements are part of the `glassfish-web.xml`/`payara-web.xml`, `glassfish-application-client.xml`/`payara-application-client.xml`, `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml`, and `glassfish-application.xml`/`payara-application.xml` deployment descriptor files.

IMPORTANT: glassfish-* deployment descriptors are deprecated and support will be removed in a future release.
It is recommended to use the payara-* equivalents instead.

For more information about how these elements behave in each of the deployment descriptor files, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#elements-of-the-payara-platform-deployment-descriptors[Elements of the Payara Server Deployment Descriptors]" in the Payara Server Application Deployment section.
