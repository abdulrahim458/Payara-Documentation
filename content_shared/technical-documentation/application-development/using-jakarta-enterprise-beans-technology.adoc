...
:page-description: Use Jakarta Enterprise Beans in Azul Payara — develop stateless, stateful, singleton, and message-driven beans with Payara-specific timer, interceptor, and clustering features.
:ordinal: 9

This chapter describes how Jakarta Enterprise Beans (EJB) technology is supported in Azul Payara.
...
Azul Payara provides a number of value additions that relate to EJB development. References to more in-depth material are included.
...
IMPORTANT: Read-only beans are specific to Azul Payara and are not part of the Enterprise JavaBeans Specification, v2.1. Use of this feature for an EJB 2.1 bean results in a non-portable application. +
...
Payara Server provides a number of ways by which a read-only bean's state can be refreshed. By setting the `refresh-period-in-seconds` element in the `payara-ejb-jar.xml` file and the `trans-attribute` element (or `@TransactionAttribute` annotation) in the `ejb-jar.xml` file, it is easy to configure a read-only bean that is one of the following:
...
The `pass-by-reference` element in the `payara-ejb-jar.xml` file allows you to specify the parameter passing semantics for co-located remote EJB invocations. This is an opportunity to improve performance.

However, use of this feature results in non-portable applications. See "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#pass-by-reference[pass-by-reference]" in the Payara Server Application Deployment section.
...
Bean instances in the free pool transition from a *Pooled* state to a *Cached* state after `ejbCreate` and the business methods run. The size and behavior of each pool is controlled using pool-related properties in the EJB container or the `payara-ejb-jar.xml` file.
...
Pooling and caching parameters for the `payara-ejb-jar.xml` file are described in "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#bean-cache[bean-cache]" in the Payara Server Application Deployment section.
...
An optional element in the `payara-ejb-jar.xml` file, `use-thread-pool-id`, specifies the thread pool that processes the requests for the bean.
...
Since a transaction often spans many method calls, you might want to find out if the updates made by a method succeeded or failed immediately after method execution. To force a flush at the end of a method's execution, use the `flush-at-end-of-method` element in the `payara-ejb-jar.xml` file.
...
See "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#flush-at-end-of-method[flush-at-end-of-method]" in the Payara Server Application Deployment section.
...
The EJB Timer Service configuration can store persistent timer information in any database supported by Payara Server for persistence. For configurations of supported drivers, see "xref:../payara-server-documentation/general-administration/administering-database-connectivity.adoc#configuration-specifics-for-jdbc-drivers[Configuration Specifics for JDBC Drivers]" in the Payara Server General Administration section.
...
* *Minimum Delivery Interval*: Specifies the minimum time in milliseconds before an expiration for a particular timer can occur. This guards against tiny timer increments that can overload the server. The default value is `1000`.
* *Maximum Redeliveries*: Specifies the maximum number of times the EJB timer service attempts to redeliver a timer expiration after an exception or rollback of a container-managed transaction. The default value is `1`.
...
Also refer to "xref:../payara-server-documentation/general-administration/administering-database-connectivity.adoc#enabling-the-jdbc__default-resource-in-a-clustered-environment[Enabling the jdbc/__default Resource in a Clustered Environment]" in the Payara Server General Administration section.

For information about migrating EJB timers, see "xref:../payara-server-documentation/high-availability/administering-payara-server-instances.adoc#migrating-ejb-timers[Migrating EJB Timers]" in the Payara Server High Availability section.
...
image:ejb/set-ejb-timer-persistence.png[alt="Set EJB Persistence to Data Grid",width=100%]
...
A stateless session bean container uses a bean pool to service requests. The Payara Server specific deployment descriptor file, `payara-ejb-jar.xml`, contains the properties that define the pool:
...
For more information about `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml`, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-ejb-jar.xml-file[The payara-ejb-jar.xml File]" in the Payara Server Application Deployment section.
...
The following is a sample `glassfish-ejb-jar.xml`/`payara-ejb-jar.xml` deployment descriptor that configures 2 EJBs with the settings mentioned beforehand.
...
Stateful session beans are managed using an internal cache. The size and behavior of stateful session beans cache are controlled by specifying the following `payara-ejb-jar.xml` parameters:
...
For more information about `payara-ejb-jar.xml`, see "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-ejb-jar.xml-file[The payara-ejb-jar.xml File]" in the Payara Server Application Deployment section.
...
For more information about `server.policy`, see xref:securing-applications.adoc#the-server.policy-file[The `server.policy` File].
...
|JNDI context
a|Yes, `InitialContext` and `java:comp/env`
...
xref:using-the-java-naming-and-directory-interface.adoc#accessing-the-naming-context[Accessing the Naming Context]. For more information about transaction recovery, see xref:using-the-transaction-service.adoc#using-the-transaction-service[Using the Transaction Service]. For more information about Administered Objects, see "xref:../payara-server-documentation/general-administration/administering-the-java-message-service.adoc#administering-jms-physical-destinations[Administering JMS Physical Destinations]" in the Payara Server General Administration section.
...
To enable SFSB checkpointing at the application or EJB module level during deployment, use the `asadmin deploy` command with the `--availabilityenabled` option set to `true`.

[#sfsb-level]
===== SFSB Level

To enable SFSB checkpointing at the bean level, set `availability-enabled="true"` in the `ejb` element of the bean definition in the `payara-ejb-jar.xml` file as follows:
...
To specify additional optional checkpointing of SFSBs at the end of non-transactional business methods that cause important modifications to the bean's state, use the `checkpoint-at-end-of-method` element within the `ejb` element in `payara-ejb-jar.xml`.
...
For details, see "xref:../payara-server-documentation/application-deployment/elements-of-deployment-descriptors.adoc#checkpoint-at-end-of-method[checkpoint-at-end-of-method]" in the Payara Server Application Deployment section.
...
* `ejbLoad` is called only when a transactional method is called or when the bean is initially created (in the cache), or at regular intervals controlled by the bean's `refresh-period-in-seconds` element in the `payara-ejb-jar.xml` file.
...
Use the `refresh-period-in-seconds` element in the `payara-ejb-jar.xml` file to refresh a read-only bean periodically.
...
Read-only beans are deployed in the same manner as other entity beans. However, in the entry for the bean in the `payara-ejb-jar.xml` file, the `is-read-only-bean` element must be set to `true`. That is:
...
Also, the `refresh-period-in-seconds` element in the `payara-ejb-jar.xml` file can be set to some value that specifies the rate at which the bean is refreshed. If this element is missing, no refresh occurs.
...
For further information on these elements, refer to "xref:../payara-server-documentation/application-deployment/deployment-descriptor-files.adoc#the-payara-ejb-jar.xml-file[The payara-ejb-jar.xml File]" in the Payara Server Application Deployment section.
...
For information about setting up load balancing for message-driven beans, see xref:using-jakarta-messaging.adoc#load-balanced-message-inflow[Load-Balanced Message Inflow].
...
The `mdb-connection-factory` element in the `payara-ejb-jar.xml` file for a message-driven bean specifies the connection factory that creates the container connection to the JMS provider.

The `jndi-name` element of the `ejb` element in the `payara-ejb-jar.xml` file specifies the JNDI name of the administered object for the JMS Queue or Topic destination that is associated with the message-driven bean.

[#message-driven-bean-pool]
==== Message-Driven Bean Pool

The container manages a pool of message-driven beans for the concurrent processing of a stream of messages. The `payara-ejb-jar.xml` file contains the elements that define the pool (that is, the `bean-pool` element):
...
For details, see "xref:../payara-server-documentation/general-administration/administering-the-monitoring-service.adoc#administering-the-monitoring-service[Administering the Monitoring Service]" in the Payara Server General Administration section.
...
When tuning performance and resource usage, make sure to consider potential JMS provider properties for the connection factory used by the container (the `mdb-connection-factory` element in the `payara-ejb-jar.xml` file). For example, you can tune the Open Message Queue flow control related properties for connection factory in situations where the message incoming rate is much higher than `max-pool-size` can handle.

Refer to "xref:../payara-server-documentation/general-administration/administering-the-monitoring-service.adoc#administering-the-monitoring-service[Administering the Monitoring Service]" in the Payara Server General Administration section for information on how to get message-driven bean pool statistics.
...
Because xref:../payara-server-documentation/high-availability/administering-deployment-groups.adoc[deployment groups] do not need to share configurations amongst their instances, there are several configuration and operational constraints that are not enforced by the server.

To ensure correct behaviour of persistent timers within a deployment group, the following conditions need to be met:

* The EJB Timer Service needs to use shared storage --an external database-- or xref:../payara-server-documentation/high-availability/administering-the-domain-data-grid.adoc[the DataGrid].
* All instances of a deployment group, and the Domain Administration Server MUST share the same configuration settings for the EJB Timer Service (see xref:using-jakarta-enterprise-beans-technology.adoc#ejb-timer-service[here] for more information).
...
WARNING: Manual registration of a tracer instance is only required if using a third-party tracer such as ZipKin or Jaeger - if using the built-in Azul Payara Request Tracing service the registration will automatically happen during creation of the initial context.
