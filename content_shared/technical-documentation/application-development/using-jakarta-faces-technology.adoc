...
As a special case of how the classloader hierarchy can be modified (see xref:class-loaders.adoc#configuring-delegation[configuring classloading delegation]), the Payara Platform allows the use of a Jakarta Faces implementation that is bundled on a web application.
