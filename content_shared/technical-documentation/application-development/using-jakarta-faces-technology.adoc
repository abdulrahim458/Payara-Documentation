...
:page-description: Use Jakarta Faces in Azul Payara — develop component-based UI web applications with Jakarta Faces (formerly JSF) on Payara Server with Payara-specific configuration.
:ordinal: 8
...
It is possible to specify an Azul Payara specific JSF context parameter in the `web.xml` deployment descriptor, named `fish.payara.faces.enableParallelInit` to provide parallel initialization benefits to the application in question.
...
IMPORTANT: As an Azul Payara specific implementation this will use the default Platform Concurrency executor if there is one, if not it will create a single use executor with 5 threads.

[#use-bundled-jsf]
== Using an Alternative Jakarta Faces Implementation

As a special case of how the classloader hierarchy can be modified (see xref:class-loaders.adoc#configuring-delegation[configuring classloading delegation]), Azul Payara allows the use of a Jakarta Faces implementation that is bundled on a web application.
