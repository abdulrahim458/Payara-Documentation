= Using Jakarta Mail
:page-description: Use Jakarta Mail in Azul Payara — inject mail sessions, send email from Jakarta EE applications, and configure SMTP, IMAP, and POP3 mail resources in Payara.
:ordinal: 20
...
For more information, see "xref:../payara-server-documentation/general-administration/administering-the-jakarta-mail-service.adoc[Administering the Jakarta Mail Service]" in the Payara Server General Administration section and the https://jakarta.ee/specifications/mail[Jakarta Mail specification^].
...
For more information about the JNDI API, see xref:using-the-java-naming-and-directory-interface.adoc#using-a-custom-jndi.properties-file[Using the Java Naming and Directory Interface].
