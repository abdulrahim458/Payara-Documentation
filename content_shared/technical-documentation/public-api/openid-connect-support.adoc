...
:page-description: OpenID Connect support in Azul Payara — configure OIDC-based authentication with identity providers using the Payara OpenID Connect security connector and annotations.
...
[cols="2,3,1,3,1,1", options="header"]
|===
| Option | MP Config property | Required/Type | Description | Default value | Requirements

a| `providerURI`
a| `payara.security.openid.providerURI`
a| `false`
a| The provider URI to discover the metadata of the OpenID Connect provider. If not specified, metadata are read from `providerMetadata`.
|
| The endpoint must be HTTPS.

a| `providerMetadata`
|
a| required if `providerURI` not specified, type: `@OpenIdProviderMetadata`
a| Alternative way to specify metadata if auto-configuration from `providerURI` can't be used.
a| `payara.security.openid.provider.*`
| The endpoint must be HTTPS.

a| `clientId`
a| `payara.security.openid.clientId`
a| `true`
...
a| `clientSecret`
a| `payara.security.openid.clientSecret`
a| `true`
...
a| `redirectURI`
a| `payara.security.openid.redirectURI`
a| `true`
a| The URL to redirect the user to upon successful authentication. Can include the variable `${baseURL}`, which is replaced by the URL of the application's context path (e.g. `https://myhost:8080/myapp`)
a| `${baseURL}/Callback`
| Must be equal to one set in the OpenID Connect provider.

a| `scope`
a| `payara.security.openid.scope`
a| `false`
...
a| `responseType`
a| `payara.security.openid.responseType`
a| `false`
...
a| `responseMode`
a| `payara.security.openid.responseMode`
a| `false`
...
a| `prompt`
a| `payara.security.openid.prompt`
a| `false`
...
a| `display`
a| `payara.security.openid.display`
a| `false`
...
a| `useNonce`
a| `payara.security.openid.useNonce`
a| `false`
...
a| `useSession`
a| `payara.security.openid.useSession`
a| `false`
| If enabled state & nonce value stored in session otherwise in cookies.
a| `true`
| N/A.

a| `jwksConnectTimeout`
a| `payara.security.openid.jwks.connect.timeout`
a| `false`
...
a| `jwksReadTimeout`
a| `payara.security.openid.jwks.read.timeout`
a| `false`
...
a| `tokenAutoRefresh`
a| `payara.security.openid.token.autoRefresh`
a| `false`
...
|
a| Must be in the form `key=value` and follow URL query pattern, i.e. `key1=value1&key2=value2&key2=value+with+spaces`. Key entries can be repeated
...
a| `payara.security.openid.provider.endSessionEndpoint`
a| OIDC provider’s logout endpoint URL. If set, overrides the URL obtained via the `end_session_endpoint` element of the OIDC provider’s metadata.
|

a| `disableScopeValidation`
a| `payara.security.openid.disableScopeValidation`
...
IMPORTANT: By default, the EL expressions are evaluated only once after the application is loaded and the evaluated values are then remembered until the application is reloaded, for performance reasons. This means that although the configuration can be evaluated dynamically the first time it's needed, it's not possible to change the configuration later on. +
If you need to dynamically modify the configuration during the lifetime of the application, follow the next section about multi-tenancy support.

[#multitenancy]
== Multi-Tenancy Support (Session-Scoped Configuration)

By default, the same configuration of the OpenID connector is applied for the whole application, for all authentication attempts. This is for performance reasons. The OpenID connector also supports re-evaluating the configuration for each user session, before each authentication attempt.

This is useful in a multi-tenant scenario to define a different configuration for each tenant. It's also useful if the user should be able to select which provider they want to use to authenticate.
...
* Use the tenant name to read the configuration value from the respective xref:../microprofile/config/index.adoc[MicroProfile Config] property
...
a| `notifyProvider`
a| `payara.security.openid.provider.notify.logout`
a| `false`
a| Notify the OIDC provider (OP) that the user has logged out of the application and might want to log out of the OP as well.
...
a| `redirectURI`
a| `payara.security.openid.logout.redirectURI`
a| `false`
a| The post logout redirect URI to which the RP is requesting that the End-User's User Agent be redirected after a logout has been performed.

If the redirect URI is empty then redirect to OpenID connect provider _authorization_endpoint_ for re-authentication.
|

a| `accessTokenExpiry`
a| `payara.security.openid.logout.access.token.expiry`
a| `false`
| Whether the application session times out when the Access Token expires.
a| `false`

a| `identityTokenExpiry`
a| `payara.security.openid.logout.identity.token.expiry`
a| `false`
...
If `fish.payara.security.annotations.LogoutDefinition#notifyProvider` is set to true then it redirects the End-User's User Agent to the `end_session_endpoint` to notify the OP that the user has logged out of the RP's application.

It will also ask the user whether they want to log out from the OP as well. After successful logout, the End-User's User Agent redirects back to the RP's `_post_redirect_uri_` configured via the `fish.payara.security.annotations.LogoutDefinition#redirectURI` property.
...
Moreover, the proxy configuration details (`hostName` and `port`) can be dynamically utilized to define the `redirectURI` within the `OpenIdAuthenticationDefinition`, replacing the `baseURL` placeholder if present. This allows for a more flexible setup, especially in scenarios involving reverse web proxies.
...
** -> *Outcome:* The generated `redirectURI` should include the request's host and port. The registered Authorized Redirect URI in Google Cloud must contain a URL with the Payara Server instance's host and port.
...
a| `jwksURI`
a| `payara.security.openid.provider.jwksURI`
a| `true`
| An OIDC Provider's JSON Web Key Set document.

a| `scopesSupported`
a| `payara.security.openid.provider.scopesSupported`
| recommended
a| List of the OAuth 2.0 scope values that this server supports, e.g. `openid`.

a| `responseTypesSupported`
a| `payara.security.openid.provider.responseTypeSupported`
a| `true`
a| List of the OAuth 2.0 response_type values that this OIDC Provider supports, e.g. `code`, `id_token`, `token id_token`.

a| `subjectTypesSupported`
a| `payara.security.openid.provider.subjectTypesSupported`
a| `true`
a| List of the Subject Identifier types that this OIDC Provider supports. Valid types include `pairwise` and `public`.

a| `idTokenSigningAlgValuesSupported`
a| `payara.security.openid.provider.idTokenSigningAlgorithmsSupported`
a| `true`
a| List of the JWS signing algorithms (algorithm values) supported by the OIDC Provider for the ID Token to encode the Claims in a JWT, e.g. `RS256`.

a| `idTokenEncryptionAlgValuesSupported`
a| `payara.security.openid.provider.idTokenEncryptionAlgValuesSupported`
a| `false`
a| List of the JWE encryption algorithms (`alg` values) supported by the OIDC Provider for the ID Token to encode the Claims in a JWT.

a| `idTokenEncryptionEncValuesSupported`
a| `payara.security.openid.provider.idTokenEncryptionEncValuesSupported`
a| `false`
a| List of the JWE encryption algorithms (`enc` values) supported by the OIDC Provider for the ID Token to encode the Claims in a JWT.

a| `claimsSupported`
a| `payara.security.openid.provider.claimsSupported`
| recommended
...
The client secret can be input directly, or for added security it can be aliased using xref:../microprofile/config/index.adoc[Microprofile Config]
...
As the OpenId Connect Client is built on top of Jakarta EE Security API, therefore `jakarta.security.enterprise.SecurityContext` interface can provide caller info which is available as a CDI bean and can be injected into any context-aware instance.
...
a| `userClaimsFromIDToken`
a| `payara.security.openid.userClaimsFromIDToken`
a| `false`
...
By default, the OpenID Connect connector validates that a scope is reported as a supported scope by the provider. However, some providers support more scopes than they actually report as supported scopes.

To disable the validation and allow using such scopes, it's possible to use the `disableScopeValidation` property of `OpenIdProviderMetadata`. See the <<configuration>> section for more details about this option.
...
In order to authenticate and authorize calls between services using the OpenID mechanism, it is possible to use authorization compatible with link:https://datatracker.ietf.org/doc/html/rfc6750[RFC 6750^].

In this case, the access token presented to the resource service is an JWT token that is used to verify that the caller has access to OAuth2 protected resources.

[#obtaining-token]
=== Obtaining JWT Token

Obtaining the token is specific to the OAuth provider and the application. The usual approach is using link:https://datatracker.ietf.org/doc/html/rfc6749#section-4.4[Client Credentials Grant^], where an application posts its clientId and secret to identity provider and receives access and refresh tokens in return.
...
The resource service is required to map the information in the JWT token to groups utilizing the `https://jakarta.ee/specifications/platform/10/apidocs/jakarta/security/enterprise/identitystore/identitystore[IdentityStore^]` interface. The OpenID connector provides the following classes to make this process possible:
...
If application requests `offline` access then the application can receive access and refresh token. Once the application has a refresh token, it can obtain a new access token at any time or as older ones expire.

Otherwise, if an application requests `online` access, your application will only receive an access token
...
+
image:security-connector/oidc/azure/app_registrations.png[alt="Select application",width=100%]

. You may also add the custom roles via **Roles and administrators** under the **Manage** section:
+
image:security-connector/oidc/azure/custom_role.png[alt="Add Custom Roles",width=100%]

. Now to map group claims, select **Token configuration** under the **Manage** section:
+
image:security-connector/oidc/azure/token_configuration.png[alt="Token configuration",width=100%]

. Press **Add groups claim** button to select group types and customize the ID and/or Access Token properties:
+
image:security-connector/oidc/azure/add_groups_claim.png[alt="Add Groups Claim",width=100%]
...
Keycloak is an Open Source Identity and Access Management Server, which is a OAuth2 and OpenID Connect(OIDC) protocol complaint. In this section,

the basic steps are described to set up a Keycloak OpenId provider:

. Refer to the Keycloak https://www.keycloak.org/docs/latest/getting_started/index.html[getting started documentation^] to run and setup Keycloak.

. After Keycloak setup done, login to Keycloak admin console and add the new realm by pressing the **Add Realm** button:
+
image:security-connector/oidc/keycloak/add-realm.png[alt="Add Realm",width=100%]

. Copy the OpenId endpoint configuration URL from endpoint section:
+
image:security-connector/oidc/keycloak/realm-endpoint.png[alt="Realm Endpoint",width=100%]

. Now add the **Role** that will be used by the application to define which users will be authorized to access the application.
+
image:security-connector/oidc/keycloak/add-role.png[alt="Add role",width=600px]

. Create the **Groups**:
+
image:security-connector/oidc/keycloak/create-group.png[alt="Create Groups",width=100%]

. Add the **User**:
+
image:security-connector/oidc/keycloak/add-user.png[alt="Add User",width=100%]

. After the user is created, set a new **password** for the user:
+
image:security-connector/oidc/keycloak/set-user-password.png[alt="Set Password",width=100%]

. Now map the user to roles. Click on **Role Mappings** tab and assign the roles to the user from the available roles:
+
image:security-connector/oidc/keycloak/user-role-mapping.png[alt="User Role Mapping",width=100%]

. Assign the user to the groups. Click on **Groups** tab and join the groups from the available groups:
+
image:security-connector/oidc/keycloak/join-group.png[alt="Join Groups",width=100%]

. Create the OpenId Client by clicking the **Client** option from sidebar and press the **create** button:
+
image:security-connector/oidc/keycloak/create-client.png[alt="Create OpenID Client",width=100%]
...
+
image:security-connector/oidc/keycloak/client-access-type-confidentail.png[alt="Access Type",width=100%]

. Next copy the client **secret** from **Credentials** tab.
+
image:security-connector/oidc/keycloak/client-secret.png[alt="Client Secret",width=100%]
...
----
@OpenIdAuthenticationDefinition(
    providerURI = "https://${keycloak-host}:${keycloak-port}/auth/realms/test-realm",
...
+
image:security-connector/oidc/keycloak/groups-claim.png[alt="Groups Claim",width=100%]
...
* xref:../microprofile/jwt.adoc[MicroProfile JWT Authentication API]
