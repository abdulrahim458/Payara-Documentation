...
:page-description: @RolesPermitted support in Azul Payara — role-based authorization annotation from the Payara Public API for securing Jakarta EE beans and REST endpoints.
...
a| `String`
| no
a| Whether accessing caller must be in any one of the given roles (`OR`) or all given roles (`AND`). Defaults to `OR`.
...
* https://jakarta.ee/specifications/platform/10/apidocs/jakarta/annotation/security/rolesallowed[@RolesAllowed annotation documentation^]