= Clustered Singleton
:page-description: Clustered Singleton in Azul Payara — create EJB singleton beans that maintain a single instance across all nodes in a Payara Server cluster or Payara Micro data grid.
...
. Hazelcast is required to be enabled (default)
...
IMPORTANT: It is also highly recommended to add a `serialVersionUID` variable to the `@Clustered @Singleton` class, in order to verify that a loaded class and the serialized object are compatible. A previously serialized entry in the Data Grid is compatible with an updated version of the same class.
...
Alternatively, you can use the `payara-ejb-jar.xml` to configure a Singleton EJB to be cluster-wide.
...
An example for a Singleton EJB using `payara-ejb.jar` (`clustered-key-name` is optional):
...
| lock
| clustered-lock-type
a| The type of distributed locking to be performed.
...
a| `clustered-attach-postconstruct`
a| Whether to call `@PostConstruct` each time the bean is created on a different node. Will result in multiple calls.
a| `true`

| callPreDestroyOnDetach
a| `clustered-detach-predestroy`
a| Whether to call `@PreDestroy` when the singleton is destroyed on an instance while still being available on another. Will result in multiple calls.
a| `true`
